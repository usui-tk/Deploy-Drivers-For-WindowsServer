...
param(
    ...
    #   Earlier revisions shipped a non-standard value ({B355481F-...})
    #   and later the Windows-shipped default; both defaults are history.
...
$Script:ScriptVersion = 'graphics-2026.08.10-r86'
...
if ([string]::IsNullOrWhiteSpace($LogFile)) {
    ...
    $autoLogLeaf  = ('Deploy-AMDGraphicsDriverOnWindowsServer_{0}_{1}_{2}.log' -f $Action, $autoLogStamp, $PID)
...
#   - BasePolicyID: operator-supplied ONLY via -WdacBasePolicyGuid -
#                  there is no default base policy identity any more
#                  (third-party audit finding C-02; SPEC D.58.8).
#                  Empty means the supplemental path is refused by
#                  Test-WdacSupplementalPolicyAdmissible.
$Script:WdacPolicyGuidDefault     = '85336828-3080-41C5-81EC-FD587DC090D3'

# Resolved values (operator override if non-empty; base has no default).
    ...
    if ($allHits.Count -gt 0) {
        ...
        if ($BranchPreference -eq 'Legacy') {
            ...
            if ($f.Count -gt 0) {
                ...
                Write-Caution 'Branch preference is Legacy but no vega-polaris URL was found on any landing page.'
                Write-Caution 'Falling back to MAIN branch - this driver almost certainly will NOT bind to your Vega-era GPU.'
                Write-Caution 'Consider supplying -InstallerUrl with a manually-located Vega/Polaris legacy installer URL.'
            ...
            if ($f.Count -gt 0) {
                ...
                Write-Caution 'Branch preference is Main but only legacy vega-polaris URLs were found.'
...
function Invoke-PrepPhase05_AnalyzeInfs { # psa-disable-line PSA6003 -- compound noun (e.g., Policies, Drivers, Catalogs) is semantically plural for set-returning helpers
    ...
    Set-PhaseMarker -Ctx $Ctx -PhaseId 'P05' `
        -Metadata @{ Total=$totalAll; Selected=$totalSelected; CsvPath=$csvPath; ReportPath=$reportTxtPath; JsonPath=$jsonPath; Variants=($preferredVariants -join ',') } `
