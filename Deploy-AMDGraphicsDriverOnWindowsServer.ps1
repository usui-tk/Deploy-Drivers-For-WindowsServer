...
param(
    ...
    #   replaced. Without this switch, I02 ABORTS with a 3-option
    #   guidance message when a foreign policy is detected. See SPEC
    #   D.25 P3.
    [switch]$ForceOverrideForeign,

    # -AuditMode:
    #   Deploy the SPF policy with WDAC Rule Option 3 (Enabled:Audit
    #   Mode) instead of Enforced. Violations are logged but kernel-
    #   mode drivers are NOT blocked. Useful for pre-production
    #   verification on a legacy host.
    [switch]$AuditMode,
...
$Script:ScriptVersion = 'graphics-2026.05.22-r33'
...
function Invoke-InstPhase02_AuthorizeDriverSigning {
    ...
    # Policy Format (MPF) infrastructure that WS2022+ uses (no CiTool,
    # no supplemental policies). WDAC on these OSes uses the Single
    # Policy Format (SPF) deployed at SiPolicy.p7b. We delegate to the
    # external orchestrator. See SPEC D.25 for details.
