...
# scripts and used as the model for this section), AMD does NOT publish
# a structured public query API for driver discovery. AMD's driver-
# distribution model has three exposed paths, only the first two of
...
function Get-LatestAmdGraphicsUrl {
    ...
    if ($allHits.Count -gt 0) {
        ...
        Write-Host ("    Branch inventory: {0} legacy (vega-polaris) / {1} main / {2} total unique" -f `
            $legacyCount, $mainCount, $totalUnique) -ForegroundColor DarkGray
        Write-Host ("    Branch preference (from GPU detection): {0}" -f $BranchPreference) -ForegroundColor DarkGray
...
function Invoke-InstPhase02_AuthorizeDriverSigning {
    ...
    if ($useWdac) {
        ...
        Write-Host  '    .\Deploy-AMDGraphicsDriverOnWindowsServer.ps1 -Action Cleanup' -ForegroundColor DarkGray
