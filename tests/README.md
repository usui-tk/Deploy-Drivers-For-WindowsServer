...
| `Test-DownloadOverrideSeparation.ps1` | Gate G-09: the download-verification override is separated from `-Force` (dedicated `-AllowUnverifiedDownload` on chipset/graphics reaching only the AMD installer sites; Microsoft/7-Zip sites machine-pinned unconditionally fail-closed; the shared gate carries no `$Force` reference and returns a record) and every gate call writes R5-M02-shaped SourceArtifact evidence; retired-phrase detector carries a synthetic self-check |
| `Test-ResearchDeploymentParity.ps1` | Gate G-20: the inventory-reconciliation tool satisfies the parity exit criterion on synthetic fixtures — verbatim classification counts, the suffix-versioned UTF-16LE-no-BOM variant path, a built-in negative control naming the unexplained row (rc=1), the operator-adjudicated allowlist path, report schema/SHA pins, and a committed-baseline parse smoke |
| `Test-ExtractionGraphCompleteness.ps1` | Gate G-17: static extraction shadow port integrity — the chipset-embedded span is byte-identical to the fragment file (markers included), structural pins (queue traversal, MaxDepth bounds, SHA dedup, four-status vocabulary, zero-INF never complete), prohibited-capability scan, measured C#/resolver/orchestrator behavior with the 13-field container schema and a built-in named-field negative, report-only research decoder agreement, and P04 wiring outside every canon region |
| `Test-ThirdPartyAttributionRetention.ps1` | Gate G-22: ISx attribution travels with the migrated code — in-span attribution in both copies, the repository-root `THIRD-PARTY-NOTICES.md` with the full MIT text and the research-notices cross-reference, README references in both languages, and an attribution-stripped synthetic negative |
| `Test-PhaseMarkerFingerprint.ps1` | Gate G-18: content-addressed phase markers v2 - pure fingerprint fixtures, C/G/M byte identity of the marker machinery, live v2 hit/miss round-trip (a schema-less v1 marker and a tampered fingerprint are misses), and call-site pins incl. the SHA-verified P03 hit with LastWriteTime gone |
| `Test-PackageInventoryIdentity.ps1` | Gate G-19: canonical `package-inventory.json` - C/G byte identity of the inventory helpers, generator pins (InfSha256 / SourceArtifactSha256 / ParseFailed path / JSON-before-CSV / JSON-first rehydration), property-preserving flat projection for both sisters, JSON round-trip with nested evidence, stripped-field negative |
| `Test-UnknownNotCompatible.ps1` | Gate G-21: the five-value WDF observation vocabulary is pinned verbatim in C/G; Declared is emitted iff a version was observed; all non-Declared states survive without coercion; Declared-without-versions, out-of-vocabulary and empty-string statuses are named |
| `Test-DeploymentPlanImmutability.ps1` | Gate G-23: immutable `deployment-plan.json` - C/G byte identity of the seven plan functions, pure row/document fixtures (SelectionReason / ExpectedMutation / trim suffix / completeness honesty / co-sign lookup), schema pins with a loadability-claim field ban, writer-once + plan-driven P06 loop pins, marker plan-SHA chain, report-only P08/I03 alignment, tampered-plan and out-of-plan-INF negatives |
| `Test-StaticExtractionNoExecution.ps1` | Gate G-16 (shadow scope): AST walk proving the seven shadow functions and the P04 shadow try block never invoke a process launch, msiexec, or the executable extraction strategies; catch clause pinned (fail-open); synthetic-positive and variable-invocation self-checks |
