...
| ルール (重要度)                            | Chipset | Graphics | NPU | BthPan | 処置 |
| ----------------------------------------- | ------: | -------: | --: | -----: | --- |
| `PSA4004` (行末セミコロン、 info)         |   31    |    37    |  0  |   31   | 装飾的。 多数リビジョンの蓄積した既存スタイル。 r59/r27/r9/r9 同期では未対応。 NPU は trailing-semicolon スタイルが推奨されなくなった後に作成されたためクリーン (0)。 |
| `PSA3004` (空 `catch`、 warning)          |   31    |    31    | 13  |   29   | fail-soft リトライとベストエフォート診断取得の混在。 本同期では個別アノテーション未実施。 Chipset r55 / Graphics r23 で追加された `finally` ブロック内のロック解放 catch は `# psa-disable-line PSA3004` でインライン抑制済みのため、この件数には含まれない。 |
| `PSA6003` (複数形名詞関数、 warning)      |   14    |    15    | 13  |   16   | 既存の公開関数名。 リネームは破壊的変更となる。 |
| `PSA2003` (-match against `$null`、w.)    |    6    |     7    |  4  |    4   | 検査した全箇所で、 `-match` の対象がスクリプトスコープの定数パターン (絶対に `$null` にならない)。 警告は技術的には正しいが運用上は known-good 形状。 |
| `PSA3001` (Start-Process -ArgumentList、w.) | 4 |    3    |  0  |    9   | 既存のラッパー関数。 引数は shell メタ文字を含まず安全に構築。 Chipset は 4 件 — r54 の `Expand-AmdInstaller_ViaInstallShield` がサブ MSI ごとの `msiexec /a` admin install のために 4 件目の呼び出しを追加した。 BthPan の 9 件は `pnputil` / `signtool` 呼び出しでの `Start-Process` の広範な利用を反映。 |
| `PSA2001` (未初期化変数、 error)          |    7    |     7    |  0  |    2   | すべて `[switch]` パラメータ (`$Force`、 `$CleanWorkRoot`、 `$UseTestSigning`、 `$AllowWorkstationInstall`、 `$ExportTraceOnExit`) で、 `$switch.IsPresent` アクセスを PSA が誤分類したもの。 既知の false positive として文書化。 |
| `PSA1001` (ブレース整合性、 error)        |    1    |     1    |  0  |    0   | Chipset と Graphics は特定の here-string 境界で PSA1001 false-positive を 1 件ずつ持つ; NPU と BthPan はクリーン。 |
| `PSA4001` (複数文/行、 info)              |    1    |     1    |  0  |    1   | 各 1 箇所の既存サイト; 装飾的。 |
| `PSA2002` (未使用パラメータ、 w.)         |    0    |     0    |  0  |    3   | BthPan のみ; 3 つのヘルパー関数が将来の拡張フック用の `$Ctx` パラメータを受け取るが現状未使用。 forward-compat のため文書化。 |
