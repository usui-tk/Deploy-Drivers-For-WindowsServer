...
各スクリプトは、 スクリプト間の衝突を防ぐため、 専用の workspace path `C:\AMD-<short>-WS\` (Microsoft inbox BthPan スクリプトは `C:\MSBthPan-WS\`) 配下に書き込みます:
...
5 番目のスクリプト (例: ROCm runtime、 audio coprocessor) を追加する場合は、 同じサブディレクトリレイアウト (`download\`、 `extracted\`、 `patched\`、 `cert\`) で `C:\AMD-<short>-WS\` (AMD 以外のドライバ向けには相当する vendor-prefix path) を使用してください。
