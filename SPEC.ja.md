...
各スクリプトは、 スクリプト間の衝突を防ぐため、 専用の workspace path `C:\Temp\Workspace_<vendor>-<short>\` 配下に書き込みます。 Chipset r58 / Graphics r26 / NPU r8 / BthPan r2 から、 4 つの workspace はすべて `C:\Temp\Workspace_*` 配下に再配置されており、 `C:\Temp` がない場合はスクリプトが自動作成します:
...
> **歴史的経緯**: チップセット r57 / グラフィックス r25 / NPU r7 (2026-05-17) より前は、 この SPEC §A.5 / §D.5 要件は**ドキュメント化されていたものの、 スクリプトに実装されていません**でした。 `Show-PowerShellEnvironment` が*現在の*エンコーディングを表示するのみで、 変更は行っていませんでした。 修正は外部ツール stdout を取り込むすべての phase (I02, I03) より前に必須となりました。
...
Chipset r58 / Graphics r26 / NPU r8 / BthPan r2 から、 4 つのスクリプトは `-LogFile <path>` パラメータを公開し、 スクリプト内部で `Start-Transcript` / `Stop-Transcript` を呼び出します。 これは実行ログを保持する正本機構として位置付けられ、 レガシーな `... *>&1 | Tee-Object -FilePath ...` イディオムの後継です。 採用理由は 2 つ:
...
| `PSA3004` (空 `catch`、 warning)          |   28    |    28    |  9  | fail-soft リトライとベストエフォート診断取得の混在。 本同期では個別アノテーション未実施。 psa.py v3.1.0 で再計測した件数。 Chipset r55 / Graphics r23 で追加された `finally` ブロック内のロック解放 catch は `# psa-disable-line PSA3004` でインライン抑制済みのため、この件数には含まれない。 |
...
| `PSA3001` (Start-Process -ArgumentList、w.) | 4 |    3    |  0  | 既存のラッパー関数。 引数は shell メタ文字を含まず安全に構築。 Chipset は 4 件 — r54 の `Expand-AmdInstaller_ViaInstallShield` がサブ MSI ごとの `msiexec /a` admin install のために 4 件目の呼び出しを追加した。 |
...
- **現リビジョン**: `chipset-2026.05.17-r57` (tag: `chipset-citool-json-and-pnputil-259-r57`)
...
- **現リビジョン**: `graphics-2026.05.17-r25` (tag: `graphics-citool-json-and-pnputil-259-r25`)
...
- **現リビジョン**: `npu-2026.05.17-r7` (tag: `npu-citool-json-and-console-utf8-r7`)
...
**修正 (Chipset r57 / Graphics r25 / NPU r7)**: P00 で `Set-ConsoleUtf8` を呼び出し、 3 つのエンコーディング (`[Console]::OutputEncoding`、 `[Console]::InputEncoding`、 `$OutputEncoding`) すべてを `[System.Text.Encoding]::UTF8` に強制。 `*>&1 | Tee-Object` を使う operator は、 ファイルエンコーディングも明示的に設定する必要がある。 正本実装は §A.5 を参照。
...
## D.16 Chipset r57 / Graphics r25 / NPU r7 — CiTool.exe の対話 ENTER プロンプト + Console UTF-8 強制
...
1. 最新の既存スクリプト (NPU r7 が最も新しい姉妹整合 reference) を出発テンプレートとしてコピー。
