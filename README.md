...
**Latest release: `2026-08-08` — Chipset r96 / Graphics r62 / NPU r39 (unchanged) / BthPan r44** (`path-a-plan-semantics-fixes`): the first field execution of `-SkipNonCosignedDrivers` (Path A, Secure Boot ON, WS2019) failed before installing anything — the P06 trim crashed on the real inventory schema, its eligibility rule would have emptied a 119-INF plan to 0 even without the crash, and the WHQL analysis never survived the `PrepareVerify` -> `Install` process boundary, aborting I01 on a missing PFX. r96 makes the trim schema-tolerant, corrects the semantics (vendor-catalog / copy-only INFs are always eligible), persists the analysis and the post-trim plan as workspace JSONs (`whql_cosign_analysis.json` / `whql_cosign_plan.json`), and skips I01 entirely when the plan contains no re-signed catalogs — the trust stores are left untouched. Post-mortem: SPEC D.40; contract: SPEC D.31.17.
