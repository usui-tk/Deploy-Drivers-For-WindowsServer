...
**Latest release: `2026-05-26` — Chipset r83 / Graphics r49 / BthPan r31 / NPU r27** (`npu-state-model-refactor-step-1-wdac-helpers`).
This release begins the **NPU state-model refactor** — the multi-stage workstream that brings the NPU script into the same `$Ctx`-based state-passing model used by Chipset / Graphics / BthPan, and that will ultimately let the five Tier B-4 helpers (`Get-OrEnsureSecureBootBaseline`, `Get-BootSigningEnvironment`, `Show-BootSigningEnvironment`, `Invoke-Cleanup`, `Resume-CtxFromWorkspace`) join Tier A. Stage 1 is a **setup release**: it ports the prerequisite WDAC tooling helpers from Chipset to NPU and constructs the canonical `$Ctx` skeleton (currently unused; placeholder for stage-2 consumers). **No runtime behaviour changes** — the existing `$Script:` globals continue to drive every phase function and every Tier B-4 helper.

Key changes (see CHANGELOG for the full entry):

- **Tier A roster: 36 → 38 functions.** Two universal WDAC tooling preflight helpers (`Test-WdacToolsAvailable`, `Get-ActiveCodeIntegrityPolicies`) are ported from Chipset / Graphics / BthPan into NPU; they are now byte-identical across all four scripts and graduate to Tier A (PSA8001-enforced). The Tier A inventory now includes a new "WDAC tooling preflight helpers (2)" family heading.

- **Three AMD-prefix WDAC helpers also ported to NPU (Tier B-3).** `Get-AmdSuppPolicyMarkerPath`, `Test-AmdWdacPolicyDeployed`, and `Uninstall-AmdWdacPolicy` are ported from Chipset to NPU verbatim. They are now byte-identical across the three AMD-family scripts (Chipset / Graphics / NPU); BthPan ships parallel MS-prefix variants by design (per-family isolation principle). These three helpers stay in `psa8001_ignore_functions` because Tier A requires 4-way identity which the helper-pair design cannot satisfy.

- **NPU `$Ctx` skeleton introduced.** A canonical 29-property `$Ctx` PSCustomObject (mirroring Chipset's schema) plus ≈ 20 NPU-specific extensions (`AmdAccountUser`, `NpuDriverPackage`, `WdacBinPath`, etc.) is now constructed at the head of NPU's `Invoke-MainEntryPoint`. The `$Ctx` is **intentionally unconsumed in this release** — stage 2 migrates phase functions to read from it; stage 3 migrates the Tier B-4 helpers. Until stage 3 lands, `$Ctx` and `$Script:` carry the same state in parallel.

- **SPEC.md §A.11.7 *Tier B-4* gains a per-stage progress table.** Stage 1 is marked ✅ Complete; stages 2 (phase function migration, ≈ 2 000 lines) and 3 (Tier B-4 helper Chipset-canon-isation, including `Invoke-Cleanup` direction = Chipset marker-file canon) are identified as planned with function-level scope.

- **`$Script:ScriptTag` updated to `npu-state-model-refactor-step-1-wdac-helpers`** across all four scripts. **All four scripts pass `psa.py 4.1.0` with 0 / 0 / 0 / 0 under the (now slightly tighter) canonical `.psa.config.json`.**

### Previous release notes

The **`2026-05-26` Chipset r82 / Graphics r48 / BthPan r30 / NPU r26** release (`psa-py-v410-shared-helper-canon-uplift`) advanced the shared helper canon workflow introduced in r81: three Tier B functions were reconciled to the Chipset canon, raising the PSA8001-enforced Tier A roster from 34 to 36 functions. The release also back-ported the BthPan `Invoke-InfVerifValidation`-era `$reasons.ToArray()` defensive form to all four scripts uniformly, eliminating a latent PS 5.1 ja-JP `[pscustomobject]` cast bug that had been guarded only in BthPan. SPEC.md §A.11.7 was reorganised into four Tier B sub-categories (B-1 / B-2 / B-3 / B-4) including the dedicated **NPU state-model refactor backlog** (Tier B-4) that the current r83 release begins to execute. The new SPEC.md §D.35 documents the full PS 5.1 ja-JP `[pscustomobject]@{ ... = @(List<T>) }` ArgumentException post-mortem.
...
Legacy server hosts (WS2019 / WS2016) no longer delegate I02 to an
external orchestrator (the prior Path C / WDAC SPF orchestrator was
removed in r70 after field validation demonstrated it could leave the
host unable to boot — see SPEC §D.30 for the full evidence chain).
On these hosts, I02 now uses Path A (trust-store-only, Secure Boot
may remain ON) when the install plan consists entirely of WHQL
co-signed drivers, or Path B (`bcdedit /set TESTSIGNING ON` plus a
reboot, **requires Secure Boot Disabled in firmware first** — see
SPEC §D.30.4 / F9) when any non-WHQL driver is present.
...
> **What about running everything in one pass?** Conceptually, all three Install actions converge on the same end state — patched INFs in the driver store plus a self-signed certificate trusted by the kernel. In practice, doing it as one uninterrupted sequence has a strictly worse failure mode: each Install can introduce a regression that only becomes visible at the next boot, and the per-script post-install verification (I04 / V06) cannot fully predict boot-time behaviour because it runs in the live OS, before the boot loader re-evaluates kernel CI against the newly-bound driver set. Running one script at a time with a reboot between bounds the blast radius of any single regression to "the most recently installed family", which is the difference between "roll back one driver via WinRE" and "reinstall the OS".
