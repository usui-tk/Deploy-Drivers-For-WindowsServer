...
**Latest release: `2026-05-27` — Chipset r86 / Graphics r52 / BthPan r34 / NPU r30** (`cross-repo-shared-utility-canon-write-caution`).
This release establishes the **5-way cross-repo shared utility canon** that spans the four AMD-family scripts in this repository and the `Download-SpeakerDeck.ps1` script in the sibling repository [`usui-tk/ai-generated-artifacts`](https://github.com/usui-tk/ai-generated-artifacts/tree/main/scripts/powershell/download-speakerdeck-oracle4engineer). 28 helper functions are now byte-identical across all five scripts (up from 5 prior to the release). The bidirectional best-of-both pass also renamed `Write-Warn2` (AMD canon) and `Write-Warn` (SpeakerDeck canon) to a unified `Write-Caution` and resolved the three remaining NPU-vs-AMD-trio divergences (`Set-Tls12`, `Set-ConsoleUtf8`, `Assert-PowerShellCompatibility`). **No runtime behaviour changes**.

Key changes (see CHANGELOG for the full entry):

- **5-way canon (AMD 4 siblings + SpeakerDeck) for 28 helper functions.** Identity / DebugTrace primitives, logging primitives (`Write-Ok` / `Write-Fail` / `Write-Skip` / `Write-Step` / `Write-Detail` / `Write-Caution`), phase banners (`Write-PhaseHeader` / `Write-PhaseFooter`), environment-prep helpers (`Set-Tls12` / `Set-ConsoleUtf8` / `Assert-PowerShellCompatibility` / `Show-PowerShellEnvironment`), and the full DebugTrace framework. Any future change to these helpers MUST be evaluated for cross-repo propagation. See SPEC §A.11.7 for the canon list and per-repo carve-outs.

- **`Write-Warn2` (AMD) / `Write-Warn` (SpeakerDeck) → `Write-Caution`.** Both names were ad-hoc workarounds for the visual collision with the PowerShell built-in `Write-Warning` cmdlet. The unified `Write-Caution` is collision-free, matches the noun-form convention of the other one-line helpers, and semantically fits the `[!]` marker ("operator must take notice, processing continues"). 426 callsites rewritten across the four AMD scripts (Chipset 109, Graphics 103, NPU 119, BthPan 95). The SpeakerDeck-side rewrite (47 callsites) is in the matching release of the sibling repo.

- **NPU drift resolution.** `Set-Tls12`, `Set-ConsoleUtf8`, and `Assert-PowerShellCompatibility` were the three remaining 4-way Tier A blockers because the NPU script had slightly different bodies. All three are now byte-identical with the other three siblings.

- **`Set-Tls12` hybrid canon.** New body negotiates TLS 1.3 + 1.2 (modern hosts) and falls back through TLS 1.1 + 1.0 via try/catch (very-old hosts). Combines the AMD canon's "prefer modern TLS" posture with the SpeakerDeck canon's "tolerate legacy hosts" posture. Each `[Net.SecurityProtocolType]` enum lookup is wrapped in try/catch so older .NET runtimes that lack `Tls13` don't fail.

- **Best-of-both selection per function.** 10 functions were taken from the SpeakerDeck canon (zero-padded `Format-Elapsed`, richer `Assert-PowerShellCompatibility` docstring, `Format-DebugFailure` with new `ElapsedMs` + `PhaseId` fields, etc.); 9 from the AMD canon (richer `Show-PowerShellEnvironment`, AMD-canon DebugTrace framework, etc.); 2 are hybrid merges (`Set-Tls12`, `Write-PhaseFooter`). The full per-function source map is in CHANGELOG.

- **`$Script:ScriptTag` updated to `cross-repo-shared-utility-canon-write-caution`** across all four scripts. **All four scripts pass `psa.py` 4.1.0 with 0 / 0 / 0 / 0**.
