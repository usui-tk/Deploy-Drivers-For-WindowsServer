...
**Latest release: `2026-05-26` — Chipset r82 / Graphics r48 / BthPan r30 / NPU r26** (`psa-py-v410-shared-helper-canon-uplift`).
This release advances the shared helper canon workflow introduced in r81: three Tier B functions are reconciled to the Chipset canon, raising the PSA8001-enforced Tier A roster from 34 to **36 functions**. The release also back-ports the BthPan `Invoke-InfVerifValidation`-era `$reasons.ToArray()` defensive form to all four scripts uniformly, eliminating a latent PS 5.1 ja-JP `[pscustomobject]` cast bug that had been guarded only in BthPan. **No runtime behaviour changes on en-US hosts or PowerShell 7.x**; the change is a defensive uniformity uplift.

Key changes (see CHANGELOG for the full entry):

- **Tier A roster: 34 → 36 functions.** `Get-SecureBootBaselineSnapshot` and `Show-SecureBootBaselineSnapshot` are promoted from Tier B to Tier A (PSA8001-enforced byte-identity across all four scripts). The promotion required cleaning up cosmetic divergences (NPU's `Write-Host`+`4-space-indent` literal lines were replaced with the canonical `Write-Detail` helper calls) and applying the PS 5.1 ja-JP `[pscustomobject]` latent-bug guard uniformly.

- **PS 5.1 ja-JP latent bug guarded uniformly.** All four scripts' `Get-SecureBootBaselineSnapshot` now uses `Reasons = $reasons.ToArray()` instead of `Reasons = @($reasons)`, matching BthPan's existing defensive form. The unified 8-line comment block references the new SPEC §D.35 post-mortem so future maintainers see the rationale immediately. On en-US hosts and PS 7.x the change is a no-op; on PS 5.1 ja-JP it eliminates a latent ArgumentException class.

- **SPEC.md §A.11.7 reorganisation.** The Tier B classification now uses four sub-categories that classify by *kind of divergence* rather than function family: B-1 (cosmetic, empty after r82), B-2 (PS 5.1 ja-JP latent-bug guard, empty after r82), B-3 (per-family identifier substitution, effectively Tier C — `Resume-CtxFromWorkspace`, `Invoke-Cleanup`), and **B-4 (NPU state-model architectural divergence — dedicated future workstream)**. Five remaining NPU divergences require a multi-thousand-line `$Script:` → `$Ctx` restructuring and are tracked as a separate future refactor PR.

- **SPEC.md §D.35 (new) — `PS 5.1 ja-JP [pscustomobject]@{ ... = @(List<T>) } ArgumentException`.** Full post-mortem of the original `Invoke-InfVerifValidation` defect localisation, the latent risk that motivated the uniform fix, and the general coding rule: when emitting a `[pscustomobject]` whose value is a `Generic.List[T]`, use `$list.ToArray()`, not `@($list)`.

- **`$Script:ScriptTag` updated to `psa-py-v410-shared-helper-canon-uplift`** across all four scripts. **All four scripts pass `psa.py 4.1.0` with 0 / 0 / 0 / 0 under the (now tighter) canonical `.psa.config.json`.**

### Previous release notes

The **`2026-05-26` Chipset r81 / Graphics r47 / BthPan r29 / NPU r25** release (`psa-py-v410-three-new-error-rules-baseline`) adopted `psa.py` 4.1.0 — the upstream minor release that adds three new error-severity, default-on static-analysis rules (`PSA1004`, `PSA2012`, `PSA2013`). All four sister scripts passed with a 0 / 0 / 0 / 0 baseline on the full latest-mainline rule set, including the three new error rules and the strict-mode `PSAP0005` inherited from r80. The release also introduced the **SPEC §A.11.7 "Shared helper canon and porting checklist"** that documents the canonical "copy from Chipset" workflow for shared helpers across the four sister scripts (Tier A — PSA8001-enforced byte-identity; Tier B — drift-tolerant backlog; Tier C — driver-family-specific; Tier D — intentionally per-script).
