...
**Latest release: `2026-05-26` — Chipset r84 / Graphics r50 / BthPan r32 / NPU r28** (`npu-state-model-refactor-step-2-phase-functions-ctx`).
This release is **stage 2 of the NPU state-model refactor** — the bulk of the restructuring work. Stage 1 ported the required WDAC tooling helpers from Chipset and constructed the canonical `$Ctx` skeleton inside NPU's `Invoke-MainEntryPoint`. Stage 2 makes that `$Ctx` **live**: every NPU phase function and every non-phase state-touching helper (except the three Tier B-4 helpers slated for stage 3) now takes `$Ctx` as a mandatory parameter and reads its runtime state from `$Ctx.Foo` instead of `$Script:Foo`. **No runtime behaviour changes** — the migration is strictly mechanical; every phase still does exactly what it did at r83.

Key changes (see CHANGELOG for the full entry):

- **184 mechanical `$Script:Foo` → `$Ctx.Foo` replacements across 27 functions.** 21 phase functions (`Invoke-PrepPhase00`〜`09`, `Invoke-VerifyPhase01`〜`06`, `Invoke-InstPhase00`〜`04`), 4 non-phase state-touching helpers (`Resolve-AmdNpuDriverUrl`, `Invoke-AmdAccountAuthentication`, `Show-RyzenAiSoftwareGuidance`, `Show-RunSummary`), `Invoke-MainEntryPoint`, and the phase dispatcher `Invoke-PhaseRunner`. The 3 Tier B-4 helpers (`Get-OrEnsureSecureBootBaseline`, `Resume-CtxFromWorkspace`, `Invoke-Cleanup`) are **deliberately untouched** for stage 3.

- **27 signature changes** (`param()` → `param([Parameter(Mandatory)] $Ctx)` for the 23 that had no existing param block; `$Ctx` inserted as the first argument for the 4 that did — `Show-RunSummary`, `Invoke-AmdAccountAuthentication`, `Resolve-AmdNpuDriverUrl`, `Invoke-PhaseRunner`). The dispatcher's `& $phase.Func` call site now passes `-Ctx $Ctx`.

- **Top-level dual-carriage state remains intentional.** The top-level `$Script:Foo = $Foo` assignments at NPU L307..L353 are kept because the three remaining Tier B-4 helpers still read `$Script:` globals; the `$Ctx` constructor reads from those `$Script:Foo` so both carriages hold the same values. The duplication is the price of keeping each stage's `psa.py 0/0/0` baseline; stage 3 removes it in a single coordinated change.

- **SPEC.md §A.11.7 *Tier B-4* progress table updated.** Stage 2 is now ✅ Complete with the per-category breakdown documented; stage 3 (Tier B-4 helper Chipset-canon-isation, including `Invoke-Cleanup` direction = Chipset marker-file canon) is the remaining planned work.

- **`$Script:ScriptTag` updated to `npu-state-model-refactor-step-2-phase-functions-ctx`** across all four scripts. **All four scripts pass `psa.py 4.1.0` with 0 / 0 / 0 / 0**; Tier A roster (PSA8001-enforced) remains at 38 functions (no Tier A change in this release).
