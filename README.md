...
Secure Boot enabled. Remediation state as of the latest release: **P0
(retraction, terminology, fail-closed base-policy gate) and the P1 waves
W1-W5 have landed** — Windows Driver Policy + kernel-image trust evidence,
non-boolean trust classification, OS-separated policy activation with
Mode T as an explicit opt-in, fail-closed download verification with
per-run PFX hygiene (chipset/graphics/BthPan), and the ProjectPreference /
measured-PnP-rank split. **Still open**: the P1 closeout items (NPU PFX
hygiene, kernel-trust evidence completion, download-override separation,
base-policy machine verification) and all of P2. No post-remediation field
validation has been run: historical validation
...
1. **P1 audit remediation closeout** — the core P1 waves have landed
   (OS-separated policy activation, kernel-image trust evidence, Windows
   Driver Policy evidence, non-boolean trust classification, fail-closed
   download verification, PFX key protection on chipset/graphics/BthPan);
   the closeout items (NPU PFX hygiene, trust-evidence completion,
   download-override separation, base-policy machine verification) remain.
...
| `-PfxPassword`             | per-script           | Password for the self-signed PFX. Chipset/graphics/BthPan: **per-run random 32-char CSPRNG when omitted** (W4); NPU: `''` with a known-literal export branch — **open item, audit H-05R (W7)** |
...
- **Storage**: PFX in `C:\Temp\Workspace_AMD-{Chipset,Graphics,NPU}\cert\` or `C:\Temp\Workspace_Microsoft-BthPan\cert\`. On chipset / graphics / BthPan the PFX password is a **per-run random 32-char CSPRNG value when `-PfxPassword` is omitted** (W4); the exported PFX is ACL-restricted to Administrators+SYSTEM and **deleted after Install** (the certificate stays in the store; P07 regenerates on demand). The NPU script still ships an empty-string default with a known-literal export branch — open item, audit H-05R (planned for W7). An earlier revision of this document claimed the PFX was not password-protected, which contradicted the code and is corrected here (audit L-01).
