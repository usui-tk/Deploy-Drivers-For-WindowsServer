...
| Windows Server 2019 | 17763 | **Path A** (trust-store only, if all drivers are WHQL co-signed) / **Path B** (`-UseTestSigning`, requires Secure Boot Disabled in firmware) | r70 deprecates the previous WDAC SPF orchestrator path; see SPEC §D.30. WHQL co-signed drivers (e.g. `AmdMicroPEP.sys`, `amdgpio2.sys`) load via trust-store import alone; non-WHQL drivers (e.g. `amdi2c.sys`, `amdsfhkmdf.sys`) require Path B. r71 (planned) will add `-SkipNonCosignedDrivers` to enable a trust-store-only install on hosts that prefer to keep Secure Boot ON. |
...
| `-ForceUnsafe`             | (off)                | **r69+ (Chipset/Graphics/BthPan only).** Bypass the CRITICAL acknowledgement checklist that I00 PreInstallReview prompts the operator with when conditions C1/C2/C5 fire (display driver replacement on single-display host; BitLocker ON + AMD PSP driver replacement; host hasn't been rebooted in 24+ hours). Intended for CI/CD automation only; the bypass is logged via `Set-DebugStep` in the run transcript. **Do NOT use in production.** See SPEC §D.28 |
