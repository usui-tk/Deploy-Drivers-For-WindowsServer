...
W1-W8 have landed** — Windows Driver Policy + kernel-image trust evidence,
...
per-run PFX hygiene (all four sisters since W7), the ProjectPreference /
measured-PnP-rank split, the security-narrative closure, and the
download-override separation with SourceArtifact evidence (W8). **Still
open**: the P1 closeout items (kernel-trust evidence completion,
...
1. **P1 audit remediation closeout** — the core P1 waves have landed
   ...
   the closeout items (trust-evidence completion, base-policy machine
   verification) remain.
