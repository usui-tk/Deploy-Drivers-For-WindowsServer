...
**Latest release: `2026-05-26` — Chipset r85 / Graphics r51 / BthPan r33 / NPU r29** (`npu-state-model-refactor-step-3-tier-b4-helper-canon`).
This release is **stage 3 (the final stage) of the NPU state-model refactor**. Stage 1 ported the prerequisite WDAC tooling helpers from Chipset and constructed the canonical `$Ctx` skeleton inside NPU's `Invoke-MainEntryPoint`. Stage 2 made that `$Ctx` live across every NPU phase function and helper. Stage 3 finishes the workstream: the five Tier B-4 helpers (`Get-OrEnsureSecureBootBaseline`, `Get-BootSigningEnvironment`, `Show-BootSigningEnvironment`, `Invoke-Cleanup`, `Resume-CtxFromWorkspace`) are replaced with byte-identical copies of the Chipset canon, the NPU `$Ctx` schema is fully reconciled with Chipset's, and every top-level `$Script:` state assignment is removed (only identity / logging / instrumentation / `WdacPolicyGuid` infra remains at script scope). **No runtime behaviour changes for AMD chipset / Graphics / BthPan operators**; the NPU script's cleanup semantics changed from cert-subject-CN enumeration to marker-file lookup (see SPEC §D.36.1 for why).

Key changes (see CHANGELOG for the full entry):

- **Tier A grew from 38 to 39 functions.** `Get-OrEnsureSecureBootBaseline` was removed from `psa8001_ignore_functions`; PSA8001 now actively enforces its 4-way byte-identity invariant. The four remaining Tier B-4 helpers establish an AMD-trio 3-way byte-identity invariant (Chipset / Graphics / NPU) but do not reach 4-way Tier A due to structural blockers (one-line Chipset/Graphics comment drift, BthPan implementation differences, per-driver-family cert filenames); these are documented in SPEC §A.11.7 future-work.

- **`Invoke-MainEntryPoint` regression fix.** r84's mechanical replacement had erroneously rewritten the function signature to `param([Parameter(Mandatory)] $Ctx)`, but the caller invokes it with no arguments. This would have failed at runtime with a parameter-binding error on the first script execution. r85 restores the correct `param()` signature and rewrites the body to read script-level parameters directly. `psa.py` does not currently catch this regression class (PSA2001 doesn't model "function is called with no arguments but declares a mandatory parameter").

- **NPU `Invoke-Cleanup` adopts the marker-file canon.** NPU's pre-r85 body enumerated `Cert:\LocalMachine\Root\` and matched on the cert subject CN; r85 reads `PolicyId` from `$Ctx.Paths.Cert\AmdSuppPolicyId.txt` and calls `Uninstall-AmdWdacPolicy`. The marker-file pattern eliminates two failure modes (multiple-deployment ambiguity, subject-CN drift between releases); SPEC §D.36.1 documents the post-mortem.

- **NPU `$Ctx` schema reconciled with the Chipset canon.** `$Ctx.Paths` is now a populated nested `[pscustomobject]` (`Paths.Cert`, `Paths.Patched`, `Paths.Markers`, `Paths.Logs`, etc.). `$Ctx.CertPfxPath` / `$Ctx.CertCerPath` / `$Ctx.CertThumbprint` are now populated. The NPU-only `$Script:DetectedPlatform` hashtable was folded under `$Ctx.DetectedPlatform`.

- **r84 "Tier A 41-way" overstatement corrected (forward-looking).** The r84 SPEC §A.11.7 stage-2 row and CHANGELOG entry described the planned stage-3 outcome as "PSA8001 then enforces 41-way Tier A" and "38 → 43". The actual outcome is **38 → 39**. Per repository policy the r84 historical text is preserved verbatim; SPEC §D.36.3 carries the forward-looking correction with the per-helper structural-blocker analysis.

- **`$Script:ScriptTag` updated to `npu-state-model-refactor-step-3-tier-b4-helper-canon`** across all four scripts. **All four scripts pass `psa.py` 4.1.0 with 0 / 0 / 0 / 0**.
