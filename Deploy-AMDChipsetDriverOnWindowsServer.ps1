...
param(
    ...
    # These two switches take effect ONLY when this script runs on
    # Windows Server 2019 (build 17763) or Windows Server 2016 (build
    # 14393), where the default I02 path (Path C) delegates to the
    # external orchestrator script
    # Deploy-WdacSinglePolicyFormatOnLegacyWindowsServer.ps1 which
    # builds and deploys a WDAC Single Policy Format (SPF) policy at
    # %WINDIR%\System32\CodeIntegrity\SiPolicy.p7b. They are no-ops on
    # WS2022+ where the MPF supplemental-policy path (Path A) is used,
    # and no-ops when -UseTestSigning is in effect (Path B).
    #
    # -ForceOverrideForeign:
    #   Required when the legacy host already has a foreign WDAC SPF
    #   policy deployed (state=Foreign). With this switch, the existing
    #   policy is BACKED UP to %ProgramData%\Deploy-Drivers-For-WindowsServer\
    #   wdac\backups\<ISO-timestamp>-foreign-policy.p7b.bak before being
    #   replaced. Restore via:
    #     .\Deploy-WdacSinglePolicyFormatOnLegacyWindowsServer.ps1 `
    #         -Action Uninstall -RestoreForeignBackup
    #   Without this switch, I02 ABORTS with a 3-option guidance
    #   message when a foreign policy is detected. See SPEC D.25 P3.
    [switch]$ForceOverrideForeign,

    # -AuditMode:
    #   Deploy the SPF policy with WDAC Rule Option 3 (Enabled:Audit
    #   Mode) instead of Enforced. Violations are logged to the Code
    #   Integrity event log but kernel-mode drivers are NOT blocked.
    #   Useful for pre-production verification on a legacy host before
    #   committing to enforcement. Re-running I02 without -AuditMode
    #   on a host where audit mode was previously set will transition
    #   the policy back to Enforced.
    [switch]$AuditMode,
...
$Script:ScriptVersion = 'chipset-2026.05.22-r67'
...
function Invoke-InstPhase02_AuthorizeDriverSigning {
    ...
    # WS2019 (build 17763) and WS2016 (build 14393) lack the Multiple
    # Policy Format (MPF) infrastructure that WS2022+ uses. CiTool.exe
    # is not present on these OSes, and the supplemental-policy file
    # layout in %WINDIR%\System32\CodeIntegrity\CiPolicies\Active\ is
    # treated as legacy-only by the kernel-mode loader. WDAC on legacy
    # Windows Server uses the Single Policy Format (SPF) which deploys
    # to %WINDIR%\System32\CodeIntegrity\SiPolicy.p7b.
    #
    # For these legacy hosts, we delegate to the external orchestrator
    # script Deploy-WdacSinglePolicyFormatOnLegacyWindowsServer.ps1
    # which handles SPF policy generation, deployment via WMI
    # (PS_UpdateAndCompareCIPolicy.Update() - no reboot required),
    # manifest tracking, and state-aware reconciliation.
    #
    # See SPEC D.25 for the full design rationale.
    ...
    if ($isLegacyWsHost -and -not $Ctx.UseTestSigning) {
        ...
        # ---- Locate the .cer file that P07 produced ----
        ...
        # Read the cert to extract the thumbprint we want to authorize
        ...
        # ---- Invoke the external WDAC SPF orchestrator ----
        ...
        if (-not $spfResult.NoOp) {
            Write-Ok 'Legacy WDAC SPF policy is active. No reboot required (WMI activation via PS_UpdateAndCompareCIPolicy.Update()).'
        ...
        $reverseInstr = @(
            ...
            '  (When the last cert is removed, the SPF policy is fully uninstalled.)'
        ) -join "`n"
        $Ctx | Add-Member -NotePropertyName I02ReverseInstructions -NotePropertyValue $reverseInstr -Force

        # SPF + WMI activation is immediate; no reboot required.
        Set-PhaseMarker -Ctx $Ctx -PhaseId 'I02'
    ...
    if ($isLegacyWsHost -and $Ctx.UseTestSigning) {
        ...
        Write-Warn2 ('  Consider dropping -UseTestSigning to use the recommended WDAC SPF path (Path C) which keeps Secure Boot ON.')
        Write-Host ''
    }
