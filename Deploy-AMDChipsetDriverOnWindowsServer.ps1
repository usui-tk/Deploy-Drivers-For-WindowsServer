...
.PARAMETER LogFile
    ...
    transcript is created AUTOMATICALLY under <WorkRoot>\logs as
    <ScriptName>_<Action>_<yyyyMMdd-HHmmss>_<PID>.log, so every run
    leaves a log without operator opt-in (an in-WorkRoot transcript
    survives -CleanWorkRoot via a suspend/wipe/resume flow). Whether
    auto-generated or operator-specified, the script wraps its
    execution in
...
param(
    ...
    [string]$AmdFallbackUrl  = 'https://drivers.amd.com/drivers/amd_software_8.07.16.1035.exe',
    ...
    # -WdacBasePolicyGuid:
    #   The SupplementsBasePolicyID written into the WDAC supplemental
    #   policy. NO DEFAULT: without this parameter the supplemental
    #   path (Path A policy deployment) is refused. Supply the GUID of
    #   a base policy you have verified is deployed on this host with
    #   rule option 17 (Enabled:Allow Supplemental Policies). The
    #   Windows-shipped base policy GUID is no longer assumed
    #   (third-party audit finding C-02; SPEC D.58.8).
...
#####################################################################
# Automatic transcript path generation (-LogFile omitted)
#####################################################################
# When the operator does NOT pass -LogFile, the run transcript is now
# captured automatically (same policy as the central
# Update-WindowsServerIso.ps1 project: every run leaves a machine-
# readable log without operator opt-in). The auto-generated path is
#
#   <WorkRoot>\logs\<ScriptName>_<Action>_<yyyyMMdd-HHmmss>_<PID>.log
#
# i.e. the transcript lands next to the per-tool logs (msiexec /
# inf2cat / signtool / verify) that the phases already write under
# <WorkRoot>\logs. The transcript starts BEFORE the entry banner so
# the captured log contains the banner and the full P00 execution-
# environment report.
#
# Interaction with -CleanWorkRoot: the P01 wipe would collide with
# the open transcript handle. For the auto-generated (in-WorkRoot)
# transcript this is resolved by the suspend -> wipe -> resume flow
# (Suspend-RunTranscriptForWipe / Resume-RunTranscriptAfterWipe, see
# SECTION 0.26 below): the transcript is stopped and stashed to
# %TEMP% just before the wipe, moved back after the workspace
# directories are recreated, and reopened with -Append. An
# operator-specified -LogFile keeps the pre-existing relocation-
# guard behaviour (below) unchanged.
$Script:LogFileAutoGenerated = $false
if ([string]::IsNullOrWhiteSpace($LogFile)) {
    $autoLogStamp = (Get-Date).ToString('yyyyMMdd-HHmmss')
    $autoLogLeaf  = ('Deploy-AMDChipsetDriverOnWindowsServer_{0}_{1}_{2}.log' -f $Action, $autoLogStamp, $PID)
    $LogFile      = Join-Path (Join-Path $WorkRoot 'logs') $autoLogLeaf
    $Script:LogFileAutoGenerated = $true
    Write-Host ('[*] -LogFile omitted: transcript auto-path -> {0}' -f $LogFile) -ForegroundColor DarkCyan
}
...
# (Auto-generated transcripts are exempt from this guard: they
# intentionally live under <WorkRoot>\logs and survive the wipe via
# the suspend -> wipe -> resume flow in SECTION 0.26.)
$Script:LogFileRelocation = $null
if (-not [string]::IsNullOrWhiteSpace($LogFile) -and $CleanWorkRoot -and -not $Script:LogFileAutoGenerated) {
    try {
...
#####################################################################
# SECTION 0.26: Transcript suspend / resume across -CleanWorkRoot
#####################################################################
# Companion to the automatic transcript above. When the transcript
# file lives INSIDE -WorkRoot (the auto-generated default is
# <WorkRoot>\logs\...) and -CleanWorkRoot is requested, the P01 wipe
# would hit the open transcript handle. Instead of relocating the
# file out of the workspace (the operator-specified -LogFile guard
# behaviour), the P01 wipe brackets itself with these two helpers:
#
#   Suspend-RunTranscriptForWipe  - Stop-Transcript, then MOVE the
#       partial transcript to %TEMP% so the recursive delete sees no
#       open handle and no leftover file.
#   Resume-RunTranscriptAfterWipe - after the workspace directories
#       are recreated, move the partial transcript back to its
#       original path and re-open it with Start-Transcript -Append,
#       so one continuous log file covers the whole run.
#
# Both helpers are strict no-ops when the transcript is inactive or
# its path is outside the WorkRoot subtree. Failures are absorbed
# with a warning: transcript handling must never prevent the
# pipeline from doing its actual work (same principle as the
# activation cascade above).
$Script:TranscriptWipeStash = $null
function Suspend-RunTranscriptForWipe {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)] [string]$WorkRoot
    )
    $Script:TranscriptWipeStash = $null
    if (-not $Script:LogFileActive -or -not $Script:LogFileSetup) { return }
    try {
        $resolvedLog  = [System.IO.Path]::GetFullPath($Script:LogFileSetup.Path)
        $resolvedRoot = [System.IO.Path]::GetFullPath($WorkRoot)
        if (-not $resolvedRoot.EndsWith([System.IO.Path]::DirectorySeparatorChar) -and `
        ...
        if (-not $resolvedLog.StartsWith($resolvedRoot, [System.StringComparison]::OrdinalIgnoreCase)) {
            return  # transcript lives outside the wipe target - nothing to do
        }

        Write-Step 'Suspending transcript for -CleanWorkRoot wipe (will resume after)'
        try { Stop-Transcript -ErrorAction SilentlyContinue | Out-Null } catch { } # psa-disable-line PSA3004 -- intentional best-effort cleanup
        $Script:LogFileActive = $false

        $stashLeaf = [System.IO.Path]::GetFileName($resolvedLog)
        $stashPath = Join-Path ([System.IO.Path]::GetTempPath()) ('{0}.wipe-stash-{1}' -f $stashLeaf, $PID)
        if (Test-Path -LiteralPath $resolvedLog) {
            Move-Item -LiteralPath $resolvedLog -Destination $stashPath -Force
            $Script:TranscriptWipeStash = [pscustomobject]@{
                OriginalPath = $resolvedLog
                StashPath    = $stashPath
            }
            Write-Detail ('Transcript stashed -> {0}' -f $stashPath) -Color DarkGray
        }
    } catch {
        Write-Warning ('[Transcript] suspend-for-wipe failed (non-fatal): {0}' -f $_.Exception.Message)
    }
}

function Resume-RunTranscriptAfterWipe {
    [CmdletBinding()]
    param()
    if (-not $Script:TranscriptWipeStash) { return }
    $stash = $Script:TranscriptWipeStash
    $Script:TranscriptWipeStash = $null
    try {
        $logDir = [System.IO.Path]::GetDirectoryName($stash.OriginalPath)
        if (-not [string]::IsNullOrWhiteSpace($logDir) -and -not (Test-Path -LiteralPath $logDir)) {
            New-Item -ItemType Directory -Path $logDir -Force | Out-Null
        }
        if (Test-Path -LiteralPath $stash.StashPath) {
            Move-Item -LiteralPath $stash.StashPath -Destination $stash.OriginalPath -Force
        }
        # Re-open with -Append so the pre-wipe and post-wipe halves form
        # one continuous transcript. The -LiteralPath variant is kept as
        # a fallback for the PS 5.1 parameter-binding quirk documented
        # in the activation cascade above.
        $resumed = $false
        try {
            Start-Transcript -Path $stash.OriginalPath -Append -Force -ErrorAction Stop | Out-Null # psa-disable-line PSA3005 -- -Path first with -LiteralPath fallback below (see activation-cascade rationale)
            $resumed = $true
        } catch {
            try {
                Start-Transcript -LiteralPath $stash.OriginalPath -Append -Force -ErrorAction Stop | Out-Null
                $resumed = $true
            } catch {
                Write-Warning ('[Transcript] resume-after-wipe failed: {0}' -f $_.Exception.Message)
            }
        }
        if ($resumed) {
            $Script:LogFileActive = $true
            Write-Ok ('Transcript resumed -> {0}' -f $stash.OriginalPath)
        } else {
            Write-Warning '   Continuing without log capture from this point on.'
        }
    } catch {
        Write-Warning ('[Transcript] resume-after-wipe failed (non-fatal): {0}' -f $_.Exception.Message)
    }
}
...
function New-AmdDriverWdacSupplementalPolicy {
    ...
    if ([string]::IsNullOrWhiteSpace($BasePolicyId)) {
        throw ('New-AmdDriverWdacSupplementalPolicy: BasePolicyId is empty. A supplemental policy requires ' +
...
function Save-WhqlCoSignPlanJson {
    # Persist the post-trim -SkipNonCosignedDrivers plan summary
    # produced by the P06 trim, so the install phases (I01 skip gate,
    # I02 short-circuit) can read it across the PrepareVerify ->
    # Install process boundary via Get-WhqlCoSignPlanInfo
    # (SPEC §D.31.17 / §D.40 / §D.41).
    #
    # Scope rule (SPEC §D.41): only records selected for this host's
    # install scope (VariantSelected) participate in the name sets and
    # counts. Out-of-scope variant rows (e.g. W11x64 rows on a host
    # that installs WTx64) otherwise collide by INF name with trimmed
    # in-scope rows and poison the counts - the 2026-08-08 field
    # failure. Records without a VariantSelected property are included
    # conservatively.
    #
    # PlanCatalogSignCount counts the in-scope eligible records that
    # will receive a pipeline-regenerated, self-signed catalog at
    # P08/P09 (catalog-eligible and no missing referenced files). It
    # is the I01 gate criterion: the certificate must be trusted
    # whenever at least one such catalog exists, regardless of whether
    # any INF text needed patching.
    #
    # Best-effort: a write failure never fails P06; the install phases
    # then fall back to conservative behaviour.
    [CmdletBinding()]
    [OutputType([void])]
    param(
        [Parameter(Mandatory)] $Ctx,
    ...
    try {
        if (-not $Ctx.Paths -or -not $Ctx.Paths.Root) { return }
        $scopedEligible = New-Object System.Collections.Generic.List[pscustomobject]
        foreach ($rec in $EligibleRecords) {
            $inScope = $true
            if ($rec.PSObject.Properties['VariantSelected']) {
                $inScope = ($rec.VariantSelected -eq $true -or $rec.VariantSelected -eq 'True')
            }
            if ($inScope) { $scopedEligible.Add($rec) }
        }
        $scopedPreTrim = New-Object System.Collections.Generic.List[pscustomobject]
        foreach ($rec in $PreTrimRecords) {
            $inScope = $true
            if ($rec.PSObject.Properties['VariantSelected']) {
                $inScope = ($rec.VariantSelected -eq $true -or $rec.VariantSelected -eq 'True')
            }
            if ($inScope) { $scopedPreTrim.Add($rec) }
        }
        $eligibleNames = New-Object 'System.Collections.Generic.HashSet[string]' ([System.StringComparer]::OrdinalIgnoreCase)
        foreach ($rec in $scopedEligible) {
            $n = $null
        ...
        foreach ($rec in $scopedEligible) {
            ...
            if ($rec.PSObject.Properties['EligibleForCatalog']) {
                $catalogEligible = ($rec.EligibleForCatalog -eq $true -or $rec.EligibleForCatalog -eq 'True')
            }
            $hasMissingRefs = $false
            if ($rec.PSObject.Properties['MissingReferencedFiles'] -and $rec.MissingReferencedFiles) {
                $hasMissingRefs = -not [string]::IsNullOrWhiteSpace([string]$rec.MissingReferencedFiles)
            }
            if ($catalogEligible -and -not $hasMissingRefs) { $catalogSignCount++ }
        }
        $planAnalysed = 0
        $planNonCoSigned = 0
...
function Get-WhqlCoSignPlanInfo {
    ...
    # §D.31.17 / §D.40 / §D.41). Source precedence:
    #   1. whql_cosign_plan.json     (post-trim plan persisted by P06;
    #      requires SchemaVersion >= 2 - older plan files lack the
    #      PlanCatalogSignCount field and are ignored so that a stale
    #      plan can never justify skipping I01)
    ...
    #   PlanNeedsSelfSigning - $true when the plan carries at least one
    #                          catalog this pipeline self-signs at
    #                          P08/P09 (I01 required). Only the v2 plan
    #                          JSON can prove the contrary; every other
    #                          source reports $true (conservative).
    #   PlanCatalogSignCount - self-signed catalog count (-1 unknown)
    #   NonCoSignedCount     - non-fully-co-signed analysed INFs in the
    #                          plan scope (-1 when unknown)
            ...
            if (Test-Path -LiteralPath $planPath) {
                ...
                if ($null -ne $plan -and $plan.SkipNonCosignedDrivers -and
                    ...
                    return [pscustomobject]@{
                        Known                = $true
                        Source               = 'plan-json'
                        PlanNeedsSelfSigning = ([int]$plan.PlanCatalogSignCount -gt 0)
    ...
    if ($null -ne $analyses) {
        ...
        # The analysis alone cannot prove the plan carries no
        # self-signed catalogs (catalogs are regenerated for the whole
        # selected scope, not just the analysed subset - SPEC §D.41):
        # report PlanNeedsSelfSigning=$true so I01 always runs on
        # these sources.
        return [pscustomobject]@{
            Known                = $true
            Source               = $source
...
#      pages link to. Filename conventions:
#          amd_software_<MAJOR>.<MINOR>.<BUILD>.<REV>.exe (2026-07+)
#          amd_chipset_software_<MAJOR>.<MINOR>.<BUILD>.<REV>.exe (legacy)
...
#               https://drivers\.amd\.com/drivers/amd(?:_chipset)?_software_(\d+\.\d+\.\d+\.\d+)\.exe
...
#
#     https://drivers.amd.com/drivers/amd_software_<VERSION>.exe (2026-07+)
#     https://drivers.amd.com/drivers/amd_chipset_software_<VERSION>.exe (pre-2026-07)
#
#   NAMING RENAME (observed 2026-07, confirmed 2026-08-07): AMD
#   dropped the "chipset_" infix from newly-published installer
#   filenames (first seen with 8.07.16.1035, released 2026-07-30).
#   Older releases remain downloadable under the legacy name. The
#   discovery regex and the download-cache filters therefore accept
#   BOTH forms; a probe run that matches NOTHING dumps the fetched
#   page bodies to <WorkRoot>\logs as evidence for the next rename.
#
#   Where VERSION = <MAJOR>.<MINOR>.<BUILD>.<REV>
#
#   Examples (verified live 2026-05; naming re-verified 2026-08):
#     amd_software_8.07.16.1035.exe (2026-07 release, NEW naming)
...
function Get-LatestAmdChipsetUrl {
    ...
    param(
        ...
        # Optional directory that receives the fetched page bodies as
        # evidence files when EVERY probe misses (see fallback branch).
        [string]$DiagnosticsDir
    )
    $browserUA = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0 Safari/537.36'
    # 2026-07: AMD renamed newly-published installers from
    # amd_chipset_software_<v>.exe to amd_software_<v>.exe (first seen
    # with 8.07.16.1035, released 2026-07-30). Accept BOTH namings so
    # legacy releases and cached pages keep matching. The optional
    # group cannot match the sibling amd_raid_software_*.zip download
    # published on the same landing pages.
    $pattern   = 'https://drivers\.amd\.com/drivers/amd(?:_chipset)?_software_(\d+\.\d+\.\d+\.\d+)\.exe'
    $perPage    = @()
    $allHits    = @()
    $pageBodies = [ordered]@{}
    ...
    foreach ($url in $LandingUrls) {
        ...
        try {
            ...
            $bodyLen = if ($resp.Content) { $resp.Content.Length } else { 0 }
            $pageBodies[$short] = $resp.Content
            $perPage += [pscustomobject]@{ Page=$short; Found=$hits.Count; Bytes=$bodyLen }
            foreach ($m in $hits) {
            ...
            $perPage += [pscustomobject]@{ Page=$short; Found=0; Bytes=0; Note=$_.Exception.Message }
            Write-Caution "  Page failed ($short): $($_.Exception.Message)"
        }
    }

    Write-Host '    Probe results:' -ForegroundColor DarkGray
    foreach ($s in $perPage) {
        Write-Detail ("  [{0,-50}] hits={1} bytes={2}" -f $s.Page, $s.Found, $s.Bytes) -Color DarkGray
    }
    ...
    Write-Caution 'No landing page yielded a parseable URL - falling back.'
    # Evidence preservation: when every probe misses, dump each fetched
    # page body to the diagnostics directory so the NEXT naming-
    # convention change on AMD's side can be diagnosed from the run
    # artifacts alone (this is how the 2026-07 amd_chipset_software_ ->
    # amd_software_ rename was confirmed).
    if (-not [string]::IsNullOrWhiteSpace($DiagnosticsDir)) {
        try {
            if (-not (Test-Path -LiteralPath $DiagnosticsDir)) {
                New-Item -ItemType Directory -Path $DiagnosticsDir -Force | Out-Null
            }
            $probeStamp = (Get-Date).ToString('yyyyMMdd-HHmmss')
            $probeIndex = 0
            foreach ($pageKey in $pageBodies.Keys) {
                $probeIndex++
                $safeKey  = (($pageKey -replace '\.html$', '') -replace '[^A-Za-z0-9\.\-]', '_')
                $dumpPath = Join-Path $DiagnosticsDir ('amd-landing-probe-{0}-{1:D2}_{2}.html' -f $probeStamp, $probeIndex, $safeKey)
                Set-Content -LiteralPath $dumpPath -Value $pageBodies[$pageKey] -Encoding UTF8
                Write-Detail ('  Probe evidence saved -> {0}' -f $dumpPath) -Color DarkGray
            }
        } catch {
            Write-Warning ('Probe-evidence dump failed (non-fatal): {0}' -f $_.Exception.Message)
        }
    }
...
function Expand-AmdInstaller {
    ...
    _ClearDest
    Write-Caution "    7-Zip auto-detect produced no usable payload (exit $exit1) - trying next strategy"
    ...
    try {
        ...
        Write-Caution "    InstallShield /a chain completed but produced no usable payload"
    } catch {
        Write-Caution "    InstallShield /a strategy failed: $($_.Exception.Message)"
    }
    _ClearDest
...
function Expand-AmdInstaller_ViaLaunch {
    ...
    if ($toDelete.Count -gt 0) {
        ...
        foreach ($p in $toDelete) {
            ...
            try {
                ...
                Write-Caution "      [ FAIL ] $($_.Exception.Message)"
            }
        }
    } else {
        Write-Host ''
    ...
    if (-not $newDir) {
        Write-Host ''
        Write-Caution "    Diagnostic dump of $amdRoot at timeout (launchStart=$($launchStart.ToString('HH:mm:ss'))):"
        if (Test-Path $amdRoot) {
    ...
    if (-not $verify.Success) {
        Write-Caution ("    robocopy reported exit={0} but post-copy verification FAILED:" -f $robocopyExit)
        Write-Caution ("      file counts (src/dst): {0}/{1}    dir counts (src/dst): {2}/{3}" `
            -f $verify.SrcFileCount, $verify.DstFileCount, $verify.SrcDirCount, $verify.DstDirCount)
        if ($verify.MissingFiles.Count -gt 0) {
            Write-Caution ("      Missing in destination ({0} file(s); showing first 10):" -f $verify.MissingFiles.Count)
            $verify.MissingFiles | Select-Object -First 10 | ForEach-Object {
                Write-Detail ("    - $_") -Color DarkYellow
            }
        }
        if ($verify.ExtraFiles.Count -gt 0) {
            Write-Caution ("      Unexpected in destination ({0} file(s); showing first 10):" -f $verify.ExtraFiles.Count)
...
function Expand-AmdInstaller_ViaInstallShield {
    ...
    if ($subFailDiag.Count -gt 0) {
        ...
        try {
            ...
            Write-Caution ("  Could not write sub-MSI diagnostic file: {0}" -f $_.Exception.Message)
        }
    }
    ...
    if ($allInfs.Count -gt 0) {
        ...
        if ($preferred.Count -gt 0) {
            foreach ($p in $preferred) {
                ...
                if ($count -eq 0) {
                    Write-Caution ("      No INFs in preferred variant '{0}' for host OS. Downstream P05/P06/I03 will have nothing to install." -f $p)
                }
            }
        }
    }
...
function Invoke-PrepPhase01_PrepareWorkspace {
    ...
    if ($Ctx.CleanWorkRoot -and (Test-Path $Ctx.WorkRoot)) {
        ...
        # An in-WorkRoot transcript (the -LogFile-omitted default) would
        # hold an open handle inside the tree being deleted. Suspend it
        # (stop + stash to %TEMP%); Resume-RunTranscriptAfterWipe below
        # moves it back and re-opens with -Append once the directories
        # have been recreated. See SECTION 0.26.
        Suspend-RunTranscriptForWipe -WorkRoot $Ctx.WorkRoot
    ...
    $paths = [pscustomobject]@{
        ...
        Manifests = Join-Path $Ctx.WorkRoot 'manifests'
        ShadowExtract = Join-Path $Ctx.WorkRoot 'shadow-extracted'
    }
    ...
    # Resume the transcript that Suspend-RunTranscriptForWipe stashed
    # (no-op when no wipe happened or the transcript lives elsewhere).
    Set-DebugStep 'resume transcript after -CleanWorkRoot wipe'
    Resume-RunTranscriptAfterWipe
...
function Invoke-PrepPhase03_FetchInstaller {
    ...
    if ($Ctx.InstallerUrl) {
        ...
        $info = Get-LatestAmdChipsetUrl -LandingUrls $Ctx.AmdLandingUrls -FallbackUrl $Ctx.AmdFallbackUrl -DiagnosticsDir $Ctx.Paths.Logs
        Write-Host ''
        Write-Detail "Version    : $($info.Version)"
        Write-Detail "Source     : $($info.Source)"
    ...
    if (Test-Path $path) {
        ...
        Write-Caution "Discarding undersized existing file ($([math]::Round($existing.Length/1MB,1)) MB)"
...
function Invoke-PrepPhase04_ExtractInstaller {
    ...
    foreach ($n in $nested) {
        ...
        if ($extractedCount -eq 0) {
            Write-Caution ("      7-Zip exit={0} but 0 files extracted from {1} - flagged for downstream INF-count check" -f $sevenZipExit, $n.Name)
        } else {
    ...
    # Wave W12 shadow: run the static extraction port alongside the
    # executable strategies above. Evidence only - the tree produced by
    # Expand-AmdInstaller stays authoritative for every downstream phase,
    # no deployment decision reads the shadow output, and any shadow
    # failure is fail-open by design.
    Set-DebugStep 'static extraction shadow (evidence only)'
    try {
        if (Test-Path $Ctx.Paths.ShadowExtract) {
            Get-ChildItem -LiteralPath $Ctx.Paths.ShadowExtract -Force -ErrorAction SilentlyContinue |
                Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
        }
        Write-Step 'Static extraction shadow (evidence only; current tree stays authoritative)'
        $shadowGraph = Invoke-AmdStaticExtractionShadow -InstallerPath $Ctx.Installer `
                                                        -DestinationRoot $Ctx.Paths.ShadowExtract `
                                                        -SevenZipPath $Ctx.SevenZip `
                                                        -MaxDepth 5
        $shadowGraph.ToolIdentity = [pscustomobject]@{
            ScriptVersion = $Script:ScriptVersion
            FragmentId    = 'amd-static-extraction v1'
        }
        $shadowGraph.ParityNote.CurrentTreeInfCount = $infCount
        $shadowGraphPath = Join-Path $Ctx.Paths.Manifests 'extraction-graph.json'
        Write-AmdStaticExtractionGraph -Graph $shadowGraph -Path $shadowGraphPath
        $shadowInfTotal = [int]$shadowGraph.ParityNote.ShadowInfBaseCount +
                          [int]$shadowGraph.ParityNote.ShadowInfVariantCount
        $shadowSummary = ('Shadow static extraction: status={0}; INF base={1} + variants={2} (total {3}) vs current tree INF={4}; graph: {5}' -f `
            $shadowGraph.Status, $shadowGraph.ParityNote.ShadowInfBaseCount,
            $shadowGraph.ParityNote.ShadowInfVariantCount, $shadowInfTotal, $infCount, $shadowGraphPath)
        if ($shadowGraph.Status -eq 'ExtractionComplete' -and $shadowInfTotal -ge $infCount) {
            Write-Ok $shadowSummary
        }
        else {
            Write-Caution $shadowSummary
        }
    }
    catch {
        $shadowError = $_.Exception.Message
        Write-Caution ('Static extraction shadow FAILED (fail-open; deployment is unaffected): {0}' -f $shadowError)
        try {
            $failGraph = [pscustomobject][ordered]@{
                SchemaVersion  = '1.0'
                GeneratedAtUtc = [DateTime]::UtcNow.ToString('o')
                ToolIdentity   = [pscustomobject]@{
                    ScriptVersion = $Script:ScriptVersion
                    FragmentId    = 'amd-static-extraction v1'
                }
                Status         = 'ShadowFailed'
            ...
            Write-AmdStaticExtractionGraph -Graph $failGraph -Path (Join-Path $Ctx.Paths.Manifests 'extraction-graph.json')
        }
        catch {
            Write-Verbose 'Shadow failure graph could not be written (ignored - fail-open).'
        }
    }
...
function Invoke-PrepPhase05_AnalyzeInfs { # psa-disable-line PSA6003 -- compound noun (e.g., Policies, Drivers, Catalogs) is semantically plural for set-returning helpers
    ...
    if ($ineligible.Count -gt 0) {
        Write-Host ''
        Write-Caution "INFs ineligible for catalog generation (phantom file references): $($ineligible.Count)"
    ...
    # WDF requirement vs this host (SPEC D.54). The inventory now carries what
    # each INF asks the framework for; this is where it meets what the host
    # provides. Recorded at script scope so the run summary digest can reach
    # it without depending on $Ctx, whose shape differs across the sisters.
    $wdfHostRuntime = Get-HostWdfRuntime -BuildNumber ([int][Environment]::OSVersion.Version.Build)
    $Script:WdfShortfall = Get-WdfShortfallSummary -InfRecord @($detailReport) `
...
function Invoke-PrepPhase06_PatchInfs { # psa-disable-line PSA6003 -- compound noun (e.g., Policies, Drivers, Catalogs) is semantically plural for set-returning helpers
    ...
    # side-effect didn't reach them. The 8.02.18.557 release has only 1
...
function Invoke-PrepPhase07_CreateCertificate {
    ...
    if ($preexisting.Count -gt 0) {
        Write-Caution ("Found {0} existing certificate(s) in LocalMachine\My with the same Subject - deleting before re-creation" -f $preexisting.Count)
        foreach ($oldCert in $preexisting) {
            ...
            try {
                ...
                Write-Caution ("    Failed to delete {0}: {1}" -f $oldCert.Thumbprint, $_.Exception.Message)
...
function Invoke-PrepPhase08_GenerateCatalogs { # psa-disable-line PSA6003 -- compound noun (e.g., Policies, Drivers, Catalogs) is semantically plural for set-returning helpers
    ...
    if ($infDirs.Count -eq 0) {
        ...
        Write-Host ''
        Write-Caution "P08: no INF-bearing directories found under the patched root."
    ...
    if ($inf2catVer) {
        ...
        Write-Caution "    inf2cat tool path    : $($Ctx.Inf2cat) (version metadata unavailable)"
    }
    ...
    if ($supportedByTool.Count -eq 0) {
        Write-Caution "    Could not parse inf2cat help output - will try preferred value blindly."
    }
        ...
        foreach ($s in $ineligibleDirs) {
            ...
            foreach ($cat in $orphans) {
                try {
                    ...
                    Write-Caution ("    [warn] could not delete orphan .cat '{0}': {1}" -f $cat.FullName, $_.Exception.Message)
                }
            }
        }
    ...
    foreach ($dir in $infDirsToProcess) {
        ...
        if ($succeeded) {
            ...
            Write-Detail "inf2cat: [$variant] $rel" -Color DarkGray
            Write-Caution "  FAILED (exit=$exitDisplay)"
    ...
    if ($failCount -gt 0 -and $failureSamples.Count -gt 0) {
        Write-Host ''
        Write-Caution "Sample failure details (first $($failureSamples.Count) of $failCount):"
        Write-Host ''
        $idx = 0
...
function Invoke-PrepPhase09_SignCatalogs { # psa-disable-line PSA6003 -- compound noun (e.g., Policies, Drivers, Catalogs) is semantically plural for set-returning helpers
    ...
    if ($cats.Count -eq 0) {
        Write-Caution 'P09: no eligible .cat files remain after phantom-file filter - nothing to sign.'
    ...
    if ($emptyCount -gt 0) {
        Write-Caution "    Pre-flight: $emptyCount of $($cats.Count) .cat file(s) are EMPTY (size=0 bytes)"
        Write-Caution "    These will fail signing and indicate inf2cat silently produced corrupt output."
    } else {
...
function Invoke-VerifyPhase05_DryRunInstall {
    ...
    $pfx = Join-Path $Ctx.Paths.Cert 'AMD-Chipset-Driver-CodeSign.pfx'
    if (-not (Test-Path $pfx)) {
        Write-Caution '  PFX missing - I01 would FAIL'
    } else {
        $cert = [System.Security.Cryptography.X509Certificates.X509Certificate2]::new($pfx, $Ctx.PfxPassword)
        Write-Host "  Subject     : $($cert.Subject)"
        Write-Host "  Thumbprint  : $($cert.Thumbprint)"
    ...
    if ($Ctx.UseTestSigning) {
        ...
        if ($deployed) {
            ...
            Write-Host '  AppControlDecision layer only; no kernel-load claim (SPEC D.58). No reboot where CiTool or RefreshPolicy.exe is available.' -ForegroundColor DarkGray
        }
    }
    Write-Host ''
    ...
    if ($infs.Count -eq 0) {
        Write-Caution '  No INFs to install - I03 would do nothing'
    } else {
    ...
    try {
        ...
        Write-Caution ("Secure Boot baseline display failed: {0}" -f $_.Exception.Message)
    }
...
function Invoke-VerifyPhase06_HardwareImpactAnalysis { # psa-disable-line PSA6003 -- compound noun (e.g., Policies, Drivers, Catalogs) is semantically plural for set-returning helpers
    ...
    # the [C] classification for script-installed drivers and reported
    # them as [B] vendor (because Win32_PnPSignedDriver.InfName returns
    # the OEM-numbered short form, not the original name, and Step 0a's
    # .cat thumbprint match cannot resolve the catalog path reliably
    # for all WS2016/2019/2022/2025 build variants). See SPEC §D.32.2
    # for the field evidence that motivated this change.
    ...
    if ($hwAll.Count -eq 0) {
        Write-Caution 'No AMD-affiliated PnP devices detected on this machine.'
    ...
    if ($infIndex.Keys.Count -eq 0) {
        Write-Caution '  No patched INFs to compare against (run P05/P06 first).'
        Write-Host ''
        Write-PhaseFooter 'V06' 'done'
        return
    }
...
function Invoke-InstPhase00_PreInstallReview {
    ...
    if (-not (Test-Path $Ctx.Paths.Patched)) {
        Write-Caution 'I00: patched/ directory missing; running on cached state. P05/P06 must have completed in a previous run.'
    }
    ...
    $hw    = @($hwAll | Where-Object IsAmdHardware)
    if ($hw.Count -eq 0) {
        Write-Caution 'No AMD HARDWARE detected. I03 will still register the drivers in the driver store.'
    }
    $infIndex = Build-PatchedInfHwidIndex -Ctx $Ctx
    ...
    foreach ($d in $hw) {
        ...
        if ($infs.Count -gt 0) {
            ...
            # re-runs after a successful install. See SPEC §D.32.2.
            $cat = if ($cur) {
                Get-DriverSourceCategory -Provider $cur.Provider -Signer $cur.Signer -InfName $cur.InfName -ExpectedSelfSignThumbprint $Ctx.CertThumbprint -KnownOurInfSet $ourInfSet
            } else {
                @{ Code='?'; ShortLabel='[?]'; Label='No driver bound'; Color='DarkGray' }
    ...
    if (Test-Path $pfx) {
        try {
            ...
            Write-Caution ('  Could not load PFX for preview: {0}' -f $_.Exception.Message)
        }
    } else {
        Write-Caution ('  PFX not found at {0} - I01 will fail.' -f $pfx)
    }
    Write-Host ''
...
function Invoke-InstPhase01_TrustCertificate {
    ...
    if (-not (Test-MutatingPhaseAdmissible -Ctx $Ctx -PhaseId 'I01')) {
    ...
    if ($Script:SkipNonCosignedDrivers) {
        ...
        if ($planInfo.Known -and -not $planInfo.PlanNeedsSelfSigning) {
            Write-Ok '  Trimmed install plan contains no catalogs for this pipeline to self-sign - certificate trust import is not required.'
...
function Invoke-InstPhase02_AuthorizeDriverSigning {
    ...
    if (-not $Ctx.UseTestSigning -and $Script:SkipNonCosignedDrivers) {
        ...
        # workspace (SPEC §D.31.17 / §D.40 / §D.41). The original
        # in-process condition ($Ctx.WhqlCoSignAnalysis) was always
        # $null in the split workflow, making this short-circuit
        # unreachable there. A plan whose analysed subset is empty
        # (every patch-needing INF was trimmed, or nothing needed
        # patching) also short-circuits: the remaining kernel content
        # is vendor-signed and needs no kernel-mode signer
        # authorization. Self-signed CATALOGS are a driver-store
        ...
        if ($planInfo.Known -and $planInfo.NonCoSignedCount -eq 0 -and -not $planFullyExamined) {
            ...
            # A refusal that carries on is not a refusal. This branch used to
            # print and fall through into path selection, so a plan the script
            # had just declared unfit to judge went on to be authorized: with
            # Secure Boot off it would have set the BCD testsigning flag and
            # asked for a reboot. Nothing about the plan stopped it - the only
            # thing that did, on the host where this surfaced, was an
            # unrelated Path B prerequisite check that happened to sit in
            # front. Stop here instead, and leave the machine untouched.
            Write-Detail  '  No signing authorization is applied and nothing on this system is changed.'
            Set-PhaseMarker -Ctx $Ctx -PhaseId 'I02' -Metadata @{ ShortCircuit = $true; Reason = 'plan-not-fully-examined'; AnalysedInfCount = $planInfo.AnalysedInfCount; UnverifiedCount = $planInfo.UnverifiedCount; PlanSource = $planInfo.Source }
            Write-PhaseFooter 'I02' 'skipped'
            return
        }
    ...
    if ($useWdac) {
        ...
        $deploy = Install-AmdWdacPolicy -XmlPath $xmlPath -BinaryOutPath $cipPath -ActivationPlan $activationPlan
    ...
    # CI would reject until the next boot. The result was a confusing
    # mix of "REBOOT_REQUIRED" disposition lines from pnputil and
    # I04 functional-health probes that could not run because no
    # driver had actually loaded yet. The new RebootRequiredBeforeI03
    # flag makes the halt explicit; I03 / I04 read it at entry and
    # short-circuit with a clear "halt for reboot" message. The flag
    # is per-process, NOT persisted to disk: the re-run after reboot
    # starts with $Ctx.RebootRequiredBeforeI03=$false, so I03 / I04
    # proceed normally then.
...
function Invoke-InstPhase03_InstallDrivers { # psa-disable-line PSA6003 -- compound noun (e.g., Policies, Drivers, Catalogs) is semantically plural for set-returning helpers
    ...
    if (-not (Test-MutatingPhaseAdmissible -Ctx $Ctx -PhaseId 'I03')) {
    ...
    # already) but every PnP load attempt for an unloaded device would
    # be rejected with CM_PROB_DRIVER_FAILED_LOAD (code 39) until the
    # reboot. To keep the install transcript honest and the host in a
    # clean intermediate state, I03 halts immediately when the flag is
    # set. The operator reboots once and re-runs -Action Install: on
    # the re-run, $Ctx.RebootRequiredBeforeI03 starts as $false (the
    # flag is per-process, not persisted), and I03 / I04 proceed
    # normally. See SPEC §D.32.3 for the design rationale.
    ...
    $infs = $infsToInstall
    if ($infs.Count -eq 0) {
        Write-Caution 'I03: no eligible INFs remain after phantom-file filter - nothing to install.'
    ...
    foreach ($inf in $infs) {
        ...
        if ($isSuccess) {
            ...
            if ($rebootRequired) {
                $rebootCount++
                Write-Caution '  installed (REBOOT REQUIRED)'
            } elseif ($isNoOp) {
                $noOpCount++
            ...
            $failCount++
            $exitDisplay = if ($null -eq $exit) { 'launch-failed' } else { $exit }
            Write-Caution "  exit=$exitDisplay (see $logFile)"
        }
    }
    ...
    if ($rebootCount -gt 0) {
        ...
        Write-Host ''
        Write-Caution ('{0} driver(s) need a reboot to activate.' -f $rebootCount)
        Write-Caution 'After reboot, run -Action Install AGAIN (same command). The script will'
        Write-Caution 'detect that I01/I02/I03 are done and run I04 to verify the new state.'
    }
...
function Invoke-InstPhase04_PostInstallVerification {
    ...
    # step is documented in I03's halt message above.
    if ($Ctx.RebootRequiredBeforeI03) {
    ...
    if (-not $Ctx.InstallResults) {
        Write-Caution 'I04: no I03 install results found in context. I03 must run first in the same session.'
        Write-PhaseFooter 'I04' 'done'
        return
    }
    if (-not $Ctx.BeforeDriverState) {
        Write-Caution 'I04: no BEFORE driver snapshot found. I03 must run first in the same session.'
        Write-PhaseFooter 'I04' 'done'
        return
    }
    ...
    foreach ($devId in $afterState.Keys) {
        ...
        if ($candidates.Count -eq 0) {
            ...
            # symptom: "I03 reports 1 reboot needed but I04 reports 5".
            $disposition = 'LOADED'
        ...
        # the kernel actually finished loading the driver. On a Secure
        # Boot enforced host where the SPF policy didn't actually let
        # the self-signed driver pass kernel CI, the device ends up
        # bound on paper but with ConfigManagerErrorCode != 0
        # (CM_PROB_DRIVER_FAILED_LOAD = 39, CM_PROB_NEED_RESTART = 14,
        # etc.) and the associated service stopped. Demote those to
        # the new LOAD_FAILED bucket so the per-device summary and the
        # functional health probe stop disagreeing — this is exactly
        # the surface symptom Takayuki saw on 2026-05-23 (Audio
        # CoProcessor + HD Audio Controller showed up in [LOADED] yet
        # also in the [FAIL] functional probe). See SPEC.md §D.26.
...
function Show-Help {
    ...
    Write-Host '                             Default: amd_software_8.07.16.1035.exe'
    Write-Host ''
...
# Configuration evidence, stage 'pre' (default ON; -SkipEvidenceCollection
# opts out) - runs before the first phase so the evidence reflects the
# pre-deployment system state. ListPhases performs no system interaction
# and never triggers collection.
