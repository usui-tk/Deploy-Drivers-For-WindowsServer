...
2. **Run the test suite**: `./tests/Invoke-TestSuite.ps1`. All cases must pass. No dependencies — Windows PowerShell 5.1 or PowerShell 7.x, Windows or Linux, nothing to install. The suite runs in seconds and does not touch driver state, so there is no reason to skip it. It covers the class of defect the static analyzer cannot see: a call passing a parameter the callee does not have, a wildcard containing an unterminated character class, a `try` whose scope is wrong. All three of those shipped to a production host with the analyzer reporting zero findings (see [SPEC.md §D.45](./SPEC.md)). If your change touches a function a case exercises, run the case against the *unfixed* code first and confirm it fails — a check that has never failed has not been shown to be capable of failing. Contributor guide: [`tests/README.md`](./tests/README.md); scope and rationale: [TESTING.md §27](./TESTING.md).
3. **Run `-Action PrepareVerify -CleanWorkRoot`** on a real Windows host with the target AMD consumer devices (see [TESTING.md](./TESTING.md)) and confirm the full P00-V06 pipeline completes without errors. Because this pipeline targets AMD consumer hardware, meaningful PR validation requires physical access to such devices.
4. **Update relevant docs** when you change user-visible behaviour, phase semantics, output format, or parameter sets. Per the repository-wide documentation language policy (see [SPEC.md §A.12](./SPEC.md#a12-documentation-language-policy)):
...
#    Or:     curl -sSLO https://raw.githubusercontent.com/usui-tk/ai-generated-artifacts/main/quality-tools/powershell-static-analyzer/psa.py
...
# 1. Run static analyzer (Linux / WSL / macOS / Windows with Python 3) on all five scripts
...
python3 psa.py Collect-WindowsServerConfigurationEvidence.ps1
# Expected: 0 errors on all five; warnings/info must match SPEC §A.11.5 baseline.
