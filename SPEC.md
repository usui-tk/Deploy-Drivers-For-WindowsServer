...
**Planning note for the NPU state-model refactor.** This is intentionally tracked here in normative SPEC text — not as an `// XXX: TODO` comment in any script — because the per-revision-history-in-CHANGELOG rule (PSAP0003 / PSAP0004) also forbids `TODO` markers from accumulating in script bodies. Stages 1 and 2 have completed with `psa.py 0/0/0` maintained across all four scripts; stage 3 is expected to preserve the same baseline at the stage boundary, with intermediate states explicitly accepted as transitional. After stage 2, the NPU script's phase functions and four non-phase state-touching helpers consume `$Ctx` exclusively, while the three Tier B-4 helpers (`Get-OrEnsureSecureBootBaseline`, `Resume-CtxFromWorkspace`, `Invoke-Cleanup`) still read `$Script:` globals; the top-level `$Script:Foo` assignments thus remain in place as the transitional carriage that feeds those three remaining consumers. Stage 3's helper body replacement removes both the `$Script:` reads and the corresponding top-level assignments in a single coordinated change.
