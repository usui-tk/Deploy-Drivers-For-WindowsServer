...
- **Deploy integration.** Each deploy script exposes `-CollectEvidence`; the SECTION 0.27 helper `Invoke-ConfigurationEvidenceCollector` invokes the collector in-process at stage `pre` before the first phase and at stage `post` as the LAST step of the top-level `finally` (after summary, transcript close, run-artifact archive and console-mode restore, so the collector's artifacts do not nest into the run archive). Best-effort by contract: missing collector, nonzero exit or throw produce warnings only. `ListPhases` never triggers collection.
