...
The four scripts capture a full console transcript on every run via a script-internal `Start-Transcript` / `Stop-Transcript` pair. **r91+: when `-LogFile` is omitted (the default), the transcript path is auto-generated** as `<WorkRoot>\logs\<ScriptName>_<Action>_<yyyyMMdd-HHmmss>_<PID>.log` and the transcript starts before the entry banner, so every captured log contains the banner and the complete P00 execution-environment report. There is deliberately no opt-out switch (matching the central `Update-WindowsServerIso.ps1` logging policy: transcripts are always captured). An explicit `-LogFile <path>` overrides the location. This is the canonical mechanism for retaining a run log; it supersedes the legacy `... *>&1 | Tee-Object -FilePath ...` idiom for two reasons:
...
Canonical implementation pattern (mirror across all four sister scripts). The listing below shows the simplified operator-specified core; the r91+ implementation extends it with an auto-generation prelude (computes the default `<WorkRoot>\logs` path and sets `$Script:LogFileAutoGenerated` when `-LogFile` is omitted), the verified-activation cascade, and the SECTION 0.26 suspend/resume helpers described under Required behaviour:
...
- **`-CleanWorkRoot` interaction (r91+)**: an auto-generated transcript lives inside the WorkRoot that P01 is about to wipe. The `Suspend-RunTranscriptForWipe` / `Resume-RunTranscriptAfterWipe` helpers (SECTION 0.26) stop the transcript and stash the file to `%TEMP%` immediately before the wipe, move it back after the workspace directories are recreated, and re-open it with `-Append` — one continuous file covers the whole run while the wipe itself remains a plain full-tree `Remove-Item` (no exclusion logic). Both helpers are best-effort: a failed transition emits `Write-Warning` and the run continues. Operator-specified `-LogFile` paths inside the WorkRoot keep the pre-existing relocation guard (the transcript is moved next to the script before the wipe); the guard is exempted only for auto-generated transcripts, which use the suspend/resume flow instead.
...
| `-LogFile`                   | string   |          | Transcript path override; auto-generated under `<WorkRoot>\logs` when omitted (r91+). See §A.5 |
...
| --- | ---: | ---: | ---: |
| `tools/amd-chipset-driver-research/Invoke-AmdChipsetDriverResearch.ps1` | **2** | 156 | 255 |
| `tools/amd-graphics-driver-research/Invoke-AmdGraphicsDriverResearch.ps1` | **1** | 137 | 243 |
| `tools/amd-npu-driver-research/Invoke-AmdNpuDriverResearch.ps1` | **5** | 130 | 225 |
| `tools/amd-npu-driver-research/tools/Collect-AmdNpuHardwareIdentityEvidence.ps1` | 0 | 16 | 0 |
...
### A.11.4c Registered analyzer findings in the `tools/` layer

The eight errors in the table above fall into three registered classes: five
**PSA2011 false positives**, two **PSA2010 findings that are real but
unreachable in every qualified run**, and one **PSA2001 false positive**. They
are recorded here rather than suppressed, and the reason they cannot be
suppressed is itself part of the contract.
...
the neighbouring command; the `Split-Path` calls use positional `-Path`. As of
toolkit `3.0.0` this construct appears on five lines across the three toolkit
scripts (one in the Chipset script, one in the Graphics script, three in the
NPU script).

**Evidence.** An AST walk over every `Split-Path` command in each of the three
toolkit scripts and the collector finds **zero** occurrences that actually
carry both `-LiteralPath` and `-Parent` (re-verified against the `3.0.0`
sources):
...
**PSA2010 — `Write-AmdWarn` (real, unreachable in qualified runs).** The
Chipset and NPU scripts each contain one call to `Write-AmdWarn` inside the
`catch` branch of the Windows catalog native-type initialization, and no file
in the repository defines that function (the Graphics script uses a defined
helper at the equivalent site). If that branch ever executed on Windows, the
`catch` would itself fail with `CommandNotFoundException` — the run would still
fail closed, but with a masked diagnostic instead of the intended warning. The
branch is an error path that did not execute in any accepted Windows Client or
Windows Server gate, so no qualified behavior is affected. The fix belongs
upstream in the toolkit sources at their next revision and has been reported to
the toolkit-development lane; it is registered here, not patched, for the
provenance reasons below.

**PSA2001 — `$SkipPublicExport` (false positive).** The analyzer reports an
undefined variable inside `Invoke-NpuEvidenceSnapshot`, but the variable is the
NPU script's top-level `[switch]$SkipPublicExport` parameter read through
normal parent-scope resolution. Function-local analysis does not model script
scope; the reference is legitimate.

**Why no inline suppression.** The obvious remedy — adding
`# psa-disable-file PSA2011` to the script — is unavailable. Each toolkit's
`public/publication-manifest.json` binds its generated dataset to the SHA-256
of its research script, and the accepted Windows Client and Windows Server
qualification records pin those same hashes. Editing the
...
off repository-wide to quiet five false positives in research tooling would
...
The registered PSA2010 defect is likewise expected to disappear at the next
toolkit source revision; if it survives one, raise it again rather than
re-registering it silently.
...
| 2 | `npu-state-model-refactor-step-2-phase-functions-ctx` | Migrate every NPU phase function (`Invoke-Prep*`, `Invoke-Verify*`, `Invoke-Inst*` — 21 functions, 130 `$Script:` references replaced) and the 4 non-phase state-touching helpers in scope (`Resolve-AmdNpuDriverUrl`, `Invoke-AmdAccountAuthentication`, `Show-RyzenAiSoftwareGuidance`, `Show-RunSummary` — 48 `$Script:` references replaced) plus the dispatcher (`Invoke-PhaseRunner`) from `$Script:Foo` to `$Ctx.Foo` references, and from `param()` to `param([Parameter(Mandatory)] $Ctx)` signatures (184 total replacements). The dispatcher's `& $phase.Func` call site now passes `-Ctx $Ctx`. Top-level `$Script:Foo` assignments are kept as the transitional second carriage of state because the three remaining Tier B-4 helpers (`Get-OrEnsureSecureBootBaseline`, `Resume-CtxFromWorkspace`, `Invoke-Cleanup`) still read `$Script:` globals; the duplication is the price of keeping each stage's `psa.py 0/0/0` baseline. | ✅ Complete |
| 3 | `npu-state-model-refactor-step-3-tier-b4-helper-canon` | Replaced the five Tier B-4 helpers (`Get-OrEnsureSecureBootBaseline`, `Get-BootSigningEnvironment`, `Show-BootSigningEnvironment`, `Invoke-Cleanup`, `Resume-CtxFromWorkspace`) with byte-identical copies from the Chipset canon (driver-family-specific strings such as `AMD-Chipset-Driver` → `AMD-NPU-Driver` substituted in `Resume-CtxFromWorkspace`). Adopted the marker-file `Invoke-Cleanup` pattern as canon, dropping the NPU-specific cert-subject-CN removal logic. Rewrote the NPU `Invoke-MainEntryPoint` signature from `param([Parameter(Mandatory)] $Ctx)` back to `param()` (fixing the silent mandatory-parameter regression introduced by stage 2's mechanical replacement) and rewrote its `$Ctx` construction block to read script-level parameters directly (`$WorkRoot`, `$OnlyPhases`, etc.) instead of via a self-referencing parent `$Ctx.Foo` lookup. Reconciled the NPU `$Ctx` schema with the Chipset canon — `Paths` is now a populated nested PSCustomObject (`Paths.Cert`, `Paths.Patched`, etc.) at `$Ctx`-construction time, and `CertPfxPath` / `CertCerPath` are alias-initialised from the NPU-historical `PfxPath` / `CerPath`. `$Ctx.CertThumbprint` is now populated by `Invoke-PrepPhase07_CreateCertificate` (Prepare) and by `Resume-CtxFromWorkspace` (Verify / Install) so the Chipset-canon helpers can read it without re-loading the PFX. Folded the NPU-only `$Script:DetectedPlatform` hashtable under `$Ctx.DetectedPlatform`. Removed every top-level `$Script:` state assignment (only logging / identity / instrumentation / `WdacPolicyGuid` infra remain at script scope, the last because the 3-way Tier B-3 helper `Test-AmdWdacPolicyDeployed` reads `$Script:WdacPolicyGuid` directly). Removed `Get-OrEnsureSecureBootBaseline` from `psa8001_ignore_functions`; PSA8001 now enforces 4-way byte-identity for this function. The four remaining Tier B-4 helpers stay in the ignore list because byte-identical 4-way alignment is blocked by either Chipset-vs-Graphics drift (`Get-BootSigningEnvironment` differs by a single comment line; see future-work below) or by the BthPan structurally-different implementation (`Invoke-Cleanup`, `Show-BootSigningEnvironment`) or by the per-driver-family cert filename (`Resume-CtxFromWorkspace`: `'AMD-NPU-Driver-CodeSign.pfx'` vs Chipset's `'AMD-Chipset-Driver-CodeSign.pfx'`). PSA8001 enforces 3-way (Chipset / Graphics / NPU AMD-trio) byte-identity for these where applicable. Result: Tier A grew from 38 to 39 functions; an AMD-trio 3-way byte-identity invariant was established for the four remaining Tier B-4 helpers. | ✅ Complete |

###### Function-level scope (across all three stages)

| Function | NPU divergence | Refactor scope |
|:---|:---|:---|
| `Get-OrEnsureSecureBootBaseline` | Previously: NPU `param()` + `$Script:DetectedPlatform.SecureBootBaseline` access. Canon: `param([Parameter(Mandatory)] $Ctx)` + `$Ctx.SecureBootBaseline` access. | **Resolved at stage 3.** The NPU body was replaced verbatim with the Chipset canon; the function is now byte-identical across all four scripts (Tier A 38 → 39) and `psa8001_ignore_functions` no longer lists it. PSA8001 actively enforces the 4-way byte-identity invariant. |
| `Get-BootSigningEnvironment` | Previously: NPU ~1.7 KB, probed only Secure Boot + testsigning. Canon: ~7.8 KB, additionally enumerates active WDAC supplemental policies via `CITool.exe` / registry fallback. | **Resolved at stage 3.** The NPU body was replaced verbatim with the Chipset canon (~7.8 KB). The function is now byte-identical between Chipset and NPU (and between Graphics and BthPan against the same canon **modulo a one-line comment difference** — see future-work item below for the 4-way Tier A path). Remains in `psa8001_ignore_functions` until that one-line drift is resolved. |
| `Show-BootSigningEnvironment` | Previously: NPU 2-line summary. Canon: full multi-row table. | **Resolved at stage 3.** The NPU body was replaced verbatim with the Chipset canon. AMD-trio 3-way byte-identical; BthPan's implementation is structurally simpler so the 4-way Tier A path is blocked there. Remains in `psa8001_ignore_functions` for the BthPan divergence. |
| `Invoke-Cleanup` | Previously: NPU cert-subject-CN-based removal via `Remove-CertFromTrustStore`, `Get-ChildItem Cert:\…`. Canon: marker-file-based removal via `Get-AmdSuppPolicyMarkerPath` + `Uninstall-AmdWdacPolicy`. | **Resolved at stage 3.** The marker-file pattern is now the canon implementation across the AMD trio (Chipset / Graphics / NPU 3-way byte-identical). NPU's previous cert-subject removal logic is documented in SPEC §D.36 as a post-mortem reference. BthPan keeps its own `Get-MsBthPanSuppPolicyMarkerPath`-based variant; 4-way alignment is structurally blocked. Remains in `psa8001_ignore_functions` for the BthPan divergence. |
| `Resume-CtxFromWorkspace` | Previously: NPU lightweight diagnostic-only scan (no state to rebuild because `$Script:` globals were recomputed at param-block time). Canon: rebuilds `$Ctx.CertPfxPath`, `$Ctx.CertThumbprint`, etc. from disk. | **Resolved at stage 3.** The NPU body was replaced verbatim with the Chipset canon (driver-family-specific strings substituted: `'AMD-Chipset-Driver-CodeSign.pfx'` → `'AMD-NPU-Driver-CodeSign.pfx'` × 4, `'AMD chipset driver'` → `'AMD NPU driver'` × 1). AMD-trio 3-way byte-identity is therefore **not** achieved here — each script differs by the embedded cert filename and the human-readable driver-family label. Remains in `psa8001_ignore_functions` as a documented Tier B-3 entry (no further alignment possible without unifying the cert filenames across driver families, which would break operator expectations). |

**NPU state-model refactor: outcome.** The three-stage refactor completed across the r75 → r85 release line, with `psa.py 0/0/0` maintained at every stage boundary. The NPU script's phase functions, the four non-phase state-touching helpers, and all five Tier B-4 helpers now consume `$Ctx` exclusively. Every top-level `$Script:` state assignment was removed; only logging / identity / instrumentation / `WdacPolicyGuid` infra remains at script scope (the last because the 3-way Tier B-3 helper `Test-AmdWdacPolicyDeployed` reads `$Script:WdacPolicyGuid` directly across the AMD family). The functional outcome was modest in raw counts — Tier A grew from 38 functions to 39 — because byte-identical 4-way alignment for the other four Tier B-4 helpers is structurally blocked by Chipset-vs-Graphics one-line drift, BthPan's distinct implementation, or per-driver-family cert filenames. The structural outcome is more significant: the NPU script is no longer architecturally divergent from the AMD-family canon, so future cross-script changes can land symmetrically rather than requiring a parallel NPU-specific port.

###### Future-work — `Get-BootSigningEnvironment` 4-way alignment

Of the four Tier B-4 helpers that remain in `psa8001_ignore_functions` after r85, `Get-BootSigningEnvironment` is the closest to 4-way Tier A status. The Chipset (7999 bytes) and NPU (7999 bytes, ported verbatim) bodies are byte-identical; the Graphics (7898 bytes) body differs from Chipset by a single comment-block insertion that documents an Adrenalin-installer corner case. Adding that comment to the Chipset / NPU sources (one-line comment change in two files) would lift the function to 4-way Tier A, growing Tier A from 39 to 40. This is deferred because the Adrenalin corner-case comment is genuinely Graphics-specific and inserting it into Chipset / NPU sources without a real defect to anchor it would degrade comment quality. The recommended trigger for the eventual alignment is the next time the comment needs an update for an actual cross-family defect.

###### Outstanding Tier B-4 backlog (stage 3 deferred items)

Three Tier B-4 entries that the NPU state-model refactor explicitly did **not** address remain as documented backlog:

1. **`Invoke-MainEntryPoint`-as-a-function vs Chipset's top-level dispatcher.** Chipset, Graphics, and BthPan execute the phase queue directly at script scope (no `function Invoke-MainEntryPoint`); NPU keeps the dispatcher wrapped in a function for testability and re-runnability. The wrapping forces NPU to suppress PSA2001 on five `[switch]`-typed script-param references with `# psa-disable-line PSA2001` because parent-scope lookup is opaque to static analysis. Inlining the dispatcher would remove the suppressions but is a large structural change and is not currently planned.

2. **NPU-specific flat workspace path properties.** NPU's `$Ctx` carries both the canonical Chipset `$Ctx.Paths.*` sub-keys *and* a parallel set of historical flat top-level properties (`$Ctx.CertDir`, `$Ctx.PfxPath`, `$Ctx.PatchedDir`, `$Ctx.DownloadDir`, `$Ctx.ExtractedDir`, `$Ctx.CerPath`). The phase functions still consume the flat form; the duplication is intentional carriage. Folding the NPU phase-function reads into the canonical `$Ctx.Paths.*` would eliminate the duplication but is a large mechanical rename and is not currently planned.

3. **`Get-BootSigningEnvironment` 4-way alignment** (described above).

These items are tracked here so that future refactor releases can pick them up coherently; per repository policy they will not be tagged as `// XXX: TODO` markers in any script body.
...
## A.14.5 Configuration evidence collector (r93+)

`Collect-WindowsServerConfigurationEvidence.ps1` is a standalone, read-only companion artifact that ships next to the four deploy scripts. Contract:

- **Design lineage.** Modelled on the iso-project `Collect-WindowsServerPostInstallEvidence.ps1`: schema-versioned `summary.json` (`windows-server-configuration-evidence/1.0`), timestamped evidence directory plus ZIP, `Invoke-CapturedCommand` external-command capture (stdout / stderr / exit code), `Get-RegistryKeySnapshot` registry evidence, `Get-FileEvidence` (metadata + SHA-256 + Authenticode), and a PASS / FAIL / REVIEW / INFO assessment report with the exit-code contract 0 (all PASS / INFO), 2 (any FAIL or REVIEW), 1 (fatal collector error).
- **Read-only guarantee.** The collector never changes system state and never reads private-key material: certificate evidence is limited to public properties, and the workspace inventory records names only.
- **Stages.** `-Stage pre|post|standalone` plus `-InvokedBy` are recorded in the evidence and in the ZIP name (`WindowsServerConfigurationEvidence_<stage>[_<invokedBy>]_<timestamp>`), so a pre/post pair taken around one deployment run is diffable.
- **Deploy integration (default ON since r94).** Evidence collection runs on every deploy-script run except `-Action ListPhases`; the opt-out switch is `-SkipEvidenceCollection` (r93 briefly shipped the opt-in `-CollectEvidence`; the polarity was inverted in r94 to satisfy the original always-on requirement, using the Skip form because PSA6006 forbids switches defaulting to `$true`). The SECTION 0.27 helper `Invoke-ConfigurationEvidenceCollector` invokes the collector in-process at stage `pre` before the first phase and at stage `post` as the LAST step of the top-level `finally` (after summary, transcript close, run-artifact archive and console-mode restore, so the collector's artifacts do not nest into the run archive). Best-effort by contract: missing collector, nonzero exit or throw produce warnings only.
- **Identity.** The collector carries the pipeline identity variables (`$Script:ScriptVersion` = `collector-<date>-cN`, `$Script:ScriptTag`, self-computed `$Script:ScriptHash`) and records them in its own evidence; the deploy-script inventory section also records the collector's hash, so evidence is self-describing.
...
| --- | --- | --- | --- | --- |
| Windows Server 2025 | 26100 | Windows 11 24H2 | `W11x64` | Package-variant compatibility heuristic: treated as the W11x64 variant target (build-based selector input only — not a Windows security-capability claim); supports Pluton / PMF / USB4 / 3D V-Cache |
...
Running NPU `-Action Install` on **Windows Server 2019** (build 17763) or **Windows Server 2016** (build 14393) **has never been exercised on a physical NPU host**. The three historically field-validated driver scripts (Chipset / Graphics / BthPan) each have at least one pre-remediation real-bench cycle on legacy Server SKUs; the NPU script has zero. After the r70 Path C deprecation (see §D.30), the legacy-Server install path on those scripts has also been formally removed in favour of operator-driven Path B (testsigning, requires Secure Boot off in firmware) — but the NPU script has no physical evidence that even Path B works for the XDNA driver stack on these OS versions.
...
Once that physical validation has been completed and reviewed, the guard in `Invoke-PrepPhase00_Initialize` can be removed in a future release and the legacy-Server path documented as field-validated for NPU.
...
## D.36 NPU state-model refactor: post-mortem of the 3-stage workstream

The NPU state-model refactor closed out at stage 3 (release line r75 → r85). The end result — Tier A grew from 38 functions to 39, with an AMD-trio 3-way byte-identity invariant established for the remaining four Tier B-4 helpers — is the headline finding for `psa8001` accounting. Three secondary findings are worth recording here as architectural rationale for future sister-script work.

### D.36.1 The cert-subject-CN `Invoke-Cleanup` pattern: why it was abandoned

NPU's pre-r85 `Invoke-Cleanup` body enumerated `Cert:\LocalMachine\Root\` and matched on `$cert.Subject -match [regex]::Escape($Script:CertSubjectCn)` to identify the certificate to remove. This worked correctly when a single deployment had run on the host, but it had two failure modes that the marker-file canon (Chipset / Graphics) does not:

1. **Multiple deployments on the same host.** If the operator had run the script under two different `$Script:CertSubjectCn` values (e.g. a Lab subject and a Production subject), the cert-subject scan would find both and need a tie-break heuristic. The marker-file canon writes the actual `PolicyId` (UUID) into `Paths.Cert\AmdSuppPolicyId.txt` at install time, so cleanup is unambiguous regardless of how many subject CNs accumulated in the local cert store.

2. **Subject-CN drift between releases.** If a future release changed `$Script:CertSubjectCn` (even as a typo fix), the new cleanup logic could not find certificates installed by the old release — they would orphan in the cert store with no automated recovery path. The marker-file canon decouples cleanup-time identity from build-time strings entirely.

Stage 3 adopted the marker-file pattern as canon and dropped the cert-subject scan entirely. The pattern is documented here rather than retained as a fallback in the new canon because tying cleanup to two parallel identity sources (marker file + cert-subject regex) would re-introduce the original ambiguity it was trying to avoid.

### D.36.2 The r83 `$Ctx` skeleton design judgment

At r83 the NPU script gained a `$Ctx` skeleton that declared the Chipset-canon property set (`CertPfxPath`, `CertCerPath`, `CertThumbprint`, `Paths`) but **initialised them as `$null` while the NPU phase functions continued to read the parallel NPU-historical properties** (`PfxPath`, `CerPath`, `PatchedDir`, `CertDir`, etc.). The reasoning at the time was to land the schema visibly so stage 2 (phase-function migration) and stage 3 (helper body replacement) could proceed against a stable target.

In hindsight this design choice cost an entire additional stage. Had the canon properties been alias-initialised from the NPU-historical values *at r83-time* — i.e. `CertPfxPath = $PfxPath`, `Paths = [pscustomobject]@{ Cert = $CertDir; ... }` — the Tier B-4 helpers could have been ported verbatim at stage 2 (where the phase functions were already being mass-edited) rather than waiting for a dedicated stage 3. The `$null` placeholders looked like "schema only, no semantics" but they actually constrained downstream consumers in a way that took longer to unwind than a cleaner alias initialisation would have.

The principle for future cross-script refactors: when introducing a canonical schema alongside legacy state, **initialise the canonical properties with the legacy values from day one**, even if the canonical-side consumers won't read them until a later stage. The `$null` placeholders cost more in cross-stage coupling than the cleaner alias initialisation would have saved in stage-2 review burden.

### D.36.3 The r84 "Tier A 41-way / 38 → 43" overstatement (forward-looking correction)

The r84 SPEC §A.11.7 stage-2 row and the r84 CHANGELOG entry described the planned stage-3 outcome as **"PSA8001 then enforces 41-way Tier A"** and the achievable Tier A delta as **"38 → 43"** (one count per Tier B-4 helper transitioning to Tier A). The actual stage-3 outcome was **38 → 39** — only `Get-OrEnsureSecureBootBaseline` reaches 4-way byte-identity. The four other Tier B-4 helpers (`Get-BootSigningEnvironment`, `Show-BootSigningEnvironment`, `Invoke-Cleanup`, `Resume-CtxFromWorkspace`) are blocked from 4-way Tier A by:

- **`Get-BootSigningEnvironment`**: Chipset and Graphics differ by a single comment line (the Graphics version documents an Adrenalin-installer corner case that doesn't apply to Chipset / NPU). Bringing all three to byte-identity requires either propagating the Adrenalin comment into Chipset / NPU sources (where it isn't applicable) or removing it from Graphics (where it has documented Adrenalin-specific value). Both options have a real cost; deferring is the right judgment until a real defect anchors the comment.
- **`Show-BootSigningEnvironment`**: BthPan's implementation is structurally simpler than the AMD-trio canon because BthPan ships an inbox driver and has no AMD-account / installer environment to display. 4-way alignment would require either expanding BthPan's display (adding fields that have no data) or shrinking the AMD canon (losing fields that have operator value).
- **`Invoke-Cleanup`**: BthPan uses `Get-MsBthPanSuppPolicyMarkerPath` (per-driver-family marker filename), AMD trio uses `Get-AmdSuppPolicyMarkerPath`. The functions are structurally identical but call into different per-family helpers; 4-way alignment would require unifying the marker filename across driver families, which is operator-visible and would break existing deployments.
- **`Resume-CtxFromWorkspace`**: each script's body embeds its own cert filename (`'AMD-Chipset-Driver-CodeSign.pfx'` vs `'AMD-NPU-Driver-CodeSign.pfx'` vs `'MS-BthPan-CodeSign.pfx'`) and human-readable driver-family label. 4-way alignment would require unifying the cert filenames across driver families — same operator-visibility concern as above.

The r84 "41-way" framing did not account for these structural constraints, instead treating each Tier B-4 helper's stage-3 body replacement as automatically yielding a 4-way Tier A promotion. The actual analysis (per-helper, with the structural blockers explicit) lifts only `Get-OrEnsureSecureBootBaseline` cleanly; the others establish 3-way (AMD-trio) byte-identity but do not reach 4-way Tier A.

Per repository policy the r84 historical text (in SPEC §A.11.7's stage 2 row, in CHANGELOG r84 entries) is preserved verbatim — historical records remain as written. This §D.36.3 entry is the **forward-looking correction**; future cross-script refactors planning a Tier-A promotion should run the per-helper structural-blocker analysis explicitly rather than assuming `body replacement → 4-way Tier A`.


## D.37 AMD chipset installer rename (2026-07): URL-discovery hardening post-mortem

### D.37.1 Symptom

A 2026-08-07 operator run (Windows Server 2016 / PowerShell 5.1 / Ryzen 5 PRO 4650U, `-Action PrepareVerify -CleanWorkRoot`) showed the Chipset script probing all 3 AMD landing pages with **0 installer-URL hits each**, then silently falling back to the pinned URL and downloading **8.02.18.557** even though AMD had published **8.07.16.1035** on 2026-07-30. No error was raised — the run "succeeded" with a stale driver package.

### D.37.2 Root cause

AMD renamed newly-published chipset installers from `amd_chipset_software_<version>.exe` to `amd_software_<version>.exe`. Confirmed live on 2026-08-07 against the Ryzen/Athlon mobile chipset landing page: the download link is `https://drivers.amd.com/drivers/amd_software_8.07.16.1035.exe` (Revision 8.07.16.1035, released 2026-07-30). The discovery regex only accepted the legacy `amd_chipset_software_` prefix, so every probe returned 0 hits. This is a plain naming-scheme change, not anti-bot blocking: the pages were fetched successfully (HTTP 200, full HTML bodies) — the pattern simply no longer matched. The same pages also link `amd_raid_software_<version>.zip`, so any widened pattern must not match the `_raid_` sibling.

### D.37.3 Fix (r91)

- Discovery regex widened to accept both namings: `amd(?:_chipset)?_software_(\d+\.\d+\.\d+\.\d+)\.exe`. The optional non-capturing group admits only the exact `_chipset` infix, so `amd_raid_software_*.zip` cannot match (wrong infix AND wrong extension).
- The P03/P04 download-cache reuse filters are widened to the same dual naming (`-Filter '*.exe'` + `Where-Object { $_.Name -match '^amd(_chipset)?_software_' }`), so a cached new-naming installer is recognised on re-runs.
- The pinned fallback URL is updated to `https://drivers.amd.com/drivers/amd_software_8.07.16.1035.exe` (new naming, newest known revision at the time of the fix; noted in-source as not yet E2E-verified through the full install path).

### D.37.4 Evidence-preservation rule (generalised)

The silent-fallback design made the failure invisible: the operator had to notice the version number in the console to detect staleness. r91 adds probe-miss evidence preservation — when all landing-page probes miss, the fetched page bodies are saved to `<WorkRoot>\logs\amd-landing-probe-<timestamp>-<nn>_<page>.html` before falling back. **Rule: any scraper with a silent fallback SHOULD preserve the non-matching input as an artifact.** Without the artifact, a naming-scheme change is indistinguishable from a network failure or an anti-bot block after the fact; with it, the next regex fix can be grounded on the actual page HTML instead of reverse-guessing.
...
## D.46 A guard in the wrong function, a pre-flight that fired on its own default, and the evidence a cross-version campaign needs

### D.46.1 A guard that compiled, passed every gate, and was never reached

The r100 degenerate-plan guards did not run. P06 correctly detected the empty
`-SkipNonCosignedDrivers` plan, printed the explanation, and closed as
`skipped` - and P08 then failed with:

```
P08: patched directory has no INFs to catalog. Run preparation phases first.
```

naming a phase that had just run and had nothing to do.

The guards were in the wrong functions:

| Intended | Actual |
|---|---|
| `Invoke-PrepPhase08_GenerateCatalogs` (line 10013) | `Invoke-PrepPhase09_SignCatalogs` (line 10547) |
| `Invoke-PrepPhase09_SignCatalogs` | `Invoke-VerifyPhase03_VerifyCatalogs` |

The r100 editing tool located each insertion point by searching for the next
`    )` after the function's opening line, assuming it was that function's
param-block terminator. For P08 it was not; the match landed roughly 500
lines below, and P09's guard cascaded one function further. Both guards were
syntactically valid, both referenced a real property on a real object, and
`psa.py`, `Parser::ParseFile` and the ValidateSet/binder audits were all
green on a file whose fix was unreachable.

This is the same failure class as D.44 and D.45.2 in a new costume: a change
that is well-formed but wrong, in a way no static check can see. The
correction is not "be careful with the editor" - it is that **placement is
now asserted**. The editor asks PowerShell's own parser for each function's
extent, inserts inside it, re-probes, and refuses to write if the guard is
not within the target function's line range. `Test-SisterConsistency.ps1`
repeats the check against the committed file, deriving the set of sisters to
check from which ones actually call `Get-EligibleInfRecordList` rather than
hard-coding a list.

BthPan is deliberately excluded by that derivation: it carries the shared
helper for byte-identity but drives a single inbox INF, so its trim never
runs and a guard there would be unreachable code.

### D.46.2 A pre-flight check that fired on the script's own default

BthPan failed at P01 on a plain `-Action PrepareVerify -CleanWorkRoot`:

```
P01: -LogFile is inside -WorkRoot and -CleanWorkRoot was requested.
```

The operator had not passed `-LogFile`. All four sisters auto-place the
transcript at `<WorkRoot>\logs\<script>_<action>_<stamp>_<pid>.log` when it
is omitted, using identical code, and set `$Script:LogFileAutoGenerated` to
record that they did. The upstream relocation guard excludes that case
correctly - it owns the suspend/wipe/resume flow for exactly this situation.
The P01 check did not, so it detected the script's own default as an operator
error and offered three resolutions, none of which apply to a path the
operator never chose.

Two things made this worse than a simple missing condition. **Only BthPan has
this pre-flight** - Chipset and Graphics have no such check - so identical
invocations of the three scripts produced two successes and one failure, and
the project's own rule that the four sisters keep matching designs was the
thing that would have caught it. And it fires on the *default* invocation,
which is the one a new operator uses first.

The fix adds `-not $Script:LogFileAutoGenerated`, making the P01 check agree
with the upstream guard. An operator-supplied `-LogFile` inside `-WorkRoot`
still refuses, which is the case the check was written for.

### D.46.3 Evidence for a cross-version campaign

A WS2016 validation campaign is planned, and WS2016 is the OS this project
adapts to most. The adaptations are documented in this SPEC and implemented
in the per-build profile table, but nothing in an evidence bundle recorded
**which of them held on the host that produced it**. A troubleshooter looking
at a failed WS2016 run had to look up what WS2016 is supposed to lack and
then assume the host matched.

Two new collector stages replace that assumption with measurement.

**`os-capability.json`** records, per host: the OS caption, build and UBR;
the PowerShell version, edition and culture; which per-build profile the
deploy scripts would select and whether the build matched exactly or fell
back; the `inf2cat` target and certificate parameters that profile implies;
the presence, source and version of fourteen cmdlets the scripts branch on;
the presence of six CIM classes including `PS_UpdateAndCompareCIPolicy`,
whose absence on WS2016 is the difference between an immediate-activate
policy path and a reboot fallback; and the resolved path and product version
of `signtool`, `inf2cat`, `pnputil`, `bcdedit` and `certutil`.

The last of those is not incidental. A WHQL verdict produced without
`signtool` is a conservative default, not a measurement, and the analysis
output alone does not distinguish the two. D.45.5's finding - 0 of 55 INFs
co-signed - is only load-bearing because `signtool` was present, and that
fact previously had to be dug out of a transcript.

**`archive-capability.json`** proves the archive mechanism works by using it:
it writes a handful of small files to a temp directory, compresses them,
reads the entry count back, and records the result along with the resolved
temp path, free space, and whether long paths are enabled. The collector's
own ZIP depends on `Compress-Archive` succeeding in a `finally`; when that
fails, the operator gets a warning and a loose directory, and working out why
afterwards means guessing at PowerShell version, item count, path length or
disk space. The probe answers "does this work on this host" before the real
archive is attempted.

The feature watch list also now records `FeatureNameKnown` per entry. The
list is written from one Server version's naming, and a name that does not
exist on another SKU would otherwise report `Unknown` forever and quietly
stop being a check. `UnknownFeatureNames` makes that visible.

Both stages are reported in the assessment: `OS capability matrix` (INFO, or
REVIEW when a required tool is absent - a capability missing on an older SKU
is expected, not a fault) and `Archive capability` (PASS/FAIL).

### D.46.4 The OS differences this project depends on

Consolidated here because a cross-version campaign needs one place to check
against, and because every row is now also measured into the bundle.

| Aspect | WS2016 (14393) | WS2019 (17763) | WS2022 (20348) | WS2025 (26100) |
|---|---|---|---|---|
| `inf2cat /os:` | `Server2016_X64` | `ServerRS5_X64` | `ServerFE_X64` | `Server2025_X64` |
| Certificate key / validity | 2048-bit / 3 years | 4096-bit / 5 years | 4096-bit / 5 years | 4096-bit / 5 years |
...
The behavioural consequences are already implemented: `Get-RebindCapability`
probes for each cmdlet and degrades the rebind strategy rather than failing,
...
The two new stages were written, passed `psa.py` and the parser, and then
**failed three times on first execution**:

1. An empty `$env:TEMP` made every `Join-Path` throw on a mandatory
   parameter, reporting "archive unavailable" when the truth was "nowhere to
   probe". Now falls back through `TMP` and `GetTempPath()` and records which.
2. `Split-Path -Qualifier` throws outright on a path with no drive letter, so
   collecting free space could abort the probe. Free space is a diagnostic
   detail and is now matched rather than parsed.
3. `'{"probe":%d}' -f $i` - the format operator treats `{` and `}` as
   placeholder delimiters, so a JSON literal on its left fails to parse
   before any file is written. Replaced with concatenation.

All three are in `Test-CollectorOsCapability.ps1` as regression guards,
including a case that empties `TEMP` and `TMP` and asserts the probe still
returns a usable path.

### D.46.6 Verification

`psa.py` 0 errors / 0 warnings / 0 info across eleven PowerShell files;
`Parser::ParseFile` 0 errors across five; test suite **4 cases, 79
...
Negative control: run against r100, the suite fails on exactly the three
defects this release fixes - both misplaced P08 guards and the BthPan
pre-flight - and passes against this tree.

Still unexercised on Windows: the CIM and registry probes in
`Get-OsCapabilityEvidence`, and the degenerate-plan path end to end. The
archive probe has run for real, on Linux.

### D.46.7 Sister impact
...
## D.52 WinRE tool availability, and the framework version this project had wrong

### D.52.1 The run that forced this

The first execution of `Collect-OfflineRecoveryEvidence.cmd` in a real
recovery environment — on a host that would not boot — stopped at stage 7:

```
[ 7/16] Registry queries...
  CrashControl:
'findstr' は、内部コマンドまたは外部コマンド、
操作可能なプログラムまたはバッチ ファイルとして認識されていません。
```

**WinRE does not ship `findstr`.** Stages 8 to 16 never ran, so the framework
binaries, the pending-servicing markers, the dump inventory and the summary
were all lost — on the one machine that needed them.

Everything before that worked, including the parts that had never been
exercised outside a booted host: mode detection selected `offline`, the
Windows volume was found by its config hive, and the destination was found by
writing a probe file — the read-only volume ahead of it was correctly
rejected (`アクセスが拒否されました`, then `found writable: D:`).

The mistake was not using `findstr`. It was **assuming** which commands WinRE
has. This project has a rule against exactly that, and it was not applied to
the recovery environment because the recovery environment had never been
measured.

### D.52.2 Correction: Windows Server 2016 ships KMDF 1.19, not 1.15

An earlier session stated, in conversation and in planning, that the in-box
KMDF on Windows Server 2016 is 1.15. **That is wrong.** From Microsoft's KMDF
and UMDF version history:

| OS build | Documented KMDF | Documented UMDF | Kind | Observed KMDF (measured) |
|---|---|---|---|---|
| 14393 — Windows Server 2016 / Windows 10 1607 | **1.19** | **2.19** | IncludedVersion | not yet measured |
| 17763 — Windows Server 2019 / Windows 10 1809 | 1.27 | 2.27 | IncludedVersion | **1.27** (agrees) |
| 20348 — Windows Server 2022 | 1.33 | 2.33 | IncludedVersion | not yet measured |
| 26100 — Windows Server 2025 | 1.33 | 2.33 | PublishedReference | **1.35** (exceeds) |

The right-hand column was added after a Windows Server 2025 host measured
KMDF 1.35 against a published 1.33 (D.56). The documented columns are not
edited to match: they record what Microsoft publishes, and their only use
...
per-row by the test suite, so a future recollection cannot quietly replace it.

### D.52.3 What the recovery collector now does

**`findstr` is gone.** Three call sites become `find`, which WinRE does have.
`find` takes one literal string per invocation and no regular expressions, so
each pattern is its own call. One site — matching the localised package-state
column — cannot be done with `find` at all, because a `.cmd` must stay plain
ASCII and the token is Japanese on this host; the manifest points at the full
table instead of trying to summarise it.

**A tool census (stage 15).** Twenty commands are tested for presence, and
the environment's own `System32\*.exe` listing is captured. The next absence
of this kind will be a line in a bundle rather than a discovery mid-incident.

**File versions, not just `dir` (stage 8).** `dir` reports size and date. The
KMDF version — the number every WS2016 compatibility question turns on — was
not in any bundle. `:fileversion` tries `wmic datafile`, which reports it
directly, and falls back to the `dir` line already recorded when `wmic` is
absent. Every `WdfCoInstaller*.dll` is enumerated including the DriverStore,
because a co-installer names the framework version its package was built for:
finding `WdfCoInstaller01031.dll` on a host running 1.19 states a mismatch no
other single artefact does. Services depending on `Wdf01000` are listed —
those are the WDF-based drivers, and on a `WDF_VIOLATION` they are the
suspect pool.

**Bugcheck reference and dump presence (stage 14).** Parameter 1 of 0x10D
names which framework contract was violated, and a power-operation timeout, a
handle-type error and a PnP IRP collision lead to three unrelated
investigations. The decode table travels inside the bundle so the reader does
not have to look it up. Dump presence is stated explicitly, because "no
minidump" means one thing with `CrashDumpEnabled = 0` and something else
entirely with `0x7`.

**Startup repair logs (stage 13).** The `Srt` directory and `ReAgent.xml`
record what Windows already tried. A repair attempt that failed is as
informative as the original fault.

### D.52.4 What the collector now assesses

`wdf-assessment.json` (stage 18/18) compares what the host provides against
what is installed on it, alongside `wdf-coinstallers.json` and
`wdf-services.json`.

Version comparison is numeric, and this is not a detail. As strings, `1.19`
sorts **below** `1.9` — which would report a driver needing 1.19 as satisfied
by a 1.9 runtime, silently and in the safe-looking direction.
`ConvertTo-WdfVersionNumber` scales the minor part so ordering matches the
real sequence, and the test asserts the string comparison really is wrong
before asserting the numeric one is right.

Only co-installers **above** the host version are flagged. An older one runs
fine, and flagging it would bury the finding in noise.

### D.52.5 The limit of this assessment, stated plainly

**None of this predicts `WDF_VIOLATION`.**

Two failure modes hang off the framework version and only one is visible in
static data:

- A driver requesting a **newer** version than the runtime provides **cannot
  load**. That is not a signing failure — neither Path A nor Path B moves it,
  the same shape as the `STATUS_DRIVER_ENTRYPOINT_NOT_FOUND` case in D.43.3.
  `inf2cat /os:Server2016_X64` sets the catalog's target OS and does **not**
  lower a KMDF requirement, so a package can be catalogued and signed
  successfully for a host it cannot run on.
- A driver that **does** load and then breaks the framework contract produces
  `WDF_VIOLATION`. No static property of the package reveals that.

What the assessment buys is bounding: which drivers are WDF-based at all, how
many of those are boot or system start (the ones that can bugcheck before
anything can be logged), and which packages cannot load before one is
installed. The `Note` field in the JSON says this, so a reader does not
mistake the artefact for a predictor.

### D.52.6 Verification

`psa.py` 0 errors / 0 warnings / 0 info across thirteen PowerShell files;
`Parser::ParseFile` 0 errors across five; test suite **6 cases, 239
assertions**, all passing; canon integrity via the central authoritative
tooling per §A.11.8a — 125 dd observation records, zero differences.

**Negative control**: run against r106, the recovery-collector case reports
14 failures.

### D.52.7 Deliberately not in this release

The INF-side extraction — reading `KmdfLibraryVersion` from each driver INF
and comparing it against the host runtime — is designed and its helper is
written, but **is not included**. The four sister scripts turned out not to
share the anchor it needs: `Test-InfHasServerDecoration` is absent from NPU,
and only Chipset builds the INF inventory rows the new columns attach to.

Placing it by guesswork is exactly the failure of D.46.1, where a guard
compiled cleanly into the wrong function and was never reached. It waits for
per-sister confirmation rather than shipping in a form that might not run.

### D.52.8 Sister impact

Collector c7 / schema 1.6, plus the recovery collector. The four sisters take
...
## D.55 Two defects a real Windows Server 2019 run found in one pass

### D.55.1 The run

A clean Windows Server 2019 install (build 17763), PowerShell 5.1, three
sisters run with `-Action PrepareVerify -SkipNonCosignedDrivers`. Chipset and
Graphics both ended with V01 FAILED. BthPan completed all phases. Nothing had
been installed.

Two separate defects, one of them shipped by the release immediately before.

### D.55.2 Defect 1 — the Verify side never learned what the Prepare side knew

D.45.6 states the principle: an empty trimmed plan is a **measurement of the
driver package, not a failure**. The Prepare side implements it. P06 detected
that none of the in-scope INFs carry a WHQL co-signature, printed the
explanation, and closed as `skipped`. P08 and P09 consulted
`$Ctx.DegeneratePlan` and closed as `skipped` too. Exactly as designed.

Then V01 looked for patched INFs, found none, and threw
`V01 found 1 missing artifact(s) - run earlier phases first`. The phase it
told the operator to run had run, had decided correctly, and had said so. The
run aborted, and **V02 through V06 never executed** — so the certificate
verification, the catalog checks, the dry-run and the hardware impact analysis
were all lost to a phase that was reporting an expected condition as a
failure.

V04 carried the same defect one phase later, on the same reasoning:
`V04: no patched INFs to verify - run P06 (PatchInfs) first`.

The guard was added to P08 and P09 when the degenerate path was built. Verify
phases were not considered then, and no test asserted anything about them, so
the asymmetry sat in the tree until a machine with a non-co-signed driver
package ran the pipeline end to end. **The Prepare side and the Verify side
disagreed about whether an empty plan was an error, and only one of them was
right.**

The fix differs per phase because their jobs differ:

- **V01** keeps running. The certificate and the inventory CSV are real
  artifacts and are still verified. Only the patched-INF check changes: under
  a degenerate plan their absence is reported as expected, not as a failure.
- **V04** exists solely to verify patched INFs. With none, it closes
  `skipped`, exactly as P08 and P09 do.

`Test-SisterConsistency` now asserts both, in the same guard table that
already covered P08 and P09 — which is where the check belonged from the
start.

### D.55.3 Defect 2 — a host version that was never measurable

The comparison shipped in D.54 read the host's UMDF version from
`WudfRd.sys`, keeping the first two parts of its file version exactly as it
does for KMDF. On this host it produced **`UMDF 10.0`**.

That is the operating system version, not a framework version. Measured in
the same run:

| Binary | File version | What it is |
|---|---|---|
| `Wdf01000.sys` | `1.27.17763.1192` | KMDF 1.27 — the library version |
| `WudfPf.sys` | `10.0.17763.9020` | OS version |
| `WUDFRd.sys` | `10.0.17763.9020` | OS version |
| `WUDFHost.exe` | `10.0.17763.9020` | OS version |

The documented UMDF version for build 17763 is 2.27. **No binary on the
machine carries it.** KMDF happens to be measurable; UMDF is not, and the
symmetry of the two readings was an assumption, never a measurement.

The consequence is worse than a wrong display. `ConvertTo-WdfVersionNumber`
maps `10.0` to 10000, and every real UMDF requirement lands between 2000 and
2099. **Every UMDF driver compared as satisfied, on every host.** A false
negative, in the direction that produces no output and no complaint — the
exact failure mode D.53.3 was written to prevent, arriving through the host
side rather than the INF side.

It also means the reassuring line in the field run — all 119 INFs within the
host version — was only half earned. The KMDF half was real: the host
measured 1.27 and the highest declared requirement in the package is 1.19.
The UMDF half, covering 8 INFs that declare 2.15, was never actually
compared.

The fix reports UMDF as **unknown** and counts the requirements it could not
judge, separately and visibly, in P05's output and in the summary object. An
unjudged requirement is not a pass; it is a question that was not asked, and
it now says so.

Deriving the UMDF version from the OS build is possible — the collector
already maps build to an expected version — and is deliberately **not** done
here. That is an expectation table, and D.47.2 is the record of what happens
when one is read as fact. Substituting it quietly would replace a visible gap
with an invisible assumption. It is left as an open decision.

The collector carried the same derivation and reported
`ActualUmdfVersion = "10.0"` against `ExpectedUmdfVersion = "2.27"` in its own
assessment JSON, so the wrong number was in the evidence too. It is corrected
in the same pass. Only KMDF was ever compared there, so no verdict was wrong —
but the field was, and a field that is wrong in an evidence bundle is worse
than an empty one.

### D.55.4 Why the tests did not catch either one

Both defects were in code that had tests, and both tests were sound.

`Get-WdfShortfallSummary` is a pure function and was tested from both
directions of the ordering trap, with a 25-entry list checked to its last
element. It passed because it was correct. The host version arrives as an
**argument**, and nothing tested where that argument came from — the one part
of the path that could not be tested on Linux was the part that was wrong.
"Test the function, not only its parts" (D.45.7) turns out to have a second
edge: testing the testable part thoroughly can leave the untestable part
entirely unexamined, and feel like coverage.

The degenerate guard had a test asserting P08 and P09 carry it. The table
listing which phases must carry it was written when only Prepare phases had
been considered, so the test asserted the exact scope of the original thinking
and could never have found a phase missing from its own list.

### D.55.5 First real-hardware reading of the requirement columns

The columns from D.53 had never been measured against a real package. This
run provides that, and it is recorded here because it is the evidence the
gating decision in D.54.6 was waiting for.

| | Chipset | Graphics | BthPan |
|---|---|---|---|
| Inventory rows | 119 | 19 | 1 |
| `IsWdfDriver = True` | 36 | 8 | 0 |
| KMDF versions declared | 1.13 (2), 1.15 (24), 1.19 (2) | 1.15 (8) | none |
| UMDF versions declared | 2.15 (8) | none | none |
| Non-empty `CoInstallerVersions` | 0 | 0 | 0 |

Three findings:

1. **The highest requirement in the package is KMDF 1.19**, comfortably below
   the 1.27 this host provides. Nothing in this package is blocked by the
   framework version on Windows Server 2019. On Windows Server 2016, whose
   runtime is 1.19, the two INFs declaring 1.19 sit exactly at the boundary
   and the rest are below it — so this package is not the case the check was
   built for. That does not make the check useless; it makes this package a
   clean negative.
2. **No INF references a WDF co-installer at all.** The odd combination flagged
   as an open question in D.53.2 — `IsWdfDriver = False` with a non-empty
   `CoInstallerVersions` — does not occur here even once, so the field
   evidence does not force a decision on it.
3. **The extraction works on real INFs.** 36 of 119 identified as WDF drivers
   with plausible version spreads, and no parse failure or empty column where
   a `.Wdf` section exists.
...
**Negative control, both defects.** The extended cases were run against the
tree that produced the field failures: the UMDF assertions failed with
`The property 'UnjudgedUmdfCount' cannot be found on this object`, and the
guard assertions failed with four namings — V01 and V04, Chipset and Graphics.
Both pass on the corrected tree.

**Executed, not only parsed.** The P05 notice was replayed against the real
inventory shape from the field run — 119 records with the measured version
spread, host KMDF 1.27, UMDF unknown — and against a control record declaring
KMDF 1.33, which it flags by name.

### D.55.7 Sister impact

Chipset r110 / Graphics r76 / NPU r53 / BthPan r58 / collector c8.

The Verify guards go to Chipset, Graphics and BthPan, whose V01 and V04 bodies
are byte-identical. NPU's V01 and V04 are structurally different and NPU never
reaches a degenerate plan, so it is left alone rather than given a guard that
could not fire.

BthPan carries the trim helper and the guards but has never produced an empty
plan: it drives a single Microsoft inbox INF that is co-signed. The guards
there are insurance, not a fix.

## D.56 Observed and documented are different columns

### D.56.1 Where the previous model ran out

D.55 stopped reading the host UMDF version because no binary carries it, and
left UMDF requirements unjudged. That was correct and incomplete: it removed a
false negative without putting anything in its place, so a package declaring
UMDF 2.15 on a host documented at 2.27 went uncompared for want of a number.

Independent field measurement supplied what was missing. Three Windows
releases were measured directly:

| Release | Build | `Wdf01000.sys` numeric fields | KMDF observed | KMDF published |
|---|---:|---|---:|---:|
| Windows Server 2019 | 17763.9020 | `1.27.17763.1192` | 1.27 | 1.27 |
| Windows Server 2025 | 26100.33158 | `1.35.26100.3323` | 1.35 | **1.33** |

The 2025 measurement is the important one. It means the capability table in
D.46.4 and D.52.2, which lists Windows Server 2022 and 2025 together at
1.33/2.33, is **wrong as a statement about 2025** — a third instance of the
D.47.2 lesson, after `PS_UpdateAndCompareCIPolicy` and the KMDF 1.15 error.

The fix is not to write 1.35 into the table. That would keep a single column
doing two jobs and make the next disagreement just as invisible. The table is
split.

### D.56.2 The two columns

**Observed** is what this machine reports. It is measured, it wins, and it can
legitimately be newer than anything Microsoft has published.

**Documented** is what Microsoft publishes for a base build. It never receives
a measured value, however good the measurement, because its entire use is to
be compared against measurement — a column that absorbs its own comparison
input stops being able to disagree, which is the only thing it is for.

`Get-WdfDocumentedBaseline` holds the published values and labels each row
with how firm it is:

| Build | Release | Documented KMDF | Documented UMDF | Kind | Confidence |
|---:|---|---:|---:|---|---|
| 14393 | Windows Server 2016 | 1.19 | 2.19 | IncludedVersion | High |
| 17763 | Windows Server 2019 | 1.27 | 2.27 | IncludedVersion | High |
| 20348 | Windows Server 2022 | 1.33 | 2.33 | IncludedVersion | High |
| 26100 | Windows Server 2025 | 1.33 | 2.33 | **PublishedReference** | Medium |

Server 2025 is `PublishedReference` because Microsoft's version history does
not list it as an included-in release; a measured 1.35 sits **above** that row
rather than contradicting it. An unmapped build documents nothing and says so.

### D.56.3 When the documented UMDF may be used, and how that is decided
...
documented UMDF is 2.33 from the same table whose KMDF is already known to be
two revisions behind the machine, so a driver requiring 2.35 would be declared
unloadable on a host that may well carry it.

The decision is made from evidence rather than assumed. **The documented UMDF
is used only when this host's measured KMDF equals its documented KMDF.** That
equality is the observable proof that the published table has kept up with
this particular build, and the UMDF entry beside it comes from the same table
and the same revision of it.

| Measured vs documented KMDF | UMDF treatment |
|---|---|
| Equal | Documented UMDF adopted, labelled as documented and corroborated |
| Measured newer | UMDF left unjudged — the table is demonstrably behind this build |
| Measured older | UMDF left unjudged, and the KMDF gap itself is flagged |
| Not measurable | Nothing judged |

Deriving UMDF as `2.<measured KMDF minor>` was considered and rejected. The
pairing holds across every generation in the table, which is exactly what
makes it tempting, but it is an inference from a pattern rather than a reading
of anything, and the source research explicitly declines to conclude a UMDF
minor version from available evidence. An inference that looks like a
measurement is the failure this section exists to prevent.

The UMDF 2 framework library `WUDFx02000.dll` is probed for presence, in both
documented locations. Presence is real evidence that a UMDF 2 runtime exists;
its file version is a Windows component version and is recorded without being
converted.

### D.56.4 A PE carries its version twice, and the two disagree

On the Windows Server 2019 field host, one file at one moment:

```
FileVersion (string resource) : 1.27.17763.1 (WinBuild.160101.0800)
ProductVersion                : 1.27.17763.1
numeric version fields        : 1.27.17763.1192
```

The string resource is typically written once at RTM and left alone; the
numeric fields move with servicing. Both are genuine, and a reader given only
one of them cannot tell which they have.

This resolves an ambiguity in the source research, which recorded
`1.27.17763.1192` and `1.27.17763.1` as observations from two different
environments. They need not be: a single host produces both. The research's
conclusion — that a full four-part file version must not be used as a fixed
identity for a framework generation — is right, and the reason is stronger
than stated, because it does not depend on having two machines.

Two consequences for this repository:

1. **Derivation uses the numeric fields.** The previous implementation split
   the FileVersion **string**, which happens to give 1.27 here and fails
   outright on the comma-separated form some binaries carry
   (`1, 27, 17763, 1` splits on `.` into one element).
2. **Both readings are recorded and shown.** `Get-BinaryVersionFact` returns
   the numeric version, the string and the product version separately, and
   P05 prints both, so the displayed evidence can be used to check the derived
   answer instead of being a third thing the reader has to trust.

### D.56.5 What P05 says now

On a build where the table is current — Server 2016, 2019, 2022:
...
```
   ...
   WDF requirement check : all 119 inventoried INF(s) are within the host.
```

On a build where measurement has moved past the table — Server 2025:

```
   WDF runtime on this host : KMDF 1.35 measured / UMDF runtime present, version not readable from any binary
       Measured KMDF is newer than the published 1.33 for Windows Server 2025. That is not an
       error - the measurement wins - but it means the published UMDF for this
       build is the same age, so UMDF requirements are left unjudged.
   NOT JUDGED : 8 INF(s) declare a UMDF requirement that was not compared.
```

The second output is the point of the design: it neither invents a number nor
pretends the question was answered.

### D.56.6 Collector

The collector probes `WUDFx02000.dll` and records it as
`Umdf2FrameworkLibrary` / `Umdf2RuntimePresent` — the library a UMDF 2 driver
binds to, as opposed to `WudfPf.sys`, `WUDFRd.sys` and `WUDFHost.exe`, which
are the platform driver, the reflector and the host process. Its assessment
gains `KmdfDocumentationComparison` and `DocumentedUmdfUsable`, carrying the
same rule so the evidence bundle and the sisters answer the question the same
way. Its KMDF derivation already used the numeric fields and is unchanged.
...
**Negative control**: the extended case was run against the previous tree and
failed with `function(s) not found: Get-BinaryVersionFact,
Get-WdfDocumentedBaseline`.

**Executed, not only parsed**: P05's notice was run through all three measured
builds — 2016 and 2019 where the documented UMDF is adopted, and 2025 where it
is refused — against the real 119-record inventory shape from the field run.

### D.56.8 Sister impact

Chipset r111 / Graphics r77 / NPU r54 / BthPan r59 / collector c9. Eight WDF
...
1. **Choose a reference script.** Use one of the **historically field-validated** scripts: `Deploy-AMDChipsetDriverOnWindowsServer.ps1`, `Deploy-AMDGraphicsDriverOnWindowsServer.ps1`, or `Deploy-MSBthPanInboxOnWindowsServer.ps1`. **Do NOT use the NPU script as a starting template** — the NPU script is classified as "🆘 Experimental / research-grade — NOT production-ready" (see README.md "Risk classification of the four scripts"), has no physical-hardware validation runs, and its idioms have not been verified to be safe to copy. Among the three historically field-validated scripts, the Chipset script has the largest test surface (Phase coverage, INF patch logic, multi-OS detection) and the MSBthPan script has the cleanest single-INF / single-HWID flow — pick whichever is closer to your new script's domain. Copy that file as your starting template.
