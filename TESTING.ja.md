...
| **BthPan (r1)** | ⏳ **予定** — ThinkPad + Intel AX210 + Windows Server 2025 build 26100.32860 が初回検証ターゲット (下記 §4 参照) | ❌ **未実行** | 新規スクリプト; 物理検証は計画段階。 ロジックは Chipset スクリプトの検証済 Phase / Secure Boot / WDAC フレームワークを共有 (Edit-InfForServer・ Get-OsContext・ Resolve-PhaseSelection 等は Chipset r57 から verbatim 継承)。 |
...
これらの回帰シナリオは、 チップセット r57 / グラフィックス r25 / NPU r7 (2026-05-17) で導入された 3 つの修正を検証します。 §1 (M75q Tiny Gen 2) で使用した WS2025 インストールを再イメージすることなく、 3 つすべてを同じ環境で実行できます。
...
| # | チェック | Pass 基準 |
|---|---|---|
| 1 | スクリプト起動時のバナーに `chipset-2026.05.17-r57` (または `graphics-2026.05.17-r25` / `npu-2026.05.17-r7`) と表示 | ✓ 正しいバージョン文字列 |
