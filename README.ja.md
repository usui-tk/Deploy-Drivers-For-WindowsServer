...
**最新リリース: `2026-05-24` — Chipset r74 / Graphics r40 / BthPan r22 / NPU r18 (変更なし)** (`legacy-ws2019-runtime-correctness-fix`)。
本リリースでは、 クリーンインストール済み WS2019 + Renoir 環境のベンチ実行で表面化した 4 つのランタイム不具合を修正しています:
(1) `Test-WhqlCoSignature` が存在しない `Find-Signtool` ヘルパを呼び出していた (正しいヘルパは `Find-KitTool 'signtool.exe'`);
(2) `signtool verify` が `/all` フラグなしで呼び出されており、 nested 形式の WHQL 署名を見逃していた;
(3) V06 が `-KnownOurInfSet` 引数の渡し漏れにより、 本スクリプトがインストールしたドライバを `[B]` (vendor) と誤分類していた (本来は `[C]` Self-Signed);
(4) I02 が新規に testsigning を ON にした場合でも、 再起動を促すメッセージを出力した直後に I03 / I04 を同一実行で続行していた。
詳細な事後分析は [SPEC §D.32](./SPEC.md#d32-runtime-correctness-fixes-from-the-2026-05-24-ws2019--renoir-bench-cycle-r74) (英語のみ) を、 回帰テストシナリオは [TESTING §16](./TESTING.md#16-r74--r40--r22-release-validation-2026-05-24-renoir--ws2019) (英語のみ) を参照してください。
...
`psa.py` (latest mainline) は `PSA1001`〜`PSA9002` の汎用ルールに加え、 プロジェクト・パイプライン規約ルール `PSAP0001`〜`PSAP0004` を含む **37 ルール体系** を 9 カテゴリに分けて実装しています。 37 ルールには `psa.py` v3.8.0 で Chipset r73 / Graphics r39 / BthPan r21 リリースとともに導入された `PSA2009` (PSCustomObject property assigned without prior declaration) が含まれます — これは Japanese-locale Windows Server 2019 で発生した `Chipset r72 P05 -> FAILED with "WhqlCoSignAnalysis" property-not-found exception` という実行時障害に対応する静的解析ゲートです。 ルールの詳細仕様については `SPEC.md` §A.11.5c を参照してください。 本レポジトリは latest mainline の `psa.py` に対して検証する方針です (特定バージョンへの固定はしません)。 方針の根拠と「新しい `psa.py` への追従」 LLM / AI ワークフローについては `SPEC.md` §A.11 *Version policy* を参照してください。 37 ルールは以下 9 カテゴリに分類されます:

| カテゴリ                                | コード範囲                | 例                                                                                                                                                                                                                                              |
| --------------------------------------- | ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 構文の整合性                            | `PSA1001`〜`PSA1003`      | 中括弧 / 丸括弧 / 角括弧のバランス                                                                                                                                                                                                              |
| 意味解析                                | `PSA2001`〜`PSA2009`      | 未定義変数、 自動変数の shadowing、 bare `$variable` に対する `-match`、 `$null` を `-eq`/`-ne` の右辺に置く問題、 条件式内の代入 / リダイレクト、 パラメーター名が自動変数を shadow している (`PSA2007`)、 `$Script:Foo++` の初期化忘れ (`PSA2008`)、 **v3.8.0 新規:** PSCustomObject プロパティを `[pscustomobject]@{...}` イニシャライザに宣言せずに代入している (`PSA2009`) |
