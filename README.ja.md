...
| 項目 | チップセットスクリプト (r57) | グラフィックススクリプト (r25) | **NPU スクリプト (r7)** | **BthPan スクリプト (r1)** |
| --- | --- | --- | --- | --- |
...
Chipset r58 / Graphics r26 / NPU r8 / BthPan r2 から、 4 つのスクリプトすべてに `-LogFile <path>` パラメータが追加され、 `Start-Transcript` / `Stop-Transcript` 経由でコンソール出力全体をファイルにキャプチャできます:
...
チップセット r57 / グラフィックス r25 / NPU r7 以降、 I02 で ENTER 入力待ちのまま hang する問題は解消され、 ja-JP Windows 上で日本語ログ出力 (特に CiTool.exe のステータス行) が正しくレンダリングされるようになった。
