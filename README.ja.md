...
**最新リリース: `2026-05-26` — Chipset r83 / Graphics r49 / BthPan r31 / NPU r27** (`npu-state-model-refactor-step-1-wdac-helpers`)。
本リリースは **NPU state-model refactor** の開始リリースです — NPU スクリプトを Chipset / Graphics / BthPan と同じ `$Ctx` ベースの状態受け渡しモデルに揃え、 最終的に 5 つの Tier B-4 helper (`Get-OrEnsureSecureBootBaseline`、 `Get-BootSigningEnvironment`、 `Show-BootSigningEnvironment`、 `Invoke-Cleanup`、 `Resume-CtxFromWorkspace`) を Tier A に昇格させる多段階ワークストリームです。 段階 1 は **準備リリース**であり、 必要となる WDAC tooling helper を Chipset から NPU に portage し、 canonical な `$Ctx` skeleton を構築 (現時点では未使用、 段階 2 の consumer のためのプレースホルダ) します。 **実行時動作の変更はなし** — 既存の `$Script:` グローバルが各 phase 関数と各 Tier B-4 helper の動作を従来通り駆動します。

主な変更点 (詳細は CHANGELOG を参照):

- **Tier A 関数数: 36 → 38**。 2 つの汎用 WDAC tooling preflight helper (`Test-WdacToolsAvailable`、 `Get-ActiveCodeIntegrityPolicies`) を Chipset / Graphics / BthPan から NPU に portage しました。 これにより 4 兄弟すべてで byte-identical となり Tier A に昇格 (PSA8001 で強制)。 Tier A 関数の内訳に「WDAC tooling preflight helpers (2)」family が新規追加されました。

- **3 つの AMD prefix WDAC helper も NPU に portage (Tier B-3)**。 `Get-AmdSuppPolicyMarkerPath`、 `Test-AmdWdacPolicyDeployed`、 `Uninstall-AmdWdacPolicy` を Chipset から NPU へ verbatim copy しました。 これにより AMD 系 3 スクリプト (Chipset / Graphics / NPU) で byte-identical となります。 BthPan は per-family isolation 原則に従って対応する MS prefix 版 (`Get-MsBthPanSuppPolicyMarkerPath` 等) を持つ設計のため、 これら 3 つは引き続き `psa8001_ignore_functions` に残ります (Tier A は 4 兄弟 byte-identity が前提であり、 helper-pair 設計はそれを満たせないため)。

- **NPU `$Ctx` skeleton を導入**。 NPU の `Invoke-MainEntryPoint` の冒頭で、 Chipset の canonical な 29 プロパティに加えて NPU 固有の拡張 (`AmdAccountUser`、 `NpuDriverPackage`、 `WdacBinPath` 等、 約 20 プロパティ) を含む `$Ctx` PSCustomObject を構築するようになりました。 本リリースでは `$Ctx` は **意図的に未消費** — 段階 2 で phase 関数が `$Ctx` を読むようになり、 段階 3 で Tier B-4 helper が `$Ctx` を引数として受け取るようになります。 段階 3 完了まで、 `$Ctx` と `$Script:` は同じ状態を並列に保持します。

- **SPEC.md §A.11.7 *Tier B-4* に stage 別進捗テーブルを新設**。 段階 1 は ✅ 完了とマーク、 段階 2 (phase 関数移行、 約 2 000 行) と段階 3 (Tier B-4 helper の Chipset canon 化、 `Invoke-Cleanup` 方向 = Chipset の marker-file 方式が canon) は計画中と明記、 関数レベルのスコープも段階別に整理されています。

- **`$Script:ScriptTag` を `npu-state-model-refactor-step-1-wdac-helpers` に更新** (4 スクリプトすべて統一)。 **4 スクリプトすべてが (やや厳しくなった) canonical `.psa.config.json` 下で `psa.py 4.1.0` を 0 / 0 / 0 / 0 でパスします**。

### 過去のリリース注記

**`2026-05-26` Chipset r82 / Graphics r48 / BthPan r30 / NPU r26** リリース (`psa-py-v410-shared-helper-canon-uplift`) は r81 で導入した shared helper canon workflow を一歩進めるリリースでした。 3 件の Tier B 関数を Chipset canon に揃え、 PSA8001 で強制される Tier A 関数数を 34 → 36 関数 に拡大しました。 また BthPan の `Invoke-InfVerifValidation` 時代の `$reasons.ToArray()` 防御形式を 4 兄弟すべてに一様に back-port し、 BthPan のみで guard されていた latent な PS 5.1 ja-JP `[pscustomobject]` キャストバグを解消しました。 同リリースで SPEC.md §A.11.7 は差異の種類別 (B-1 / B-2 / B-3 / B-4) に再編され、 **NPU state-model refactor backlog** (Tier B-4) が今回の r83 で開始される多段階ワークストリームとして文書化されました。 SPEC.md §D.35 (新規) では PS 5.1 ja-JP `[pscustomobject]@{ ... = @(List<T>) } ArgumentException` の完全な post-mortem が記録されています。
