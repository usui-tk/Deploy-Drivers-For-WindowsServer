...
4 つの PowerShell スクリプトは同じ 21 phase アーキテクチャ、 同じ自己署名モデル、 同じ WDAC 認可パスを共有します。 それぞれ別ワークスペース (`C:\AMD-Chipset-WS`、 `C:\AMD-Graphics-WS`、 `C:\AMD-NPU-WS`、 `C:\MSBthPan-WS`)、 別の自己署名証明書、 別の WDAC supplemental policy GUID を使用するため、 相互に干渉しません。
...
```
C:\AMD-Chipset-WS\         (または C:\AMD-Graphics-WS\・C:\AMD-NPU-WS\・C:\MSBthPan-WS\)
