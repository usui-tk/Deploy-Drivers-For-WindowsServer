...
**最新リリース: `2026-05-27` — Chipset r86 / Graphics r52 / BthPan r34 / NPU r30** (`cross-repo-shared-utility-canon-write-caution`)。
本リリースは、 当リポジトリの 4 つの AMD-family スクリプトと、 姉妹リポジトリ [`usui-tk/ai-generated-artifacts`](https://github.com/usui-tk/ai-generated-artifacts/tree/main/scripts/powershell/download-speakerdeck-oracle4engineer) の `Download-SpeakerDeck.ps1` スクリプトをまたがる **5-way の cross-repo shared utility canon** を確立します。 28 個の helper 関数が 5 スクリプトすべてで byte-identical となりました (リリース前は 5 個のみ 5-way 一致)。 双方向 best-of-both パスにより `Write-Warn2` (AMD canon) と `Write-Warn` (SpeakerDeck canon) は統一名 `Write-Caution` に rename され、 残っていた NPU 対 AMD-trio の 3 関数 (`Set-Tls12` / `Set-ConsoleUtf8` / `Assert-PowerShellCompatibility`) の divergence も解消されました。 **実行時動作の変更はなし**。

主な変更点 (詳細は CHANGELOG を参照):

- **5-way canon (AMD 4 兄弟 + SpeakerDeck) で 28 関数を共有**。 Identity / DebugTrace 基本関数、 logging 基本関数 (`Write-Ok` / `Write-Fail` / `Write-Skip` / `Write-Step` / `Write-Detail` / `Write-Caution`)、 phase バナー (`Write-PhaseHeader` / `Write-PhaseFooter`)、 環境準備 helper (`Set-Tls12` / `Set-ConsoleUtf8` / `Assert-PowerShellCompatibility` / `Show-PowerShellEnvironment`)、 DebugTrace framework 全体。 これらの helper への将来の変更は **cross-repo 伝播を評価する必要あり**。 SPEC §A.11.7 に canon リストと per-repo carve-out を文書化。

- **`Write-Warn2` (AMD) / `Write-Warn` (SpeakerDeck) → `Write-Caution`**。 どちらも PowerShell 組み込み `Write-Warning` cmdlet との衝突回避のための ad-hoc な命名でした。 統一名 `Write-Caution` は衝突なし、 他の 1 行 helper の noun-form 慣習にも合致、 `[!]` marker の semantics (「operator は注意を払う必要があるが処理は継続」) にも合致します。 AMD 4 スクリプトで 426 件 callsite を書き換え (Chipset 109、 Graphics 103、 NPU 119、 BthPan 95)。 SpeakerDeck 側 (47 件) は姉妹リポジトリの対応リリースで実施。

- **NPU の divergence 解消**。 `Set-Tls12` / `Set-ConsoleUtf8` / `Assert-PowerShellCompatibility` は historical に NPU が他の 3 兄弟と微妙に違うコードを持っていた 3 つの 4-way Tier A blocker でした。 これらすべてを byte-identical 化しました。

- **`Set-Tls12` の hybrid canon**。 新しい body は TLS 1.3 + 1.2 (modern host 向け) を best-effort で有効化し、 TLS 1.1 + 1.0 (very-old host 向け) への fallback を `try/catch` で追加します。 AMD canon の「modern TLS を優先」 と SpeakerDeck canon の「legacy host も容認」 の両方のスタンスを統合。 各 `[Net.SecurityProtocolType]` enum lookup を `try/catch` でラップし、 `Tls13` enum を持たない古い .NET runtime でも失敗しません。

- **関数ごとに best-of-both を選択**。 10 関数は SpeakerDeck canon から採用 (zero-padded `Format-Elapsed`、 充実した `Assert-PowerShellCompatibility` docstring、 `Format-DebugFailure` の新規 `ElapsedMs` + `PhaseId` フィールド等)、 9 関数は AMD canon から採用 (詳細な `Show-PowerShellEnvironment`、 AMD canon の DebugTrace framework 等)、 2 関数は hybrid merge (`Set-Tls12`、 `Write-PhaseFooter`)。 完全な per-function source map は CHANGELOG に記載。

- **`$Script:ScriptTag` を `cross-repo-shared-utility-canon-write-caution` に更新** (4 スクリプトすべて統一)。 **4 スクリプトすべてが `psa.py 4.1.0` を 0 / 0 / 0 / 0 でパス**します。
