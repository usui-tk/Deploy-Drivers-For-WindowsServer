...
**最新リリース: `2026-05-26` — Chipset r84 / Graphics r50 / BthPan r32 / NPU r28** (`npu-state-model-refactor-step-2-phase-functions-ctx`)。
本リリースは **NPU state-model refactor の段階 2** であり、 再構築作業の本体部分です。 段階 1 は必要となる WDAC tooling helper を Chipset から portage し、 NPU の `Invoke-MainEntryPoint` 内に canonical な `$Ctx` skeleton を構築しました。 段階 2 ではその `$Ctx` を **実際に消費する状態**にします: NPU のすべての phase 関数と、 すべての非 phase の state-touching helper (段階 3 で扱う 3 つの Tier B-4 helper を除く) が `$Ctx` を必須パラメータとして受け取り、 ランタイム状態を `$Script:Foo` ではなく `$Ctx.Foo` から読むようになりました。 **実行時動作の変更はなし** — 移行は厳密に機械的であり、 各 phase の動作は r83 と完全に同一です。

主な変更点 (詳細は CHANGELOG を参照):

- **27 関数で 184 件の機械的な `$Script:Foo` → `$Ctx.Foo` 置換**。 21 個の phase 関数 (`Invoke-PrepPhase00`〜`09`、 `Invoke-VerifyPhase01`〜`06`、 `Invoke-InstPhase00`〜`04`)、 4 つの非 phase 状態保持 helper (`Resolve-AmdNpuDriverUrl`、 `Invoke-AmdAccountAuthentication`、 `Show-RyzenAiSoftwareGuidance`、 `Show-RunSummary`)、 `Invoke-MainEntryPoint`、 そして phase dispatcher の `Invoke-PhaseRunner`。 3 つの Tier B-4 helper (`Get-OrEnsureSecureBootBaseline`、 `Resume-CtxFromWorkspace`、 `Invoke-Cleanup`) は段階 3 のために **意図的に未変更**です。

- **27 件の signature 変更** (既存 param block がない 23 関数では `param()` → `param([Parameter(Mandatory)] $Ctx)`; 既に param block を持つ 4 関数 — `Show-RunSummary`、 `Invoke-AmdAccountAuthentication`、 `Resolve-AmdNpuDriverUrl`、 `Invoke-PhaseRunner` — では `$Ctx` を**最初の引数**として挿入)。 phase dispatcher の `& $phase.Func` 呼び出し箇所では `-Ctx $Ctx` を渡すようになりました。

- **top-level での dual-carriage state は引き続き意図的に維持**。 NPU の L307..L353 にある top-level の `$Script:Foo = $Foo` 代入は本リリースでは維持されます。 これは 3 つの Tier B-4 helper が依然として `$Script:` グローバルを読むためであり、 `$Ctx` constructor がそれら `$Script:Foo` から読み込むため両 carriage は同じ値を保持します。 この二重保持は各段階の `psa.py 0/0/0` ベースラインを保つための代償であり、 段階 3 で単一の調整変更として一気に解消されます。

- **SPEC.md §A.11.7 *Tier B-4* の stage 進捗テーブルを更新**。 段階 2 は ✅ 完了とマークされ、 カテゴリ別の内訳 (21 phase 関数 + 4 非 phase helper + 1 dispatcher + 184 機械的置換 + 5 caller 修正) を文書化。 残る計画は段階 3 (Tier B-4 helper の Chipset canon 化、 `Invoke-Cleanup` 方向 = Chipset の marker-file 方式が canon) のみとなりました。

- **`$Script:ScriptTag` を `npu-state-model-refactor-step-2-phase-functions-ctx` に更新** (4 スクリプトすべて統一)。 **4 スクリプトすべてが `psa.py 4.1.0` を 0 / 0 / 0 / 0 でパス**します。 Tier A 関数数 (PSA8001 で強制) は 38 関数のまま (本リリースで Tier A の変更はなし)。
