...
**最新リリース: `2026-05-25` — Chipset r75 / Graphics r41 / BthPan r23 / NPU r19** (`legacy-ws2019-ps51-japp-correctness-fix`)。
本リリースでは、 r74 リリースが調査したのと同じ WS2019 ja-JP + Renoir ベンチホストに対する follow-up 診断で明らかになった 3 つの不具合を修正するとともに、 **r74 で記録した 4 件中 2 件の不具合の真因の診断を誠実に訂正**しています:
...
本リリースでは併せて新しい `psa.py` v3.9.0 ルール 2 つ (**PSA2010** — 未定義関数呼び出し検出; **PSA2011** — `Split-Path -LiteralPath -Parent` 検出) も導入されており、 これらは対応する不具合を静的解析時点で捕捉します。 4 スクリプトすべてが r75 ベースラインで `psa.py 3.9.0 --severity error` を 0 errors でパスします。
...
`psa.py` (latest mainline) は `PSA1001`〜`PSA9002` の汎用ルールに加え、 プロジェクト・パイプライン規約ルール `PSAP0001`〜`PSAP0004` を含む **45 ルール体系** を 9 カテゴリに分けて実装しています。 45 ルールには `psa.py` v3.8.0 で Chipset r73 / Graphics r39 / BthPan r21 リリースとともに導入された `PSA2009` (PSCustomObject property assigned without prior declaration) が含まれます — これは Japanese-locale Windows Server 2019 で発生した `Chipset r72 P05 -> FAILED with "WhqlCoSignAnalysis" property-not-found exception` という実行時障害に対応する静的解析ゲートです。 また `psa.py` v3.9.0 で Chipset r75 / Graphics r41 / BthPan r23 / NPU r19 リリースとともに導入された `PSA2010` (未定義関数呼び出し検出, error) と `PSA2011` (PS 5.1 ja-JP で `Split-Path -LiteralPath ... -Parent` が AmbiguousParameterSet を発生させる検出, error) も含まれており、 これらは r75 §D.33 Defect A (`Split-Path` binder バグ) と §D.32.2 の `Find-Signtool` typo 系統を静的解析時点で捕捉します。 3 ルールの詳細仕様については `SPEC.md` §A.11.5c / §A.11.5d / §A.11.5e を参照してください。 本レポジトリは latest mainline の `psa.py` に対して検証する方針です (特定バージョンへの固定はしません)。 方針の根拠と「新しい `psa.py` への追従」 LLM / AI ワークフローについては `SPEC.md` §A.11 *Version policy* を参照してください。 45 ルールは以下 9 カテゴリに分類されます:
