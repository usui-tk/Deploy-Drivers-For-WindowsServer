...
**最新リリース: `2026-05-26` — Chipset r81 / Graphics r47 / BthPan r29 / NPU r25** (`psa-py-v410-three-new-error-rules-baseline`)。
本リリースは `psa.py` 4.1.0 (3 つの新規 error severity / default-on 静的解析ルール `PSA1004` / `PSA2012` / `PSA2013` を追加した upstream の minor リリース) を採用します。 4 つの sister script はすべて、 新規 3 ルールと前リリースから引き継いだ strict-mode の `PSAP0005` を含む最新 mainline ルールセット全体に対して **0 / 0 / 0 / 0 ベースライン**を達成しました。 **実行時動作の変更はありません** — 静的解析カバレッジの uplift リリースです。

主な変更点 (詳細は CHANGELOG を参照):

- **`psa.py` 4.1.0 (上流) — 3 件の新規 error 重大度ルール**。 `PSA1004` は bare `(if/switch/foreach/while/...)` を式として使用するパターン (PowerShell がこれを `if` というコマンドの呼び出しとしてパースし実行時に失敗する) を検出。 `PSA2012` は `[Parameter(Mandatory)]` パラメータ N 個の関数を N 個未満の positional 引数で呼び出すパターン (PowerShell が対話的に不足値を要求し、 非対話セッションでスクリプトがハング) を検出。 `PSA2013` は `$Script:Foo` を読んでいるが代入がスクリプト全体に存在しないパターン (PowerShell が静かに `$null` として評価し、 typo バグを下流で表面化させる) を検出。 すべて default-on。 詳細は SPEC §A.11.5f を参照。

- **`$Script:ScriptTag` を `psa-py-v410-three-new-error-rules-baseline` に更新** (4 スクリプトすべて統一)。 **4 スクリプトすべてが canonical `.psa.config.json` 下で `psa.py 4.1.0 --severity error` を 0 errors / 0 warnings / 0 info でパスします**。

- **SPEC.md §A.11.7 (新規) — Shared helper canon と porting checklist**。 4 sister script 間の shared helper の canonical な「Chipset からコピーする」ワークフローを文書化 (Tier A: PSA8001 で byte-identity 強制中 / Tier B: 4 script 共有だが drift 許容中 / Tier C: driver family 固有 / Tier D: 意図的に per-script)。 NPU の「簡素化スクリプト」常設例外は文書化された backlog 項目として retire — NPU の 3 件の Tier B simplification は将来 Chipset canon との reconciliation 対象として明示化されました。

- **ルール数テキストの parameterise — `README.md` / `README.ja.md` / `SPEC.md` / `TESTING.md` / `CONTRIBUTING.md` 全体で `46` ルール参照をルール範囲・family 参照に置換**。 今後 upstream でルールが追加された際に、 ドキュメントツリー全体を同時更新する必要がなくなりました。 現行のルール数の canonical source は実行時の `RULES` registry (`psa.py --list-rules` で表示) と upstream の `psa.py` SPEC.md §4 です。
