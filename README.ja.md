...
**最新リリース: `2026-05-26` — Chipset r85 / Graphics r51 / BthPan r33 / NPU r29** (`npu-state-model-refactor-step-3-tier-b4-helper-canon`)。
本リリースは **NPU state-model refactor の段階 3 (最終段階)** です。 段階 1 は必要となる WDAC tooling helper を Chipset から portage し、 NPU の `Invoke-MainEntryPoint` 内に canonical な `$Ctx` skeleton を構築しました。 段階 2 ではその `$Ctx` を NPU のすべての phase 関数と helper にわたって live にしました。 段階 3 はワークストリームを完了させます: 5 つの Tier B-4 helper (`Get-OrEnsureSecureBootBaseline`、 `Get-BootSigningEnvironment`、 `Show-BootSigningEnvironment`、 `Invoke-Cleanup`、 `Resume-CtxFromWorkspace`) を Chipset canon の byte-identical コピーで置換し、 NPU の `$Ctx` schema を Chipset に完全に reconcile し、 すべての top-level `$Script:` 状態代入を削除します (identity / logging / instrumentation / `WdacPolicyGuid` の infra のみ script scope に残ります)。 **AMD Chipset / Graphics / BthPan 利用者にとっての実行時動作変更はなし**。 NPU スクリプトの cleanup 動作は cert-subject-CN による列挙から marker-file の参照へと変更されました (理由は SPEC §D.36.1 を参照)。

主な変更点 (詳細は CHANGELOG を参照):

- **Tier A 関数数が 38 → 39 に拡大**。 `Get-OrEnsureSecureBootBaseline` が `psa8001_ignore_functions` から削除され、 PSA8001 が 4-way byte-identity invariant を能動的に強制するようになりました。 残る 4 つの Tier B-4 helper は AMD-trio 3-way byte-identity (Chipset / Graphics / NPU) を確立しますが、 4-way Tier A には到達しません — 構造的な障壁 (Chipset/Graphics の 1 行コメント差異、 BthPan の実装差異、 ドライバ family ごとの cert filename) があるためです。 SPEC §A.11.7 future-work に文書化されています。

- **`Invoke-MainEntryPoint` の regression 修正**。 r84 の機械的置換で誤って関数 signature が `param([Parameter(Mandatory)] $Ctx)` に書き換えられていましたが、 caller は引数なしで呼び出します。 これは初回スクリプト実行時にパラメータ binding エラーで失敗するはずでした。 r85 で正しい `param()` signature に戻し、 body を script-level パラメータを直接読む形に書き直しました。 `psa.py` はこの regression クラスを現状検出できません (PSA2001 は「mandatory parameter を宣言しているが引数なしで呼ばれる」関数をモデル化していない)。

- **NPU `Invoke-Cleanup` が marker-file canon を採用**。 r85 以前の NPU body は `Cert:\LocalMachine\Root\` を列挙して cert subject CN でマッチしていましたが、 r85 では `$Ctx.Paths.Cert\AmdSuppPolicyId.txt` から `PolicyId` を読み、 `Uninstall-AmdWdacPolicy` を呼び出します。 marker-file 方式は 2 つの失敗モード (複数 deployment の曖昧性、 release 間の subject-CN drift) を排除します。 SPEC §D.36.1 が post-mortem を文書化しています。

- **NPU `$Ctx` schema を Chipset canon に reconcile**。 `$Ctx.Paths` は populated な入れ子 `[pscustomobject]` (`Paths.Cert`、 `Paths.Patched`、 `Paths.Markers`、 `Paths.Logs` 等) になりました。 `$Ctx.CertPfxPath` / `$Ctx.CertCerPath` / `$Ctx.CertThumbprint` も populate されるようになりました。 NPU 専用の `$Script:DetectedPlatform` hashtable は `$Ctx.DetectedPlatform` の下にたたまれました。

- **r84 の "Tier A 41-way" 過大記述を修正 (forward-looking)**。 r84 SPEC §A.11.7 stage 2 行と CHANGELOG エントリは、 段階 3 の予定成果を「PSA8001 then enforces 41-way Tier A」「38 → 43」と記述していました。 実際の成果は **38 → 39** です。 リポジトリポリシーに従い r84 の歴史的記述は verbatim 保持し、 SPEC §D.36.3 が helper ごとの構造的障壁の分析を伴う forward-looking な修正を担当します。

- **`$Script:ScriptTag` を `npu-state-model-refactor-step-3-tier-b4-helper-canon` に更新** (4 スクリプトすべて統一)。 **4 スクリプトすべてが `psa.py 4.1.0` を 0 / 0 / 0 / 0 でパス**します。
