...
| **`Deploy-WdacSinglePolicyFormatOnLegacyWindowsServer.ps1`** | **Windows Server 2019 / 2016 専用 WDAC 外部オーケストレータ。** WDAC Single Policy Format (SPF) ポリシーをビルド・デプロイ・追跡する独立スクリプト。 SPF はこれらレガシー Server OS で唯一利用可能な WDAC フォーマットです (WS2022+ で使う Multiple Policy Format は `CiTool.exe` を必要としますが、 WS2019/2016 にはこのツールが存在しません)。 4 つの driver script の I02 が WS2019/2016 (build < 20348) を検出すると自動的に呼び出されます。 8 つの Action: `GetStatus`、 `AddCert`、 `RemoveCert`、 `Verify`、 `Uninstall`、 `Repair`、 `ComputeCanonicalHash`、 `ComputeOwnCanonicalHash`。 | **r04 — WS2019 でパイロット検証済み。** r67 (r03 リリース) で追加。 WS2019 build 17763 + Ryzen 5 PRO 4650U (Renoir) + Secure Boot ON での実機検証は r04 (2026-05-23) で成功。 r03 では `Add-HistoryEntry` の `param()` ブロックに `$Script:` スコープ修飾子が誤って付与されており、 ホストが Foreign 状態のループに陥る不具合が発覚。 r04 で修正済み。 SPEC §A.13 「Reuse before invention」 の sister-script 規律に従い、 Chipset r66 を verbatim にシードして構築。 34 個の共有ヘルパーを Chipset r66 から byte-for-byte 継承 (うち 30 個は PSA8001 で cross-file 同期を強制、 残り 4 個の Secure Boot baseline 診断ヘルパーは `.psa.config.json` の `psa8001_ignore_functions` 対象だが verbatim 継承は維持)。 設計根拠と r01..r04 の検証履歴は SPEC §D.25、 テストケースは TESTING §11 を参照。 |
...
| OS | Build | I02 パス | 備考 |
|---|---|---|---|
| Windows Server 2025 | 26100 | Path A (MPF supplemental policy) | 主検証ターゲット。 |
| Windows Server 2022 | 20348 | Path A (MPF supplemental policy) | 検証済み。 |
| Windows Server 2019 | 17763 | **Path C (外部オーケストレータ経由のレガシー WDAC SPF)** | r67 で追加された新規パス。 WS2019 + Renoir + Secure Boot ON での実機パイロット検証は r04 (2026-05-23) で成功。 r03 では `Add-HistoryEntry` の `param()` ブロックに付与された `$Script:` スコープ修飾子が原因でホストが Foreign 状態にループする不具合があり、 r04 で修正済み。 SPEC §D.25 を参照。 |
| Windows Server 2016 | 14393 | **Path C (外部オーケストレータ経由のレガシー WDAC SPF)** | Path C はこの build もカバーします (r04 の修正がそのまま適用されます)。 物理 WS2016 ホストでの実機検証は待機中。 |
| Windows 10 / 11 (Workstation) | 任意 | PrepareVerify のみ | Install phase は自動的に block されます。 |

レガシー Server ホスト (WS2019 / WS2016) では I02 が自動的に
`Deploy-WdacSinglePolicyFormatOnLegacyWindowsServer.ps1` に委譲します。
WS2022+ で使用される `CiTool.exe` + Multiple Policy Format
インフラストラクチャは旧 Windows カーネルでは利用できないためです。
このオーケストレータは driver script と同一ディレクトリに配置するか
(本リポジトリに含まれます)、 もしくは `raw.githubusercontent.com`
への outbound 接続を確保することで driver script が fetch
できるようにしてください。 完全な設計根拠は
[SPEC §D.25](./SPEC.md) を参照してください。
