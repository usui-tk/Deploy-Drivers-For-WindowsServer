...
| `Deploy-MSBthPanInboxOnWindowsServer.ps1` | **Microsoft inbox Bluetooth PAN ドライバ (`bthpan.inf` / `bthpan.sys`) 有効化パイプライン。** ソース: ホスト自身の `C:\Windows\System32\DriverStore\FileRepository\bthpan.inf_amd64_*` ディレクトリ — **リモートダウンロード不要**。 単一 INF・ 単一 HWID (`BTH\MS_BTHPAN`)。 Phantom OK (bth.inf による代理マッチ) と真の解消 (Class=Net、 Service=BthPan) を Windows Server 上で明示的に区別します。 | **新規** (r1) — 初版リリース。 Phase / Secure Boot / WDAC フレームワークは AMD スクリプトと同一を verbatim 継承。 INF パッチ対象が 1 ファイル・ 1 HWID と非常に小さい。 ThinkPad + Intel AX210 + WS2025 build 26100.32860 が第一の物理検証ターゲット予定。 |
| `README.md` | 英語版ドキュメント。 |  |
| `README.ja.md` | 本ドキュメント (日本語版)。 |  |
| `SPEC.md` | 開発者向け仕様書 (スクリプト別詳細、 INF パース戦略、 WDAC policy 構造)。 |  |
| `SPEC.ja.md` | SPEC.md の日本語版。 |  |
| `TESTING.md` | 物理ハードウェアでの検証結果。 NPU スクリプトの極めて限定的な検証状況も記載。 |  |
| `TESTING.ja.md` | TESTING.md の日本語版。 |  |
| `CONTRIBUTING.md` | Issue・PR ガイドラインと regression test 手順。 |  |
| `LICENSE` | MIT License。 |  |

4 つの PowerShell スクリプトは同じ 21 phase アーキテクチャ、 同じ自己署名モデル、 同じ WDAC 認可パスを共有します。 それぞれ別ワークスペース (`C:\Temp\Workspace_AMD-Chipset`、 `C:\Temp\Workspace_AMD-Graphics`、 `C:\Temp\Workspace_AMD-NPU`、 `C:\Temp\Workspace_Microsoft-BthPan`)、 別の自己署名証明書、 別の WDAC supplemental policy GUID を使用するため、 相互に干渉しません。 Chipset r59 / Graphics r27 / NPU r9 / BthPan r9 から、 4 つのワークスペースはすべて `C:\Temp\Workspace_*` 配下に再配置されています (クラスタ管理および一括削除を容易化する目的)。 `C:\Temp` がない場合はスクリプトが自動作成します。

---

## 2026-05-18 リリースで追加された機能 (r60 / r28 / r10 / r10)

本リリースは **スクリプト間整合性の確保と psa.py 3.2.0 統合** を目的としたパスであり、 パイプラインの新規機能追加はありません。 既存機能は end-to-end で保持されています。 主な変更点:

- **AMDNpu ヘルパー関数の整合化**。 NPU スクリプト (r9 → r10) に、 BthPan r9 で確立されながら NPU には未移植だったヘルパーを追加 — `Write-Detail` (4 スペースインデントの継続行表示)、 `Assert-PowerShellCompatibility` (PowerShell 互換性チェックを `Show-PowerShellEnvironment` の表示から分離して hard-fail 化)、 ハッシュ一致版の `Show-PowerShellEnvironment` (Chipset / Graphics / MSBthPan と同じ 169 行の body)。 NPU 固有だった `Test-AdminPrivilege` および `Set-NetworkProtocol` は、 姉妹スクリプトに合わせて `Assert-Admin` および `Set-Tls12` に名称変更。 `Set-Tls12` は Chipset / Graphics / MSBthPan の正準 body を採用 (TLS 1.2 + TLS 1.3 が利用可能な場合、 **TLS 1.0 / 1.1 は RFC 8996 に基づき意図的に除外**)。 旧 AMDNpu body の TLS 1.0 / 1.1 有効化はセキュリティ上のリグレッションとして削除。

- **AMDNpu に NPU 専用 `Show-DriverInstallationOrderNotice` を追加**。 加えて、 簡易版の `Get-BootSigningEnvironment` / `Show-BootSigningEnvironment` stub (Secure Boot および testsigning のみを probe; 完全な WDAC 列挙は Chipset / Graphics / MSBthPan ファミリに残置) を実装。

- **psa.py 3.2.0 ベースライン**。 リポジトリ内すべての PowerShell スクリプトが `psa.py --config .psa.config.json` で **0 errors / 0 warnings / 0 info** をパス。 4 スクリプト同時にカノニカル静的解析ベースラインが完全にクリーンとなった最初のリビジョンです。 リポジトリには新たに `.psa.config.json` (下記 [Static analysis](#psapy--powershell-静的解析) 参照) が同梱されており、 プロジェクト・パイプラインルール (PSAP0001 phase 命名、 PSAP0002 スクリプト識別子の存在) を opt-in した状態で、 PSA8001 (ファイル間 function body drift) のスクリプト固有 phase 関数を除外する設定がなされています。

- **姉妹スクリプト整合性の自動強制**。 PSA8001 (psa.py 3.2.0 新規ルール) が 14 個の共有ヘルパー関数 (`Format-Elapsed`、 `_LogLine`、 `Write-Detail`、 `Start-DebugTrace` / `Stop-DebugTrace` / `Write-DebugFailureReport` ファミリ全体、 `Export-DebugTraceJson` 等) を能動的にガードします。 今後 4 スクリプト間でこれらに drift が生じる PR は静的解析ゲートで失敗します。

- **psa.py 3.2.0 偽陽性修正 (スクリプト側変更なし)**。 r59 / r27 時点で Chipset / Graphics に観測されていた `PSA1001` (ブレース不整合) および `PSA2001` (未定義変数) の偽陽性は、 PowerShell の `""` (ダブルクォート二重化) エスケープと `` `` `` (バックチック二重化) エスケープに関する psa.py 字句解析器のバグ、 および `$Script:` スコープ修飾子の参照誤判定が原因でした。 psa.py 3.2.0 で修正済み。 スクリプト側の修正は不要でした。

2026-05-17 リリース (r59 / r27 / r9 / r9) からの引継ぎ機能: Debug Trace Facility (SECTION 1b)、 `Resume-CtxFromWorkspace` rehydration、 `-LogFile` 自動再配置 (SECTION 0.25)、 PS 5.1 ja-JP `Split-Path -LiteralPath` バグの回避 (`[System.IO.Path]::GetDirectoryName()` への置換)。

## 2026-05-17 リリースで追加された機能 (r59 / r27 / r9 / r9)

4 つのスクリプトは 2026-05-17 で同期リリースされています。 リビジョン番号はスクリプトごとに独立しています (Chipset → r59、 Graphics → r27、 NPU → r9、 BthPan → r9) が、 実質的な変更内容は同じ「Debug Trace 追加と resume 対応」の一括移植です。 BthPan の r2 から r9 への進化で確立されたパターンを各姉妹スクリプトに移植しました。 主な変更点:

- **Debug Trace Facility (SECTION 1b、 各スクリプト約 882 行)**。 14 関数構成の汎用診断ヘルパー (`Start-DebugTrace` / `Set-DebugStep` / `Stop-DebugTrace` / `Format-DebugFailure` / `Write-DebugFailureReport` / ファイル出力 / phase 失敗時自動エクスポート / JSON snapshot)。 各 P/V/I phase 関数の呼び出し箇所に `Set-DebugStep` チェックポイントを配置 (Chipset・ Graphics は 92 箇所、 NPU は phase 関数本体が短いため 44 箇所)。 phase が失敗すると、 JSONL ストリーム + 失敗 step を明示した自己完結 JSON snapshot が `<WorkRoot>\logs\` 配下に出力されます。

- **`Resume-CtxFromWorkspace` rehydration ヘルパー**。 過去 run で準備済みの workspace に対して `-Action Verify` / `-Action Install -OnlyPhases I01` を再実行できる。 P02-P09 のリトライ不要。 Chipset・ Graphics・ BthPan は `$Ctx` PSCustomObject を再構築。 NPU は `$Script:` スコープ変数を param block 評価時に確定する設計のため、 軽量版 (既存 PFX / CER / patched INF を検出して 1 行 confirmation を出力するのみ) を採用。

- **`-LogFile` の自動再配置 (SECTION 0.25)**。 `-LogFile` で指定されたパスが workspace 外の場合、 `<script-dir>\` (失敗時は `%TEMP%\`) に自動再配置し、 `Start-Transcript` が実際にディスクに書き込めたことを read-back で検証 (silent failure 防止)。

- **PS 5.1 ja-JP `Split-Path` バグ修正**。 `Split-Path -LiteralPath $path -Parent` が Windows PowerShell 5.1 ja-JP 環境で `AmbiguousParameterSet` を発生させ、 長年 `-LogFile` の有効化を silent に妨げていました。 4 スクリプトすべて `[System.IO.Path]::GetDirectoryName()` に置き換え済み。

PSA 静的解析ベースラインは 4 スクリプトとも end-to-end で維持されています。 r9 リリースで新規 finding は発生していません。
...
| 項目 | チップセットスクリプト (r59) | グラフィックススクリプト (r27) | **NPU スクリプト (r9)** | **BthPan スクリプト (r9)** |
| --- | --- | --- | --- | --- |
| **成熟度** | 安定版、 複数の検証サイクル完了 | 安定版、 複数の検証サイクル完了 | **🆘 実験的 — 初版、 物理 NPU ハードウェアでの検証は未実施** | **新規 (r1)** — 初版リリース。 Phase / Secure Boot / WDAC フレームワークは検証済 verbatim 継承。 単一 INF surface が小さく、 1 セッションで物理検証が完結可能。 |
...
├── README.ja.md                                   本ドキュメント (日本語版)
├── TESTING.md                                     物理ハードウェアでの検証結果 (英語版)
├── TESTING.ja.md                                  物理ハードウェアでの検証結果 (日本語版)
...
| **2** | `-AmdAccountUser <email> -AmdAccountPassword <SecureString> -ForceAmdAccountAuth` | EULA 受諾フローを自動実行させたい場合。**❌ 2026-05-10 の検証で `account.amd.com` が JavaScript-driven SPA であることが確認されたため、デフォルト無効化済み。`-ForceAmdAccountAuth` で opt-in 可能 (現状の AMD ポータルでは失敗が想定されます)。** 詳細な検証レポートは TESTING.ja.md §3.6 を参照してください。 |
...
セクションバナーテーブル内の継続行 (PowerShell 環境ダンプ、 OS プロファイル、 Secure Boot ベースライン、 INF インベントリ行、 V05/V06/I00 サブブロック等) は `Write-Detail` ヘルパー経由で出力されます。 これはタイムスタンプとマーカー接頭辞を持たない 4 スペースインデント行で、 「すべての行にマーカーを付ける」 規約の唯一の許容例外です。 ログを読む operator は、 4 スペースインデント行を直前のマーカー行の従属的な継続行として扱ってください。 (チップセット r56 / グラフィックス r24 で導入。 SPEC §A.5 参照。)
...
Chipset r59 / Graphics r27 / NPU r9 / BthPan r9 から、 4 つのスクリプトすべてに `-LogFile <path>` パラメータが追加され、 `Start-Transcript` / `Stop-Transcript` 経由でコンソール出力全体をファイルにキャプチャできます:
...
**r57 / r25 / r6 より前のリビジョンのみ。** CiTool.exe を `--json` フラグなしで呼び出していたため、 console に「続行するには、 Enter キーを押してください」を表示して stdin 入力を待ち、 script が hang していました。 active console window で ENTER を押下すると進行を再開します。 これはチップセット r59 / グラフィックス r27 / NPU r9 で修正済みで、 すべての CiTool.exe 呼び出しに `--json` を付与しました。 これは Microsoft の CiTool 設計における正規の非対話モードフラグで、 ヘルプ出力にも「出力を json として書式設定し、 入力を抑制する」と明記されています。 スクリプトをアップグレードすれば hang は発生しなくなります。 根本原因の解析と検証は SPEC §D.16 を参照してください。

### "CiTool ログ行が '蜃ｦ逅・・謌仙粥縺励∪縺励◆' のような文字化けで表示される"

**r57 / r25 / r6 より前のリビジョンのみ。** これは「処理が成功しました」 という UTF-8 バイト列を cp932 (Shift-JIS) として解釈した結果の文字化けです。 CiTool.exe は stdout に UTF-8 を書きますが、 PowerShell はそれを ja-JP の規定値である `[Console]::OutputEncoding` (cp932) でデコードしてしまいました。 SPEC §A.5 / §D.5 では P00 での UTF-8 強制が規定されていましたが、 実装が抜けていました。 チップセット r59 / グラフィックス r27 / NPU r9 にて、 P00 内の `Set-ConsoleUtf8` で修正されています。 詳細は SPEC §D.16 を参照してください。

### "同一の Install 実行で I03 が '3 failed' と報告するが I04 では 'Failed: 0' になる"

**r57 / r25 より前のリビジョンのみ。** I03 の分類ロジックは pnputil `exit=259` (`ERROR_NO_MORE_ITEMS`) を failure として扱っていましたが、 I04 PostInstallVerification は実際のデバイス状態を読んで、 これらを `REBOOT_NEEDED` (同じ INF を別パスから呼び出した最初の install で binding が既に queue されている場合) または no-op (ドライバパッケージが driver store に既に存在) として正しく識別していました。 exit=259 のケースは、 通常は重複ソースの INF (例: `Chipset_Software\SMBus Driver\W11x64\SMBUSamd.inf` と `SMBus Driver\W11x64\SMBUSamd.inf` の両方を I03 が呼び出し、 2 回目で 259 が返る) によるものです。 チップセット r57 / グラフィックス r25 で修正済み: 新しい I03 サマリーは `ok` / `need reboot` / `no-op` / `failed` の 4 カテゴリを報告し、 exit=259 は新しい `no-op (already present)` ステータス (Write-Skip / DarkGray) にマップされます。 詳細は SPEC §D.17 を参照してください。
...
PSA8001 のファイル間解析を動作させるため、 4 つのスクリプトはすべて単一の `psa.py` 呼出しで渡す必要があります (1 ファイルだけ渡しても、 比較対象のピアが居ないため PSA8001 は何も emit しません)。 r60 / r28 / r10 / r10 時点でのベースラインは **0 errors / 0 warnings / 0 info**。
...
`*.md` ファイル全般 (`README.md`、 `README.ja.md`、 `TESTING.md`、 `TESTING.ja.md`、 `SPEC.md`、 `SPEC.ja.md`、 `CONTRIBUTING.md` を含む) は **UTF-8 without BOM** で **LF** 改行を使用 — GitHub ネイティブの Markdown レンダリング規約に合わせています。 `.gitattributes` のルール:
...
- [TESTING.md](./TESTING.md) — 物理ハードウェアでの検証結果および NPU スクリプトの極めて限定的な検証状況。
...
Chipset スクリプト r54 リビジョン以降、 P04 ExtractInstaller phase は AMD Chipset Software 8.x (8.02.18.557 以降) で採用された新しい 2 層構造のインストーラアーキテクチャをサポートする。 インストーラは NSIS shell の内側に InstallShield SFX を包んでおり、 7-Zip だけでは完全に展開できない。 r54 は `InstallShield /a + 再帰的 msiexec /a` 戦略を追加し、 このフォーマットを end-to-end で取り扱う。

新しい展開処理は OS バリアント別の INF カバレッジ診断を出力するため、 オペレータは host OS に応じた driver サブディレクトリが適切に展開されたことを確認できる:

| Host OS ファミリ | 優先 INF サブディレクトリ |
| --- | --- |
| Windows Server 2025 / 2022 (Windows 11 ベース) | `W11x64\` |
| Windows Server 2019 / 2016 (Windows 10 ベース) | `WTx64\` |

完全なアーキテクチャ (2 層構造の wrapper、 35 個の sub-MSI、 `pnputil` を介した AMD の実 driver 登録ロジック) については [SPEC.ja.md §B.1 "AMD 8.x インストーラアーキテクチャ (r54+)"](SPEC.ja.md#amd-8x-インストーラアーキテクチャ-r54) を参照。 この展開パスの回帰テストは [TESTING.ja.md §8 "r54+ — AMD Chipset Software 8.x 展開診断フォーマット"](TESTING.ja.md#8-r54--amd-chipset-software-8x-展開診断フォーマット) を参照。

---

## r57+ / r25+ / r7+ — CiTool 非対話モード + Console UTF-8 強制

チップセット r59 / グラフィックス r27 / NPU r9 以降、 I02 で ENTER 入力待ちのまま hang する問題は解消され、 ja-JP Windows 上で日本語ログ出力 (特に CiTool.exe のステータス行) が正しくレンダリングされるようになった。

### 変更点

| 項目 | r57 / r25 / r6 より前 (不具合あり) | r57 / r25 / r7 以降 (修正済み) |
| --- | --- | --- |
| CiTool.exe 呼び出し | `& CiTool.exe --update-policy <cip>` が「続行するには、 Enter キーを押してください」 で blocking (operator の ENTER 入力を 1 呼び出しあたり 60-75 秒待機) | `& CiTool.exe --update-policy <cip> --json` — `--json` flag が対話プロンプトを抑制する (Microsoft 公式の非対話モード) |
| ja-JP console エンコーディング | `[Console]::OutputEncoding` が cp932 のまま。 CiTool の UTF-8 stdout が文字化けする (`蜃ｦ逅・・謌仙粥縺励∪縺励◆`) | P00 で `Set-ConsoleUtf8` を呼び出し、 `[Console]::OutputEncoding`、 `[Console]::InputEncoding`、 `$OutputEncoding` をすべて UTF-8 に設定 |
| pnputil exit=259 (チップセット / グラフィックス) | I03 サマリーで `failed` に分類され、 I04 の `REBOOT_NEEDED` / no-op 認識と矛盾 | 新しい `no-op (already present)` ステータスを `Write-Skip [~]` 経由で表示。 I03 サマリーは `{ok} ok ({reboot} need reboot, {noop} no-op) / {failed} failed / {skipped} skipped` を報告 |
| I02 console 出力 | bare `Write-Host '    Activation method: ...'` (SPEC §A.5 違反。 r56 / r24 の Write-Detail sweep で漏れていた) | SPEC §A.5 準拠のため `Write-Detail` ヘルパーに移行 |

根本原因分析と検証手順は以下を参照:

- **SPEC §D.16** — CiTool.exe `--json` と Console UTF-8 強制 (operator がローカルで実行可能な検証コマンドを掲載)
- **SPEC §D.17** — pnputil exit=259 の再分類 (チップセットの重複ソース INF 挙動)
