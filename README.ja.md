...
**エビデンス層**: [`tools/amd-chipset-driver-research/`](./tools/amd-chipset-driver-research/) には読み取り専用の research toolkit とその accepted baseline (AMD chipset 26 releases・hardware-matched INF 674 行・INF ごとの WDF 宣言と SHA-256 provenance) が置かれています。 これは**エビデンスであってポリシーではありません**: デプロイパイプラインは実行時にこれを一切読みません。 パッケージ族に関する主張 (上記の WS2016 KMDF の議論や TESTING §44 など) が、 印象ではなく実測 baseline を引用できるようにするために存在します。 W11 以降は [`tools/inventory-reconciliation/`](./tools/inventory-reconciliation/) がこの baseline とデプロイ実行の `inf_inventory.csv` を突合し、 parity exit criterion を機械判定します (TESTING §46)。

---

## 新着情報

**最新リリース: `2026-08-22` — 調査ツールキット同期リリース `amd-driver-research-v3.0.0`** (製品スクリプトの変更なし・版数バンプなし): `tools/` 配下の 3 つの AMD 調査ツールキットを **3.0.0** へ同期昇格する。昇格は Claude–ChatGPT の 2 サイクル・リリースレビュー統治 — Cycle A (独立初回レビューと是正、レビュー版 68 でクローズ) と Cycle B (実機ゲートに対する v3.0.0 昇格、版 80 でクローズ) — を経ており、すべての受理判定はレビューバンドル内にハッシュ連鎖で保存されている。Windows Client 再生成ゲートは 3 ツールキットすべての公開面で受理された (Chipset のフルラン Gate 2C、NPU の Gate 2N は短縮 2/2 に加えフルラン **15/15**・終了コード 0 でローカル `DEV_17F0` PnP 証跡から `376` パッケージを自動選択、Graphics の bounded Gate 2G は **10/10**・終了コード 0)。Windows Server 2025 の bounded `PathSafety,Test` スモークも 3 本すべてで 2/2 合格し、NPU は是正済みの 37 関数アーキテクチャ収束契約 (不一致 0) 上で合格した。GitHub への適用は**ディレクトリミラーではなく明示パッチセット**である: レビュー済み操作マニフェストの下で 177 上書き + 45 追加 + 削除 0、追跡 `tools/**` は 330 から **375** ファイルへ。リポジトリ所有の dotfile、Chipset のランタイム staging、`inventory-reconciliation/`、`source-fragments/` は非接触のまま保持した。3 本の調査スクリプトはすべて `#requires -Version 5.1` を固定し、再生成された **166 関数**のソース同一共通コアを各ツールキットの `data/` にバイト同一で共有する。データセットの要点: chipset baseline は **26 releases / hardware-matched INF 674 行** へ拡大 (`8.08.12.551` を追加)、graphics はリリース `26.8.1` と署名解析面を追加、NPU カタログは検証済み `280` 最小 / 歴史的 `RAI1.5-280` (回帰専用) / production `376` の成果物集合とスキーマ検証済み適用可否で 3.0.0 再生成された。SPEC **A.11.4b/A.11.4c** は tools 層のアナライザ基線を更新し、error 級 8 件を登録した (AST で反証済みの PSA2011 誤検出 5 件、Windows 専用エラー分岐内の PSA2010 `Write-AmdWarn` 呼出 2 件 — 実在するが全資格取得ランで非到達・上流へ報告済み — および PSA2001 のスコープ誤検出 1 件)。4 本の配備スクリプトとコレクタは非接触 — Chipset r122 / Graphics r87 / NPU r60 / BthPan r68 / Collector c11 — スイートも **28 ケース / 1225 アサーション** で変更なし。

- **`2026-08-14` — 新規調査ツールキット `amd-npu-research-v1.0.0`** (製品スクリプトの変更なし・版数バンプなし): `tools/amd-npu-driver-research/` をツールキット **1.0.0** として追加 — 調査スクリプト、その文書、JSON スキーマ 24 本、厳選されたソースデータ、ハードウェア識別証跡コレクタ、および **`public/**`** 配下の完全な生成データセット (合計 79 ファイル)。本ツールキットは、CPU 名から URL への対応表ではなく、Windows Server 向け AMD Ryzen AI NPU ドライバの**厳密な SKU 単位の適用可否モデル**を機械可読形式で構築する。厳密な CPU SKU を、コードネーム・検証済み NPU 識別子・ドライバリリースの能力・AMD 公式のサポート証跡を経由して、公開入手可能な成果物または `ReviewRequired` へ解決する。AMD インストーラを実行せず、ベンダペイロードも変更しない。データセットは **112 の厳密なプロセッサ SKU**、**336** の互換性マトリクス行、**112** の適用可否行を網羅し、3 つの推奨判定が 112 プロセッサを過不足なく分割する — **62** が最新の公開静的候補を選択、**22** が NPU ドライバ不要、**28** が `ReviewRequired`。最後の区分は機能の未完成ではなく、証跡が不十分な組合せを**意図的に fail-closed** とする結果である。公開モデルは先行 2 本と同一: コミット対象の生成面は `public/**` のみ、ランタイム作業域は非追跡、生成 JSON と Markdown は BOM なし UTF-8 ・ LF、`publication-manifest.json` が全 **22** ペイロードのパス・長さ・ SHA-256 を記録し、全件 `HandEdited: false` でソーススクリプトのハッシュに束縛される。ここでコミットするスクリプトそのものが Windows PowerShell 5.1 と Linux 上の PowerShell 7.6.4 で **13/13** ステージを終了コード 0 で完了し、Windows 実行はベンダからの新規自動再取得を実施、両ランタイムの public 面は **バイト同一** (23/23)。**Windows Server での NPU ランタイムサポートは一切主張しない** — 検証した INF セレクタの下限と `ProductType` サーバ除外記述の不在は静的候補の推論を支えるのみで、Ryzen AI Z2 Extreme の証跡は Windows **クライアント**ランタイムのみである。`.gitattributes` はチップセット・グラフィックスと同様、本ツールキットの `public/**` を verbatim で保存する。4 本の展開スクリプトとコレクタは非接触 — Chipset r122 / Graphics r87 / NPU r60 / BthPan r68 / Collector c11 — スイートも **28 ケース / 1225 アサーション** で変更なし。

- **`2026-08-13` — 新規調査ツールキット `amd-graphics-research-v1.0.0`** (製品スクリプトの変更なし・版数バンプなし): `tools/amd-graphics-driver-research/` をツールキット **1.0.0** として追加する。調査スクリプト・付属文書・29 本の JSON スキーマ・キュレート済みシード/回帰データ・18 本の著作レポート、そして **`public/**`** 配下の生成データセット一式で、計 135 ファイル。本ツールキットは AMD グラフィックスドライバの製品とリリースを探索し、実物のベンダー成果物を取得・展開して、その INF 内容から Windows Server 適用性を解析する。調査用ツールであり、配備パイプラインの一部ではない。データセットは **21 製品グループ** にまたがる **23 件の選定成果物** を対象とし、**482 件の INF ドライバレコード** と、3 種の互換性マトリクスそれぞれに **1,928 行** の Windows Server 適用性行を持つ。成果物単位の件数は集約インデックスと全キーで一致する。公開模型はチップセット版と同一で、`public/**` が唯一のコミット対象生成面、ランタイム作業域は非追跡、生成 JSON/CSV はバイト単位でそのまま公開し、生成 Markdown は UTF-8 (BOM なし)・LF に正規化する。`publication-manifest.json` は 64 件のペイロード全件について サイズ・SHA-256・生成様式・由来ソースハッシュ を記録し、全エントリが `HandEdited: false`。**29 本のスキーマ** はすべてコンパイルでき、公開 JSON **35 件**を検証エラー 0 で検証する — 併置されたスキーマでデータセットを検証できる。`.gitattributes` はチップセット版と同様、本ツールキットの `public/**` を verbatim 格納する。4 本の配備スクリプトとコレクタは非接触 — Chipset r122 / Graphics r87 / NPU r60 / BthPan r68 / Collector c11 — スイートは **28 ケース / 1225 アサーション** で変化なし。

- **`2026-08-12` — 調査ツールキット `amd-chipset-research-v2.0.0`** (製品スクリプトの変更なし・版数バンプなし): `tools/amd-chipset-driver-research/` をツールキット **2.0.0** へ更新し、生成された調査データセット一式を初めて公開する。1.0.0 世代の `inventory/accepted-baseline/` スナップショット模型は退役し、新設の `PUBLICATION-POLICY.md` が定義する単一の許可リスト領域 **`public/**`** に一本化した。ランタイム作業域 (`inventory/`・`reports/`・`work/`・`private/evidence/`) は非追跡のまま。データセットは **65 ファイル** — AMD チップセットリリース `2.04.04.111` 〜 `8.07.16.1035` を網羅する 25 本のリリース別 正準 Raw JSON 解析、集約インデックス、28 本のレポート、そして 64 件のペイロード全件について サイズ・SHA-256・生成様式・由来ソースハッシュ を記録する `publication-manifest.json` から成る (全エントリが `HandEdited: false` — 生成値が誤っていれば生成器を直して再実行するのであって、手で修正することはない)。**643** 件の INF パッケージレコードは公開 inventory CSV とキー集合が厳密一致し、MSI 宣言的解析は **25/25** が読取専用で解析され **13,993** 行を採取して all-null 行はゼロ、Windows Server 適用性 (WS2016/2019/2022/2025) の **700** 個の数値は公開 Raw JSON のみから再計算できる。ベンダー証跡はバイト忠実である: パス可搬化は **フィールド単位** で適用され、`/SETFILTERUSB`・`/SETRYZENPPKG`・`/SETPCI`・`/info.xml`・`/DevID.xml` といった AMD セレクタ／MSI／埋込 XML のトークンは ベンダーバイナリ内の表記そのままで記録され、実行ホストのパスのみが正規化される。集約インデックスではさらに Windows PowerShell 5.1 の `{ value, Count }` コレクションラッパーを素の配列へ正準化する — 認識条件は `value` が列挙可能で長さが `Count` に一致すること、したがって同名の 2 プロパティを持つだけのドメインオブジェクトは対象外 — ラッパーが残存した場合は公開が fail-closed で停止する。リリース別 正準 Raw JSON はこの射影で書き換えられない。ゲート **G-20** は退役したベースライン CSV から `public/inventory/amd-chipset-driver-inventory.csv` (643 行・同一スキーマ) へ付け替えた。`inventory-reconciliation` ツールは inventory パスをパラメータで受け取るため、コード変更は不要だった。4 本の配備スクリプトとコレクタは非接触 — Chipset r122 / Graphics r87 / NPU r60 / BthPan r68 / Collector c11 — スイートは **28 ケース / 1225 アサーション** で変化なし。

- **`2026-08-11` — Chipset r122 / Graphics r87** (`immutable-deployment-plan`): 監査是正の第 14 ウェーブ (W14) として **R5-M03** (feedback R5 / 5B-5) をクローズしました。 P06 は `-SkipNonCosignedDrivers` trim の後・**最初の INF mutation の前**に正準 **`deployment-plan.json`** を固定します — 監査の logical contract `SourceArtifact -> PackageInventory -> DeploymentPlan -> PatchedArtifacts` の実体化です。 各行は監査のフィールド集合を運びます: `Inf` / `SelectedSourceInfPath` / `SelectedSourceInfSha256` (content identity)、W13 rich detail 由来の `Devices[]`、`SourceVariant`、`SelectionReason`、`ExpectedMutation`、型付き観測を伴う `WdfDecision`、`PackageCompletenessDecision` (chipset は phantom 参照評価 — research の shortcut より優位と監査が評価した機構 — を報告し、graphics は正直に `Evaluated=false` を報告)、`KernelTrustObservation` (P05 WHQL 分析由来の `CoSigned`/`NotCoSigned`/`NotAnalysed` — **観測であり、決して loadability claim ではありません**)、そして per-selection の **`PayloadFiles[]` SHA-256** (W13 Q-W13-1 deferral の返済: chipset は `Get-InfReferencedFile` で `[SourceDisksFiles*]` を列挙し、graphics は sibling copy が実際に運ぶ source フォルダの実体をハッシュ化)。 P06 の **patch/copy ループは plan 行を反復**します — 「P06 は plan 外の INF を patch しない」が構造になり、かつ実測により挙動保存です (`NeedsPatch` は P05 時点で既に `VariantSelected` かつ `EligibleForCatalog` を内包するため、plan の patch 行集合は旧フィルタと同一)。 縮退した co-sign trim は**空の plan** を書きます — planned-empty は never-planned と区別可能です。 P06 marker は **schemaVersion 2** へ移行します: 入力 `{ InventorySha256, SkipNonCosignedDrivers, DeploymentPlanSchemaVersion }`、出力 `{ DeploymentPlanSha256, PatchedCount, CopiedCount, DegeneratePlan, PatchedInfManifestSha256 }`。ヒットはさらにディスク上の plan と patched INF 層を再ハッシュします — 5B-5 の **resume drift 保護**: 再開実行が別の package 選択に対して沈黙のまま進むことはもうできません。 **`deployment-plan-execution.json`** (fail-open) は plan SHA に紐づく per-row の `Outcome` + `PatchedInfSha256` を記録します — `installed -> patched INF SHA -> source INF SHA` の逆引きです。 P08 と I03 は **report-only** の alignment 照合を実行し、`patched/` 配下の plan 外 INF (sibling copy による輸送) を名指しします。install 挙動は不変で、enforcement は static-first ウェーブへ意図的に deferred です。 権威 extraction が実行型である間、container 層は plan から意図的に不在です (shadow graph は fail-open・非権威) — 記録済みのスコープ縮退です。 新ゲート **G-23**。スイート実測は **28 ケース / 1225 assertions**。 NPU r60 / BthPan r68 / Collector c11 は不変です (BthPan は裁定 Q-W14-5 により文書化された out-of-scope: 単一 package の universe で、provenance は W13 の集合 fingerprint により既に連鎖済み)。

- **`2026-08-10` — Chipset r121 / Graphics r86 / BthPan r68** (`content-addressed-cache`): 監査是正の第 13 ウェーブ (W13) として **R5-H03** と **R5-H06** をクローズし、**R5-M01** の型付き WDF 観測を実装しました。 phase marker は **schemaVersion 2** へ移行します: 共有の純関数 (`Get-PhaseFingerprintHash` — ordinal キー順・正規 `key=value` 行・小文字 SHA-256。I/O なしで fixture テスト可能) が各フェーズの宣言済み**入力/出力フィールド集合**を fingerprint 化し、`Test-PhaseMarker -ExpectedInputFields` が marker を content-addressed なキャッシュエントリに変えます — schema なしの旧形式 marker は**設計上 miss**、`-Force` は常に miss、そしてすべての miss はどちら側が乖離したかを名指しします。 P03 は `LastWriteTime` 最新のダウンロードをもう信用しません: キャッシュヒットは記録された正確なファイルを再ハッシュし、記録済み SHA-256 と一致しなければなりません (repack・同名すり替え・stale 混在のシナリオはここで死にます)。要求 URL は**記録される identity であり、再検証トリガーではありません** — 上流の新リリースはバイト同一なローカル成果物を無効化しません (5B-3 の字義からの申告済み逸脱・次回監査へ明示)。 P04 は `{ ArtifactSha256, ExtractionSchemaVersion }` で fingerprint し、**INF 層マニフェスト** (展開済み全 `*.inf` のソート済み relpath+SHA) を出力として記録します。ヒットには入力の一致と再計算マニフェストの一致の両方が必要です。 P05 はマニフェストを連鎖し、新しい正準 **`package-inventory.json`** を書き出します — 各レコードは歴史的フィールドを保持したまま `SourceArtifactSha256` / `InfSha256` / `InspectionStatus` / 型付き `Wdf.KMDF|UMDF` 証跡 (`{ Status, Versions[], Evidence[] }`・1 始まり行番号・5 値語彙・**Unknown は決して Compatible にならない**) を獲得し、読めない INF はフェーズを中断させず `ParseFailed` レコードになります。 CSV/TXT レポートは単一の property 保存 projection を通じた正準 JSON の**派生ビュー**へ降格し、すべての rehydration サイト (P05 キャッシュヒット・P06 の必須ロード・chipset の best-effort ローダ 3 箇所) は **JSON 優先**になります — 旧 CSV round-trip が沈黙のうちに落としていた rich detail が復元されます。旧 CSV は pre-W13 ワークスペース向けの読み取り専用フォールバックとして残ります。 MSBthPan はダウンロードのない universe に同じ契約を適用し、DriverStore の source/copied **集合 fingerprint** を使います。 W12 の shadow graph は意図的にすべての fingerprint の外に置かれます (fail-open な証跡がキャッシュ miss を駆動してはならない)。 新ゲート **G-18 / G-19 / G-21**。marker 5 関数は 3-way sister identity リストに加わります。スイート実測は **27 ケース / 1151 assertions**。 NPU r60 / Collector c11 は不変です (NPU に marker 機構はありません — 推定でなく実測)。

- **`2026-08-10` — Chipset r120** (`static-extractor-shadow-port`): 監査是正の第 12 ウェーブ (W12) として静的抽出への移行 (v5 R5-H01 手順 1〜3、R5-H02) を開始しました。 research toolkit の**静的 `ISSetupStream` decoder** (C#・ISx 参考・MIT — 帰属表示はブロック内に保持し、新設のルート [`THIRD-PARTY-NOTICES.md`](./THIRD-PARTY-NOTICES.md) に記録) を、新しい **source-fragment 機構**で chipset スクリプトへ逐語移植します: `tools/source-fragments/AmdStaticExtraction.fragment.ps1` が単一の正本で、 スクリプトはマーカー付き payload をバイト同一で埋め込み、ゲート **G-17** が同一性を pin します。 decoder の上に新規著作した shadow 機構: キューベースの有界再帰 orchestrator (SHA-256 重複排除・MaxDepth・probe 優先の EXE 振り分けと 7-Zip フォールバック・MSI OLE/CFBF マジック検証)、**読み取り専用** Windows Installer COM File table リーダー (OpenDatabase mode 0 — msiexec なし・プロセス起動なし)、そしてサフィックス版数 `name.infN` 慣行のための純関数 CAB エントリ名リゾルバ — この層の欠如が、119 行のフィールド実測 universe に対する 歴史的な 31 INF 盲点を生みました。 **P04 は現行抽出の後にこの shadow を常時実行 (fail-open)** します: 実行型戦略のツリーが下流全フェーズの正のままで、deployment 判断は shadow 出力を一切読みません。証跡は `<WorkRoot>\manifests\extraction-graph.json` に書き出されます (SchemaVersion 1.0・監査指定 13 フィールド形の per-container レコード・解決済み実名と variant 番号を持つ `Infs[]`・shadow INF 数を現行ツリーと比較する `ParityNote`)。 `<WorkRoot>\shadow-extracted\` 配下の shadow ツリーは実行毎にクリーンし実行後は保持します (ディスクコストは概ね展開済みインストーラ 1 個分)。 新ゲート: **G-17** (同一性・構造・禁止能力・decoder/リゾルバ/orchestrator の実測挙動・report-only の research 一致確認)、**G-22** (ISx 帰属表示がコードと共に移動)、**G-16 shadow スコープ** (shadow がプロセス起動・msiexec・実行型抽出戦略へ到達しないことの AST 証明)。 スイート実測は **24 ケース / 986 assertions**。 Graphics へはまだ shadow を載せていません — chipset の形が定まった後の **W12G** として意図的に deferred しています。 Graphics r85 / NPU r60 / BthPan r67 / Collector c11 は不変です。
...
| `-PfxPassword`             | スクリプト別         | 自己署名 PFX のパスワード。 4 姉妹全て: **未指定時は実行ごとのランダム 32 文字 CSPRNG** (W4。 NPU は W7 で参加 — 監査 H-05R クローズ) |
...
- **保管場所**: PFX を `C:\AMD-{Chipset,Graphics,NPU}-WS\cert\` に保存。 4 姉妹全てで `-PfxPassword` 未指定時に**実行ごとのランダム 32 文字 CSPRNG パスワード**が使われ (W4。 NPU は W7 以降)、 エクスポートされた PFX は Administrators+SYSTEM に ACL 限定・**Install 完了後に削除**されます (証明書はストアに残り、 P07 が必要時に再生成)。 W7 以降は NPU スクリプトも同一契約です (監査 H-05R クローズ)。 NPU には phase-marker キャッシュが無いため、 P07 は署名実行のたびに証明書を再生成し、 削除は post-Install ブロックで行われます。 本文書の以前の版は「PFX はパスワード保護されていない」と記述しており、 コードと矛盾していました。 ここに是正します (監査 L-01)。
