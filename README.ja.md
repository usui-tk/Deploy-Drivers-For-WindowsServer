...
**最新リリース: `2026-05-26` — Chipset r82 / Graphics r48 / BthPan r30 / NPU r26** (`psa-py-v410-shared-helper-canon-uplift`)。
本リリースは r81 で導入した shared helper canon workflow を一歩進めるリリースです。 3 件の Tier B 関数を Chipset canon に揃え、 PSA8001 で強制される Tier A 関数数を 34 → **36 関数** に拡大しました。 また BthPan の `Invoke-InfVerifValidation` 時代の `$reasons.ToArray()` 防御形式を 4 兄弟すべてに一様に back-port し、 BthPan のみで guard されていた latent な PS 5.1 ja-JP `[pscustomobject]` キャストバグを解消しました。 **en-US ホスト・ PowerShell 7.x 上では実行時動作の変更はなし** — 防御的な一貫性向上リリースです。

主な変更点 (詳細は CHANGELOG を参照):

- **Tier A 関数数: 34 → 36**。 `Get-SecureBootBaselineSnapshot` と `Show-SecureBootBaselineSnapshot` が Tier B → Tier A に昇格 (PSA8001 で 4 兄弟 byte-identity 強制)。 昇格には cosmetic な差分の解消 (NPU の `Write-Host`+4 スペースインデント直書きを正典の `Write-Detail` ヘルパー呼び出しに置換) と、 PS 5.1 ja-JP `[pscustomobject]` latent bug guard の一様適用が必要でした。

- **PS 5.1 ja-JP latent bug を一様に guard**。 4 兄弟すべての `Get-SecureBootBaselineSnapshot` で、 `Reasons = @($reasons)` を `Reasons = $reasons.ToArray()` に置換 (BthPan の従来形と統一)。 統一された 8 行のコメントブロックが新規 SPEC §D.35 post-mortem を参照するようになり、 将来のメンテナーが背景を即座に理解できます。 en-US ホスト・ PS 7.x 上では no-op、 PS 5.1 ja-JP 上では latent ArgumentException クラスを排除します。

- **SPEC.md §A.11.7 再編**。 Tier B の分類は今後、 関数 family ではなく **差異の種類** で B-1 / B-2 / B-3 / B-4 に細分化されます。 B-1 (cosmetic、 r82 後は空)、 B-2 (PS 5.1 ja-JP latent bug guard、 r82 後は空)、 B-3 (per-family identifier substitution、 実質 Tier C — `Resume-CtxFromWorkspace`、 `Invoke-Cleanup`)、 そして **B-4 (NPU state-model アーキテクチャ差異 — 専用の future workstream)**。 5 件の NPU 残存差異は数千行規模の `$Script:` → `$Ctx` リファクタリングを要するため、 個別の future refactor PR として追跡します。

- **SPEC.md §D.35 (新規) — `PS 5.1 ja-JP [pscustomobject]@{ ... = @(List<T>) } ArgumentException`**。 元の `Invoke-InfVerifValidation` 不具合の localisation 過程、 一様 fix の動機となった latent risk、 そして一般コーディングルール (「`[pscustomobject]` に `Generic.List[T]` を値として渡すときは `@($list)` ではなく `$list.ToArray()` を使う」) の完全な post-mortem。

- **`$Script:ScriptTag` を `psa-py-v410-shared-helper-canon-uplift` に更新** (4 スクリプトすべて統一)。 **4 スクリプトすべてが (より厳しくなった) canonical `.psa.config.json` 下で `psa.py 4.1.0` を 0 / 0 / 0 / 0 でパスします**。
