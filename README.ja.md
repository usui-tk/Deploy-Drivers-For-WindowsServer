...
**最新リリース: `2026-08-08` — Chipset r96 / Graphics r62 / NPU r39 (変更なし) / BthPan r44** (`path-a-plan-semantics-fixes`): `-SkipNonCosignedDrivers` (Path A、 Secure Boot ON、 WS2019) の初の実地実行は、 何もインストールする前に失敗しました — P06 のトリムが実インベントリのスキーマでクラッシュし、 クラッシュがなくても適格性ルールが 119 INF のプランを 0 に空にする欠陥を抱えており、 さらに WHQL 分析が `PrepareVerify` -> `Install` のプロセス境界を越えられず、 I01 が PFX 不在で中断していました。 r96 でトリムをスキーマ耐性化し、 意味論を是正し (ベンダーカタログのままコピーされる INF は常に適格)、 分析とトリム後プランをワークスペース JSON (`whql_cosign_analysis.json` / `whql_cosign_plan.json`) として永続化し、 プランに再署名カタログが含まれない場合は I01 自体をスキップして信頼ストアを無変更のまま維持します。 ポストモーテム: SPEC D.40。 契約: SPEC D.31.17。
