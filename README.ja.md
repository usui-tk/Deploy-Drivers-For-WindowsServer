...
**最新リリース: `2026-05-24` — Chipset r80 / Graphics r46 / BthPan r28 / NPU r24** (`psa-py-v4-llm-governance-strict`)。
本リリースは r76 / r42 / r24 / r20 で開始した **LLM ガバナンスマイグレーションを完了**するリリースです。 4 つの sister script すべてが、 `psa.py` 4.0.2 を **strict モード** (`psap0005_relaxed_mode` を `.psa.config.json` から削除) で実行して、 アナライザの 46 ルール体系全体に対して **0 / 0 / 0 / 0 ベースライン**を達成しました。

主な変更点 (実行時動作の変更なし、 完全な per-rewrite 一覧は CHANGELOG を参照):

- **`psa.py` 4.0.2 (上流) — `PSAP0005` relaxed-mode カバレッジ拡張**。 r76 ベースラインで実際に観測された prose パターン (`(rNN / SPEC D.YY)`、 `(Q-X1, rNN)`、 `# rNN (graphics):`、 `predates rNN`、 `r71 adds ...` 等) を扱うために、 9 つの新規 exempt patterns (E1-E9) と comment-block-level exempt heuristic が上流に追加されました。 新アナライザの下で、 4 スクリプトの relaxed-mode 件数は 64 から 1 に減少しました。

- **99 件の strict-mode-eligible な `rNN` 参照を rewrite**。 1 つの統合チェンジセットで 4 スクリプトを一括 rewrite し、 歴史的な "added in rNN" / "before rNN" / "rNN (graphics)" 等の prose を、 設計根拠は `SPEC.md` Part D への相互参照に移しつつ **timeless wording** へ変更しました。 PSA8001 cross-script-shared helpers の byte-identity は維持されています。

- **`.psa.config.json` 簡素化**。 `psap0005_relaxed_mode` キーは省略されました (デフォルト値 `false` を採用)。 config ヘッダの PSAP0005 セクションは relaxed-mode マイグレーションベースラインの説明から、 strict-mode 定常状態の説明に書き換えられました。

- **`$Script:ScriptTag` を `psa-py-v4-llm-governance-strict` に更新** (4 スクリプトすべて統一)。 **4 スクリプトすべてが r80 ベースラインで `psa.py 4.0.2 --severity error` を 0 errors / 0 warnings でパスします**。

- **SPEC.md §D.34 (新規)** に完全な post-mortem を記載: なぜ pre-r80 §A.13 の 4 サイクル計画を断念して統合リリースに切り替えたのか、 `psa.py` 4.0.2 のアップリフトが何をもたらしたか、 カテゴリ別 rewrite 表、 同様の将来マイグレーションへの教訓。
...
**`2026-05-25` Chipset r76 / Graphics r42 / BthPan r24 / NPU r20** リリース (`psa-py-v4-llm-governance-baseline`) は **LLM ガバナンスベースライン**リリースで、 `psa.py` 4.0.0 を採用し、 r74 で導入された 9 件の `PSAP0003` 違反 inline revision-tag コメントを整理し、 新規 `PSAP0005` ルールを `psap0005_relaxed_mode: true` でマイグレーションベースラインとして opt-in しました。 r80 リリースが達成したマイグレーションターゲットを設定したリリースです。
...
`psa.py` (latest mainline) は `PSA1001`〜`PSA9002` の汎用ルールに加え、 プロジェクト・パイプライン規約ルール `PSAP0001`〜`PSAP0005` を含む **46 ルール体系** を 9 カテゴリに分けて実装しています。 46 ルールには `psa.py` v3.8.0 で Chipset r73 / Graphics r39 / BthPan r21 リリースとともに導入された `PSA2009` (PSCustomObject property assigned without prior declaration) が含まれます — これは Japanese-locale Windows Server 2019 で発生した `Chipset r72 P05 -> FAILED with "WhqlCoSignAnalysis" property-not-found exception` という実行時障害に対応する静的解析ゲートです。 また `psa.py` v3.9.0 で Chipset r75 / Graphics r41 / BthPan r23 / NPU r19 リリースとともに導入された `PSA2010` (未定義関数呼び出し検出, error) と `PSA2011` (PS 5.1 ja-JP で `Split-Path -LiteralPath ... -Parent` が AmbiguousParameterSet を発生させる検出, error) も含まれており、 これらは r75 §D.33 Defect A (`Split-Path` binder バグ) と §D.32.2 の `Find-Signtool` typo 系統を静的解析時点で捕捉します。 **46 番目のルールである `PSAP0005` (revision reference in comment body) は、 `psa.py` v4.0.0 で導入された LLM 支援メンテナンスガードレールです** — `PSAP0003` の構造化タグ形式よりも広く、 コメント本体内の任意の `rNN` 参照を検出します。 本レポジトリでは Chipset r80 / Graphics r46 / BthPan r28 / NPU r24 リリース (`psa-py-v4-llm-governance-strict`) 以降、 `PSAP0005` を**デフォルトの strict モード**で運用しており、 r76 で採用していた `psap0005_relaxed_mode: true` の移行ベースラインは廃止 (該当キーは `.psa.config.json` から削除済み) です (SPEC §A.13 *Migration roadmap* は `COMPLETED` ステータス、 §D.34 が retrospective)。 PSA2009/2010/2011 の詳細仕様については `SPEC.md` §A.11.5c / §A.11.5d / §A.11.5e を、 PSAP0005 のレポジトリ側ポリシーについては `SPEC.md` §A.13 を参照してください。 本レポジトリは latest mainline の `psa.py` に対して検証する方針です (特定バージョンへの固定はしません)。 方針の根拠と「新しい `psa.py` への追従」 LLM / AI ワークフローについては `SPEC.md` §A.11 *Version policy* を参照してください。 46 ルールは以下 9 カテゴリに分類されます:

| カテゴリ                                | コード範囲                | 例                                                                                                                                                                                                                                              |
| --------------------------------------- | ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 構文の整合性                            | `PSA1001`〜`PSA1003`      | 中括弧 / 丸括弧 / 角括弧のバランス                                                                                                                                                                                                              |
| 意味解析                                | `PSA2001`〜`PSA2011`      | 未定義変数、 自動変数の shadowing、 bare `$variable` に対する `-match`、 `$null` を `-eq`/`-ne` の右辺に置く問題、 条件式内の代入 / リダイレクト、 パラメーター名が自動変数を shadow している (`PSA2007`)、 `$Script:Foo++` の初期化忘れ (`PSA2008`)、 **v3.8.0 新規:** PSCustomObject プロパティを `[pscustomobject]@{...}` イニシャライザに宣言せずに代入している (`PSA2009`)、 **v3.9.0 新規:** 未定義関数の呼び出し (`PSA2010`, error) と PS 5.1 ja-JP で `Split-Path -LiteralPath ... -Parent` が AmbiguousParameterSet を発生 (`PSA2011`, error) |
