...
**最新リリース: `2026-05-25` — Chipset r76 / Graphics r42 / BthPan r24 / NPU r20** (`psa-py-v4-llm-governance-baseline`)。
本リリースは **LLM ガバナンスベースライン** リリースです。 `psa.py` 4.0.0 を採用し、 r74 で導入された 9 件の inline revision-tag コメントを整理し、 新規 `PSAP0005` ルール (revision reference in comment body) を `psap0005_relaxed_mode: true` でマイグレーションベースラインとして opt-in します。

主な変更点 (実行時動作の変更なし、 完全な一覧は CHANGELOG を参照):

- **`psa.py` 4.0.0** が新しい `PSAP0005` ルール — `PSAP0003` の LLM 支援メンテナンス用ガードレール companion — を導入します。 `PSAP0003` が 5 つの構造化タグ形式 (`# r42:`, `# (r42)` 等) のみを検出するのに対し、 `PSAP0005` はコメント本体内の **任意の `rNN` 参照** を検出します ("Added in the r71 release"、 "As of r74, …"、 "Before r74, …" のような散文的アンカー含む)。 本レポジトリの enforcement matrix は [SPEC §A.13](./SPEC.md#a13-development-workflow) (英語のみ) を参照してください。

- **9 件の `PSAP0003` 違反をクリーンアップ**。 r74 リリースで 9 件の inline revision-tag コメント (`# NOTE (r74):` × 3 サイトの byte-identical cross-script ブロック + `# r74:` × 5 Chipset-only + `# NOTE (r74):` × 1 Chipset-only) が残っていました。 r76 で SPEC §A.13 のポリシーに従い、 9 件すべてを timeless wording に書き換えています。 cross-script-shared コメントブロックの PSA8001 byte-identity は維持されています。

- **`PSAP0005` マイグレーションベースライン確立**。 `.psa.config.json` で `PSAP0005` を `psap0005_relaxed_mode: true` で opt-in しています。 relaxed mode は 4 つの定型 prose パターン (SECTION ヘッダ、 SPEC 相互参照、 "Added in the rNN release" 表現、 "Earlier revisions" prose) を許容します。 現在の relaxed-mode カウントがマイグレーションベースラインで、 最終目標は SPEC §A.13 *Migration roadmap* の 4 段階クリーンアップを完了して strict モード (`false`) にフリップすることです。

- **`$Script:ScriptTag` を `psa-py-v4-llm-governance-baseline` に更新** (4 スクリプトすべて統一)。 **4 スクリプトすべてが r76 ベースラインで `psa.py 4.0.0 --severity error` を 0 errors でパスします**。

LLM ガバナンスの基本思想、 per-policy-item の enforcement matrix、 許容 / 不許容 prose 例、 multi-cycle クリーンアップロードマップについては [SPEC §A.13](./SPEC.md#a13-development-workflow) (英語のみ) を参照してください。
