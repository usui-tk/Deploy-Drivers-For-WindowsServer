...
.DESCRIPTION
    ...
    Workstation) and deploys it on a Windows SERVER SKU. The pipeline:
      ...
      Install (I00..I04): Trust the cert into LocalMachine\Root +
                          ...
                          only; no kernel-load claim - SPEC D.58), call pnputil
    ...
    INSTALLATION (modifies system state):
      ...
      I02 AuthorizeDriverSigning
                             ...
                             (AppControl layer only - SPEC D.58) or
                             enable Mode T testsigning (-UseTestSigning).
...
.PARAMETER PfxPassword
    Password for the self-signed PFX. Default: empty = per-run random password (audit P1-G).
...
param(
    ...
    # the AppControlDecision layer only and makes no KernelImageTrust
    # claim (SPEC D.58). Pass -UseTestSigning (Mode T, lab only) for
    # the legacy bcdedit testsigning approach (requires Secure Boot
    # OFF in firmware).
...
if ([string]::IsNullOrWhiteSpace($LogFile)) {
    ...
    $autoLogLeaf  = ('Deploy-MSBthPanInboxOnWindowsServer_{0}_{1}_{2}.log' -f $Action, $autoLogStamp, $PID)
...
function Get-BootSigningEnvironment {
    ...
    $env.BlockReasons = @()
    $supplementalDeployed = ($env.MsBthPanSuppPolicyActive -eq $true)
...
function Show-BootSigningEnvironment {
    ...
    $rows = @(
        ...
        @{ N='WDAC supp (BthPan cert)';  V=(_FmtBool $BootEnv.MsBthPanSuppPolicyActive);Note='AppControl layer only; no kernel-load claim'       }
    )
    foreach ($r in $rows) {
...
#####################################################################
# These helpers build the OPTIONAL App Control supplemental policy
# path of I02 (explicit -WdacBasePolicyGuid required). Per SPEC D.58,
# a supplemental policy acts on the AppControlDecision layer only:
#
#   - The base CI policy keeps enforcing its own signing rules; the
#     supplemental does NOT override the default kernel signing
#     requirements and makes no KernelImageTrust claim (the earlier
#     claim to the contrary is retracted - SPEC D.58.1).
#   - The supplemental references this script's self-signed cert in a
#     kernel signing scenario rule (Add-SignerRule -Kernel).
...
#   for project-signed images. It requires Secure Boot OFF in firmware
#   (the bcdedit write is refused while Secure Boot is on), and it is
#   a lab-only, explicitly opted-in state (gate G-04).
#
...
function Invoke-BthPanPnputilRebind {
    # NOTE: an earlier implementation passed 'NUL' as both
    ...
    # が同じであるため、実行できません"). See CHANGELOG.md for the release
    # in which this was diagnosed and fixed.
    ...
    try {
        ...
        if ($proc1.ExitCode -ne 0) {
            $e1 = ''
            try { $e1 = (Get-Content -LiteralPath $err1 -Raw -ErrorAction SilentlyContinue) } catch { } # psa-disable-line PSA3004 -- best-effort stderr capture; missing/unreadable stderr just means we print the diagnostic without it
        ...
        if ($proc2.ExitCode -ne 0) {
            $e2 = ''
            try { $e2 = (Get-Content -LiteralPath $err2 -Raw -ErrorAction SilentlyContinue) } catch { } # psa-disable-line PSA3004 -- best-effort stderr capture; missing/unreadable stderr just means we print the diagnostic without it
...
function Invoke-BthPanServiceRestart {
    # NOTE: on a host where BthPan was just freshly added by pnputil
    ...
    try {
        ...
        if ($inner) {
            ...
            } catch { } # psa-disable-line PSA3004 -- best-effort native error code extraction; not all .NET exceptions carry a Win32 native code, in which case we proceed without it
        }
        ...
        } catch { } # psa-disable-line PSA3004 -- best-effort sc.exe diagnostic; if sc.exe is missing or fails we just don't print the SCM state
        return $false
    }
}
...
function Invoke-PrepPhase01_PrepareWorkspace {
    ...
    # $Script:LogFileAutoGenerated is the exclusion this check was missing.
    # When -LogFile is omitted the script AUTO-PLACES the transcript under
    # <WorkRoot>\logs\ - all four sisters do, with identical code - and the
    # upstream relocation guard already excludes that case because it owns
    # the suspend/wipe/resume flow for it. This check did not, so the
    # default invocation of BthPan (and only BthPan: the other sisters have
    # no such pre-flight) failed P01 on a path the operator never chose,
    # offering three 'resolutions' none of which apply to an auto-path.
    # The check still fires for an operator-SUPPLIED -LogFile inside
    # -WorkRoot, which is the case it was written for (SPEC SS D.46.2).
    if ($Ctx.CleanWorkRoot -and $Script:LogFileActive -and $LogFile -and -not $Script:LogFileAutoGenerated) {
        try {
            $resolvedLog      = [System.IO.Path]::GetFullPath($LogFile)
...
function Invoke-PrepPhase05_AnalyzeInfs { # psa-disable-line PSA6003 -- compound noun (e.g., Policies, Drivers, Catalogs) is semantically plural for set-returning helpers
    ...
    # INF-side WDF requirement (SPEC D.53). bthpan.inf is the Microsoft inbox
    # driver, so the expected reading is a UMDF-free, low-KMDF package; the
    # column exists so all four sisters answer the same question.
    $wdfReq = Get-InfWdfRequirement -Content (Read-InfFile -Path $infPath).Content
    ...
    # Build the WHQL co-sign analysis (see SPEC §D.31) for the single bthpan.inf and
    # attach to $Ctx so I00 / C6 / I03 (and -SkipNonCosignedDrivers
    # filtering) can read it. BthPan operates on the Microsoft inbox
    # bthpan.inf which is always WHQL co-signed; the analysis is
    # therefore expected to report IsFullyCoSigned=true and C6 will
    # not fire. The call is still made unconditionally so the data
    # contract stays consistent with the AMD scripts. See SPEC SS D.31.
    Set-DebugStep 'r71: build WHQL co-sign analysis for bthpan.inf'
    $whqlInfRecords = @([pscustomobject]@{ InfName = 'bthpan.inf'; InfPath = $infPath })
    try {
...
function Invoke-PrepPhase06_PatchInfs { # psa-disable-line PSA6003 -- compound noun (e.g., Policies, Drivers, Catalogs) is semantically plural for set-returning helpers
    ...
    # Acknowledge the -SkipNonCosignedDrivers switch for BthPan (see SPEC §D.31). BthPan deploys the
    # Microsoft inbox bthpan.inf which is always WHQL co-signed by
    # Microsoft, so the filter would never trim anything. We still
    # acknowledge the flag in the run transcript when it is set so the
    # operator sees that BthPan considered it. See SPEC SS D.31.
    if ($Script:SkipNonCosignedDrivers) {
        if ($Ctx.WhqlCoSignAnalysis -and $Ctx.WhqlCoSignAnalysis.Count -gt 0) {
            $bthPanAnalysis = $Ctx.WhqlCoSignAnalysis[0]
...
function Invoke-InstPhase00_PreInstallReview {
    ...
    $criticalItems = Get-CriticalRiskItem -Ctx $Ctx -Matched @()
...
function Invoke-InstPhase02_AuthorizeDriverSigning {
    ...
    # (bthpan-specific): signal to the phase dispatcher that subsequent
    # phases (I03, I04) MUST NOT run in this same execution. Earlier
    # revisions printed the "reboot then re-run" guidance above but
    # then proceeded to I03/I04 immediately, staging the bthpan.inf
    # entry that kernel CI would reject until the next boot. The
...
function Invoke-InstPhase03_InstallDrivers { # psa-disable-line PSA6003 -- compound noun (e.g., Policies, Drivers, Catalogs) is semantically plural for set-returning helpers
    ...
    # (bthpan-specific): I02 sets $Ctx.RebootRequiredBeforeI03 when it
    # newly enables BCD testsigning. In that case the host's kernel
    # CI cannot yet admit self-signed drivers; staging bthpan.inf via
    # pnputil would still succeed at the trust-store layer (because
    # I01 imported the cert already) but the BTH\MS_BTHPAN load
    # attempt would be rejected with CM_PROB_DRIVER_FAILED_LOAD until
    # the reboot. To keep the install transcript honest and the host
    # in a clean intermediate state, I03 halts immediately when the
    # flag is set. The operator reboots once and re-runs
    # -Action Install: on the re-run, $Ctx.RebootRequiredBeforeI03
    ...
    # D.43.3). This phase runs /install and then /scan-devices; both make
    # Windows re-evaluate devices this script never names.
...
function Invoke-InstPhase04_PostInstallVerification {
    ...
    # (bthpan-specific): mirror the I03 halt - if I02 just enabled
